...
AvatarUserSelect.prototype = {
    init: function(){
        ...
        $("#selectAllByRQButton", this.$context).on("click", function () {
            if($(this).is(":checked")) {
                $.each(self.list, function(key, value) {
                    let item = $("#" + value.linkId );
                    if(self.resultList.indexOf(value.userId) === -1 && item.is(":visible")) {
                        self.resultList.push(value.userId);
                        item.addClass("ow_mild_green");
                    }
                });
            } else {
                self.resultList = [];
                $(".ow_user_list_item").removeClass("ow_mild_green");
                $(".owm_user_list_item").removeClass("ow_mild_green");
            }
            let $countNode = $('div.count_label', self.$context);
