...
/* Comments */
var OwMobileComments = function( contextId, formName, genId, replyCommentId ){
	this.formName = formName;
	this.$cmpContext = $('#' + contextId);
    this.genId = genId;
    this.replyCommentId = replyCommentId;
    this.replyUserDisplayNameId = 'rudnId'+replyCommentId
...
//        );
                      ...
                      if (i.substr(0, 3) === 'del') {
                        ...
                        jc.buttons.ok.action = function () {
                            ...
                            if(o.replyName.length === 0){
                                $('#content-'+o.replyCommentId+'  .owm_newsfeed_comment_input').slideDown('fast');
                                ...
                                if ($('#'+o.replyUserDisplayNameId).css('display') != 'none'){
                                    ...
                                    $('#content-'+o.replyCommentId).find("input[name='replyUserId']").val('');
                                }
                            }
                            else {
                                var offset = 0;
                                ...
                                $('#content-'+o.replyCommentId).find("input[name='replyUserId']").val(o.userId);
                            }
                        }
                    }
                );
	...
	reload:function(){
		...
		$.ajax({
            type: 'POST',
            url: self.respondUrl,
            data: 'cid='+self.cid+'&commentCountOnPage='+self.commentCountOnPage+'&ownerId='+self.ownerId+'&pluginKey='+self.pluginKey+'&displayType='+self.displayType+'&entityType='+self.entityType+'&entityId='+self.entityId+'&initialCount='+self.initialCount+'&loadMoreCount='+self.loadMoreCount,
            dataType: 'json',
            success : function(data){
               if(data.error){
                        OW.error(data.error);
...
$(function() {
    ...
    $selector = ".owm_newsfeed_item .owm_newsfeed_body_status," +
        ...
        ".frmmainpage #mailboxSidebarConversationsItemSubject," +
        ...
        ".owm_frmnews_view .owm_list_item_view_body," +
...
$(document).on("ready", function () {
    $("span.owm_context_arr_c").on("click", function (e) {
        ...
        if ($(dropdown).css('display') === "none")
            $(target).addClass("frmmenu_active_opened_dropdown");
        else
            $(target).removeClass("frmmenu_active_opened_dropdown");
    });
...
function question_with_condition(elname, cname, pattern){
    var sel = $('*[name='+elname+']').closest('.owm_field_container,.owm_form_field_container');
    function check (d){
