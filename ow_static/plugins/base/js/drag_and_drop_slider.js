(function($){
	...
	$.fn.schemeSwitcher = function(options) {
		...
		$node.click(function(e){
			...
			if ($("body").hasClass("ow_admin_area")){
                var container = $(e.target).closest("body.ow_admin_area").find(".ow_page_container.ow_admin")[0];
                var clickedPointRelativeX = e.clientX - $(container).position().left - parseInt( $($(container)
                    .find(".ow_canvas")[0]).css("margin-left") );
            }
			else
                clickedPointRelativeX = e.clientX;

			for( var i=0; i < mpm.length; i++ ) {
				if ( clickedPointRelativeX > mpm[i].x1 && clickedPointRelativeX < mpm[i].x2 ) {
