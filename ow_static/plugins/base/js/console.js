...
OW_DataModel.PROTO = function()
{
    ...
    this.set = function( key, value, notifyObservers )
    {
        ...
        if ( notify && changed )
        {
            if (notifyObservers !== 'dont_call_observer')
            {
...
OW_Console = function( params, modelsData )
{
    ...
    var command = OW.getPing().addCommand('consoleUpdate',
    {
        ...
        after: function( dataList )
        {
            ...
            OW.trigger('userLoggedOutPopUp', {'data' : dataList});
...
OW_ConsoleItem =
{
    ...
    showContent: function()
    {
        ...
        if(document.getElementsByTagName("html")[0].getAttribute("lang")=="en"){
            var leftValue = $("#"+this.uniqId).position().left + 25;
            this.$contentNode.css("right","unset");
        }else{
            var leftValue = $("#"+this.uniqId).position().left;
        }
        var topValue  = $("#"+this.uniqId).position().top + 20;

        this.$contentNode.css("left",leftValue+"px");
        this.$contentNode.css("top",topValue+"px");
...
OW_ConsoleList =
{
    ...
    _ajaxSuccess: function( resp )
    {
        ...
        if ( resp.data )
        {
            this.data.set(resp.data, undefined, 'dont_call_observer');
        }
...
//notification page
function load_notifications(root_selector, ajax_url){
    var item_ids = [];
    var reached_end = false;
    var load_items = function(items){
        $.each(items, function (i, item) {
            var $item = $(item.html).data('id', item.id);
            ...
            $('.ow_console_list', root_selector).append($item);
    ...
    var send_request = function () {
        if (reached_end)
            return;
        $('.ow_preloader_content', root_selector).show();
        $('.ow_loadmore_button', root_selector).hide();

        var request = JSON.stringify({
            console: OW.Console.getData('console').get(),
            data: {},
            target: 'notification',
            offset: item_ids.length,
            ids: item_ids
        });
        $.ajax({
                type: 'post',
                ...
                success: function (resp) {
                    ...
                    if (resp.items.length) {
                        load_items(resp.items);
                        if(resp.data.listFull==true){
                            reached_end = true;
                        }
                    }
                    else {
                    ...
                    if (item_ids.length === 0) {
                        $('.ow_nocontent', root_selector).css('visibility','initial');
                    }
                    else {
                        $('.ow_nocontent', root_selector).css('visibility','hidden');
                    }
                },
                complete: function () {
                    $('.ow_preloader_content', root_selector).hide();
                    if (!reached_end) {
                        $('.ow_loadmore_button', root_selector).show();
                    }
                }
            });
    };
    ...
    send_request();
    $('.ow_loadmore_button', root_selector).click(function() {
        send_request();
    });
}