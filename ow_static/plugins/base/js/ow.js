var OwUtils = function(){
    ...
    this.showTip = function( $el, params ){
        params = params || {};
        var slideType;
        if($el[0].classList.value.includes("ow_console_item")){
            slideType = 'bot';
            if($el.offset().left < 30){
                slideType = 'right';
            }
        }else{
            slideType = 'top';
        }
        params = $.extend({side:slideType, show:null, width:null, timeout:0, offset:5, hideEvent: null}, params);
    ...
    this.showImageInFloatBox = function( src )
    {
        ...
        $('.floatbox_container').css("min-width", "auto");
...
function OW_FloatBox(options)
{
    ...
    function(event) {
            if (event.keyCode == 27) {
                if($('.jconfirm.jconfirm-material.jconfirm-rtl.jconfirm-open').length == 0 && escFlip == 0){
                    escFlip = 1;
                    var jc = $.confirm(OW.getLanguageText('base', 'are_you_sure'));
                    jc.buttons.ok.action = function () {
                        fl_box.close({
                    ...
                    $('.jconfirm.jconfirm-material.jconfirm-rtl.jconfirm-open button.btn.btn-default').click();
                }
                setTimeout(
                    function()
                    {
...
OwForm.prototype = {
    ...
    submitForm: function(){
        ...
        try{
            this.validate();
            $("#" + this.id).find("[clicked=true]").addClass('ow_inprogress');
        }catch(e){
            if( this.showErrors ){
                OW.error(e);
            }
            $("#" + this.id).find("[clicked=true]").removeClass('ow_inprogress');
            return false;
        }
        ...
        if($('body').find(".ow_blogentry_compose").length ||
           $('body').find(".ow_create_group ").length ||
           $('body').find(".ow_blogpost_compose").length){
...
var OwComments = function( params ){
    ...
    this.$hiddenInput = $('#'+this.hiddenInputId);
    this.$replyUserDisplyNameSpan = $('#'+this.replyUserDisplayNameId);
...
OwComments.prototype = {
    ...
    sendMessage: function( message ){
        ...
        var dataToSend = {
            ...
            replyUserId: self.$hiddenInput.val()
        ...
        var openReplyLists = [];
        var fullListCmp = $( '#'+self.contextId );
        // console.log(self.cid);
        // console.log(self);
        fullListCmp.find(".ow_reply_list").each(function () {
            if(this.style.display === 'block')
                openReplyLists.push(this.id);
        });
        $.ajax({
            type: 'post',
            url: self.addUrl,
            ...
            success: function(data){
                ...
                $.each(openReplyLists, function( index, value ) {
                ...
                if (self.$replyUserDisplyNameSpan.css('display') === 'table'){
                    var newPadding = parseInt(self.$textarea.css('padding-right').split('px')[0]) - parseInt(self.$replyUserDisplyNameSpan.css('width').split('px')[0])-5 + 'px';
                    self.$textarea.css('padding-right',newPadding);
                }
                self.$replyUserDisplyNameSpan.html('');
                self.$replyUserDisplyNameSpan.css('display','none');
                if (typeof (dataToSend.replyId) !== 'undefined') {
                    $('#comments-' + self.uid + ' .ow_feed_comments_viewall').click();
                }
            },
            error: function( XMLHttpRequest, textStatus, errorThrown ){
                self.$formWrapper.removeClass('ow_preloader');
...
var OwCommentsList = function( params ){
    ...
    this.commentListId = params['cid'];
    var self = this;
    if(params['replyCommentId'] > 0){
        $('.ow_feed_comments_viewall', this.$context).click(function(){
            let reply_list = $('.ow_reply_list', self.$context);
            if (reply_list.is(":visible")) {
                if (params['repliesCountToLoad'] > 0) {
                    self.initialCount += self.loadMoreCount;
                    self.reload();
                }
            }
...
OwCommentsList.prototype = {
	init: function(){
	var self = this;
        $('.ow_comments_item', this.$context).hover(function(){
            $('.cnx_action', this).show();
            }, function(){
            $('.cnx_action', this).hide();
            $('.ow_tooltip.ow_comments_context_tooltip',this).hide();
            $('.ow_comments_date_hover', this).show();
        });
...
//        );
        ...
        OW.trigger('base.comments_list_init', {entityType: this.entityType, entityId: this.entityId, replyCommentId: this.replyCommentId}, this);
                    ...
                    $('#'+i).unbind("click").click(
                        function(){
                            if (i.substr(0, 3) === 'del'){
                                    ...
                                    var openReplyLists = [];
                                    var fullListCmp = $( '#comments-'+self.cid );
                                    fullListCmp.find(".ow_reply_list").each(function () {
                                        if(this.style.display === 'block')
                                            openReplyLists.push(this.id);
                                    });
                            ...
                            else if(i.substr(0, 5) === 'reply'){
                                var commentListcmp = $('#content-'+o.replyCommentId);
                                commentListcmp.find('textarea').css('display', 'none');
                                if(o.replyName.length === 0){
                                    $('#content-'+o.replyCommentId+' .ow_comments_form_wrap ').slideDown('fast');
                                    $('#content-'+o.replyCommentId).find('textarea').focus();
                                    if ($('#'+o.replyUserDisplayNameId).css('display') != 'none'){
                                        var span = $('#'+o.replyUserDisplayNameId);
                                        var textArea = $('#content-'+o.replyCommentId).find('textarea');
                                        var newPadding = parseInt(textArea.css('padding-right').split('px')[0]) - parseInt(span.css('width').split('px')[0])-5 + 'px';
                                        span.css('display', 'none');
                                        span.html('');
                                        textArea.css('padding-right',newPadding);
                                        $('#content-'+o.replyCommentId).find('input[type=hidden]').val('');
                                    }
                                }
                                else {
                                    var offset = 0;
                                    ...
                                    if (span.css('display') != 'none'){
                                        offset = - parseInt(span.css('width').split('px')[0])-5;
                                        $('#content-'+o.replyCommentId).find('input[type=hidden]').val('');
                                    }
                                    span.html(o.replyName);
                                    span.append('<span class="ow_comment_reply_text">' + o.commentContent + '</span>');
                                    span.css('display', 'table');
                                    var a = parseInt(textArea.css('padding-right').split('px')[0]);
                                    var b = parseInt(span.css('width').split('px')[0]);
                                    newPadding =  a+b+5 + offset+ 'px';
                                    span.css('margin-right',parseInt(textArea.css('padding-right').split('px')[0])+offset+'px');
                                    textArea.css('padding-right',newPadding);
                                    span.click(function (){
                                        var textArea = $('#content-'+o.replyCommentId).find('textarea');
                                        var newPadding = parseInt(textArea.css('padding-right').split('px')[0]) - parseInt(span.css('width').split('px')[0])-5 + 'px';
                                    ...
                                    $('#content-'+o.replyCommentId).find('input[type=hidden]').val(o.userId);
                                }
                            }
                        }
                    );
        ...
        $.each(this.actionArray.users,
            function(i,o){
                $('#'+i).click(
                    function(){
                        if ( self.actionArray.users[i]['command'] == 'suspend' )
                        {
                            OW.ajaxFloatBox("BASE_CMP_CommentSetSuspendMessage", [i,self.actionArray.users[i]['comment_list_id']],{width: 520, title: OW.getLanguageText('base', 'suspend_floatbox_title', {'displayName': o['display_name']})});
                        }
                        else
                        {
                            OW.trigger("base.on_suspend_command_"+self.commentListId, ["unsuspend",i])
                        }
                    }
             );
            }
        ...
        OW.bind("base.on_suspend_command_"+self.commentListId, function( command,userSuspendId,message ) {
            var data = self.actionArray.users[userSuspendId];
            OW.Users[command == "suspend" ? "suspendUser" : "unSuspendUser"](data['uid'],data['suspend_code'],data['un_suspend_code'], null, message);
            location.reload();
        });
...
OW_Ping = function()
{
    var _stack = [], _commands = {}, lastCommands = [];
    ...
    OW.bind('base.socket.onmessage', function(message){

        // on complete
        $(lastCommands).each(function(i, command)
        {
            command._completed();
        });
        refreshRootCommand();
        ...
        if (result && result.type === 'streamOnlineUser') {
            addDataToStreamOnlineUserChart(result.time, result.number);
        }
...
OW_UsersApi = function( _settings )
{
    ...
    this.suspendUser = function( userId, suspendCode,unSuspendCode, callback, message ) {
        return _query("suspend", {userId: userId,code:suspendCode,message: message}, callback);
    };
...
function flagOptionChanged($this){
    ...
    $('.flags-container form[name="flag"]').hide();
    $('.flags-container').addClass('ow_preloader_content');
}
...
function ShowLabelRealname(questionrealname) {
    $("#DisplayNameSearchForm .ow_search_realname label").show();
}
function makeid () {
    var text = "";
...
function ShowLabelQuestion(Textboxinput){
    var textid = Textboxinput.id;
    var element = document.getElementById(Textboxinput.id).parentElement.parentElement;
    var element_id = 'text_id_' + makeid();
    element.setAttribute('id', element_id);
    $("#"+element_id+" label").show();
    $("#"+element_id+" #"+textid).addClass("ow_border_when_typing");
    if(Textboxinput.value == ""){
        $("#"+element_id+" label").hide();
        $("#"+element_id+" #"+textid).removeClass("ow_border_when_typing");
    }
}

$(document).ready(function () {
    if ($("body").hasClass("user_roles")) {
        $(document).on("click", "body", function (e) {
            if (!($(e.target).hasClass("multiselect_container") || $(e.target).closest(".multiselect_container").hasClass("multiselect_container"))) {
                $(".active_mltiselect").removeClass("active_mltiselect").slideUp();
            }
        });
    }
    if ($("body").hasClass("profile_edit") || $("body").hasClass("base_user_join")) {
        var NumberOfInput = ($(".ow_type_text input[type='text']").length);
        for (var i = 0; i < NumberOfInput; i++) {
            var id_text = $(".ow_type_text input[type='text']")[i].id;
            if ($(".ow_type_text input[type='text']")[i].value != "") {
                $(".ow_type_text label[for=" + id_text + "]").show();
                $(".ow_type_text #" + id_text).addClass("ow_border_when_typing");
            }
        }
        var NumberOfTexarea = ($(".ow_type_text  textarea").length);
        for (var i = 0; i < NumberOfTexarea; i++) {
            var id_text = $(".ow_type_text textarea")[i].id;
            if ($(".ow_type_text textarea")[i].value == "") {
                $(".ow_type_text label[for=" + id_text + "]").hide();
            } else {
                $(".ow_type_text label[for=" + id_text + "]").show();
                $(".ow_type_text #" + id_text).addClass("ow_border_when_typing");
            }
        }
        var NumberOfInputPassword = ($(".ow_type_text input[type='password']").length);
        for (var i = 0; i < NumberOfInputPassword; i++) {
            var id_text = $(".ow_type_text input[type='password']")[i].id;
            if ($(".ow_type_text input[type='password']")[i].value != "") {
                $(".ow_type_text label[for=" + id_text + "]").show();
                $(".ow_type_text #" + id_text).addClass("ow_border_when_typing");
            }
        }
    }
    $(document).on("click", ".ow_content_menu li.active a", function (e) {
        e.preventDefault()
    });

    $(".ow_avatar_label").each(function () {
        var $this = $(this);
        if (is_text_persian($this.attr('title')))
            $this.css("direction", "rtl");
        else
            $this.css("direction", "ltr");
    });

    $(document).on('click', ".download_admin_charts_data", function (e) {
        TableExport(  $(".admin_chart_data_table")  , {
            formats: ['xlsx'],
            exportButtons: true,
            trimWhitespace: true,
            filename: 'admin_charts_data'
        });
        $(e.target).closest(".admin_main_charts_container").find(".admin_chart_data_table").find(".xlsx")[0].click()
    }).on("change", "body.my_photos .ow_photo_search_actions_container select[name='choose_photo_search_show_view']," +
        "body.user_album_page .ow_photo_search_actions_container select[name='choose_photo_search_show_view']," +
        "body.friends_photos .ow_photo_search_actions_container select[name='choose_photo_search_show_view']," +
        "body.user_photos .ow_photo_search_actions_container select[name='choose_photo_search_show_view']", function(e){
        window.location = $(e.target).find(":selected").data("url");
    });
    ...
    $("#updatestatus_submit_button").on('click', function() {
        if ($('html').is(':lang(en-US)')) {
            $('#newsfeed_update_status_info_id').css("direction","ltr");
        }else if($('html').is(':lang(fa-IR)')){
            $('#newsfeed_update_status_info_id').css("direction","rtl");
        }
        if($('#emoji_menue_newsfeed').hasClass('frmemoji_button_close')){
            $('#emoji_menue_newsfeed').click();
        }
    });

    $(document).on('click', '#newFlagSendButton', function(){
        flagOptionChanged( $(".flag_floatbox_options_container").find("input:checked") );
    });

    $(document).on('change', ".flag_floatbox_options_container input", function(e) {
        $(".selected_flag_option").removeClass("selected_flag_option");
        $(e.target).closest("li").addClass("selected_flag_option");
        window.hamed = e;
    });

    $(document).on("change", ".multiselect_container.select_categories .list_container li input[type='checkbox']", function (e) {
        var changedItem = $(e.target);
        var changedItemChecked = $(changedItem).is(":checked");
        if (changedItem.hasClass("check_all_categories")){
            $(".select_categories .list_container li input[type='checkbox']").not(e.target).prop("checked", changedItemChecked).change();
        } else if ( $(changedItem).attr("class")!= undefined && ($(changedItem).attr("class")).indexOf("category_title") >= 0 ) {
            var relatedHeader = "table.users_roles_table th." + changedItem.attr("class");
            $(".multiselect_container.select_categories .list_container li input[type='checkbox'][name*='" + changedItem.data("category") + "']").prop("checked", changedItemChecked).change();
            if (changedItemChecked) {
                $(relatedHeader).show();
            } else {
                $(relatedHeader).hide();
            }
        } else{
            var relatedRow = "table.users_roles_table tr." + changedItem.attr("name");
            if (changedItemChecked) {
                $(relatedRow).show();
            } else {
                $(relatedRow).hide();
    ...
    $(document).on("click", ".users_roles_filters_container .multiselect_container .list_container", function (e) {
        if (e.stopPropagation) {
            e.stopPropagation();
        }
    });

    $(document).on("click", ".users_roles_filters_container .multiselect_container", function () {
        var that = $(this);
        var active_item_clicked = false;
        if (that.find(".list_container ").hasClass("active_mltiselect"))
            active_item_clicked = true;
        $(".active_mltiselect").removeClass("active_mltiselect").slideUp();
        if (!active_item_clicked)
            that.find("ul.list_container").slideDown().addClass("active_mltiselect");
    });

    $(document).on("change", ".multiselect_container.select_roles .list_container li input[type='checkbox']", function (e) {
        var changedItem = $(e.target);
        var changedItemChecked = $(changedItem).is(":checked");
        if (changedItem.hasClass("check_all_roles")){
            $(".select_roles .list_container li input[name*='role_index_']").prop("checked", changedItemChecked).change();
        }else {
            var relatedColumnsNumber = $(changedItem).attr('name').split("role_index_")[1];
            var number_of_checked_roles = $(changedItem).closest(".select_roles").find('input[name*="role_index_"]:checked').length;
            var user_role_title_in_roles_table_header = $(".user_role_title_in_roles_table_header");
            switch (number_of_checked_roles) {
                case 1:
                    $(user_role_title_in_roles_table_header).css("min-width", "33%");
                    break;
                case 2:
                    $(user_role_title_in_roles_table_header).css("min-width", "25%");
                    break;
                case 3:
                    $(user_role_title_in_roles_table_header).css("min-width", "20%");
                    break;
                case 4:
                    $(user_role_title_in_roles_table_header).css("min-width", "16%");
                    break;
                default:
                    $(user_role_title_in_roles_table_header).css("min-width", "50px");
            }
            if (changedItemChecked) {
                $("table.users_roles_table tr").each(function() {
                    $(this).find("td:eq(" + relatedColumnsNumber + ")").show();
                    $(this).find("th:eq(" + (parseInt(relatedColumnsNumber) + 2) + ")").show();
                });
            }else {
                $("table.users_roles_table tr").each(function() {
                    $(this).find("td:eq(" + relatedColumnsNumber + ")").hide();
                    $(this).find("th:eq(" + (parseInt(relatedColumnsNumber) + 2) + ")").hide();
                });
            }
        }
    });
...
function is_text_persian(text){
    var persianRegex = /[\u0600-\u06FF]|[\u0750-\u077F]|[\uFB50-\uFDFF]|[\uFE70-\uFEFF]/;
    if(persianRegex.test(text))
        return true;
    return false;
}

OW.bind("base.socket_prepare_console", function (data) {
    let checked =false;
    if (data.data != undefined) {
        const result = {
            "stack": [{
                'command': 'consoleUpdate',
                'result': {}
            }]
        };
        switch (data.data.type) {
            ...
            case "notification":
                result['stack'][0]['result'] = data.data.params;
                checked = true;
                break;
        }
        if(checked){
            OW.trigger('base.socket.onmessage', JSON.stringify(result));
        }
    }
    return checked;
});
 ...
 */
function init_socket(socket_host){
    socket = new WebSocket(socket_host);
    socket.onopen = function(e) {
        socket.send(JSON.stringify({'type':'introduce', 'auth': socket_auth}));
        socket.send(JSON.stringify({'type':'ping', 'auth': socket_auth}));
        OW.trigger('base.socket.onopen', e.data);
        fetchConsoleItemsData();
    };

    socket.onmessage = function(e) {
        const handleSocketInPrepare = OW.trigger('base.socket_prepare_message', JSON.parse(e.data));
        const handleSocketInvitation = OW.trigger('base.socket_prepare_console', JSON.parse(e.data));
        if(!handleSocketInPrepare && !handleSocketInvitation){
            OW.trigger('base.socket.onmessage', e.data);
        }
    };
...
function streamOnlineUser() {
    OW.bind('base.socket.onopen', function(message){
        socket.send(JSON.stringify({'type':'streamOnlineUser', 'auth': socket_auth}));
    });
}

function addDataToStreamOnlineUserChart(label, number) {
    if (streamOnlineUserChart) {
        streamOnlineUserChart.addData([number], label);
    }
}

/**
 * CONSOLE SOCKET
 * fetch request for console elements
 */
function fetchConsoleItemsData(){
    $.ajax({
        url: OW_URL_HOME + "base/console/fetch-console-data/",
        type: 'GET',
        dataType: 'json',
        success: function(data){
        },
...
function get_avatar_info(avatar_url, user_id) {
    let is_empty = false;
    let digit = user_id % 9;
    let color = "#eee";
    if (avatar_url === undefined || avatar_url.split("/").pop() === 'default_user_avatar.svg'){
        is_empty = true;
        switch (digit) {
            case 0:
                color = '#34ABE3';
    ...
    return {'is_empty': is_empty, 'color': color, 'digit': digit};
}

function profile_edit_change_password_button() {
    window.oldPassword.floatBox = new OW_FloatBox({$title: OW.getLanguageText('base', 'change_password'), $contents: $('#change-password-div'), width: 480});
    window.owForms['change-user-password'].resetForm();
    window.owForms['change-user-password'].removeErrors();
}

/**
 * Conditional profile questions
 * @author Farzan Mohammadi <farzan.mohamadii@gmail.com>
 */
...
function question_with_condition(elname, cname, pattern){
    var sel = $('*[name='+elname+']').closest('tr');
    function check (d){
        var text1 = $('select[name='+cname+'] option:selected').html(); //selectbox
        var text2 = $('input[name='+cname+']').val(); // textbox
        var text3 = $($('input[name='+cname+']:checked').prop('labels')).text() // checkbox

        if (wildTest(pattern, text1)){
            $(sel).show(600);
            return;
        }
        if (wildTest(pattern, text2)){
            $(sel).show(600);
            return;
        }
        if (wildTest(pattern, text3)){
            $(sel).show(600);
            return;
        }
        $(sel).hide(600);
    }
    $('*[name='+cname+']').change(check);
    check(0);
}
/*-------------------------*/
...
function searchUsersByRQ(filterUrl,loadMoreUrl){
    let SearchUserFormData = $("#SearchUserForm").serialize();
    $.ajax({
        type: 'POST',
        url: filterUrl,
        data: SearchUserFormData,
        success: function (data) {
            data = JSON.parse(data);
            totalUsersCount = data.totalUsersCount;
            loadingPage = data.page+1;
            loadMoreFilteredusers=true;
            moreUrl = loadMoreUrl;
            if(data.users.length < 5)
            {
                loadMoreFilteredusers = false;
            }
            let entityType = $('#SearchUserForm').find('input[name="entityType"]').val();
            let entityId = $('#SearchUserForm').find('input[name="entityId"]').val();
            let extraParams;
            extraParams = $('#SearchUserForm').find('input[name="extraParams"]').val();
            let searchAccountType;
            if($('#searchAccountTypeId').length) {
                searchAccountType = $('#searchAccountTypeId').val();
            }
            let questionValues = data.questionValues;
            $("#SearchUserForm").closest('.floatbox_container').find('.ow_ic_delete.close').click();
            floatBox = OW.ajaxFloatBox('BASE_CMP_SearchUserListSelect', [data.users,entityType,entityId,searchAccountType,data.questionValues,extraParams],
                {
                    width:600,
                    height:350,
                    iconClass: 'ow_ic_user'
                })
            let checkUserListLoaded = setInterval(function() {
                if ($('.asl_users').length) {
                    listenForScrollEvent($('.asl_users'));
                    listenForOnChangeEvent($('#searchAccountTypeId'));
                    clearInterval(checkUserListLoaded);
                }
            }, 100);
        },
...
OW.bind('base.search_user_list_select',
    function(list) {
        let checkUserSearchComponentLoaded = setInterval(function () {
            if ($('#SearchUserForm').length) {
                if($('#searchAccountTypeId').length) {
                    listenForOnChangeEvent($('#searchAccountTypeId'));
                }
                clearInterval(checkUserSearchComponentLoaded);
            }
        }, 100);
    })

function listenForOnChangeEvent(element){
    element.on('change', function() {
        let entityType = $('#SearchUserForm').find('input[name="entityType"]').val();
        let entityId = $('#SearchUserForm').find('input[name="entityId"]').val();
        let extraParams;
        extraParams = $('#SearchUserForm').find('input[name="extraParams"]').val();
        let selectedAccountType = this.value;
        $("#SearchUserForm").closest('.floatbox_container').find('.ow_ic_delete.close').click();
        let userIdList = new Array();
        let floatBox = OW.ajaxFloatBox('BASE_CMP_SearchUserListSelect', [userIdList,entityType,entityId,selectedAccountType,,extraParams],
            {
                width:600,
                height:385,
                iconClass: 'ow_ic_user'
 ...
 * @param element
 */
function listenForScrollEvent(element){
    //TODO return should be removed.
    // copy event of clicking event of AvatarUserSelect for elements is handled, but the problem is the id parameter for click must change
    return;
    ...
    element.on("scroll", function() {
        ...
        if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight && loadMoreFilteredusers) {
            let SearchUserFormData = $("#SearchUserForm").serialize();
            SearchUserFormData+="&page="+loadingPage;
            $.ajax({
                type: 'POST',
                url: moreUrl,
                data:  SearchUserFormData,
                success: function (data) {
                    data = JSON.parse(data);
                    if (data != '') {
                        if (Object.keys(data.arrayToAssign).length < 5) {
                            loadMoreFilteredusers = false;
                        }

                        $.each(data.contentHtmlArr,
                            function (index, foreachData) {
                                $('.asl_users').append(foreachData);
                                let events = $('.asl_users').children().first().data('events');
                                let currentDivId = data.jsArray[index]["linkId"];
                                $.each(events, function() {
                                    // iterate registered handler of original
                                    $.each(this, function() {
                                        $("#"+currentDivId).bind(this.type, this.handler);
                                    });
                                });
                            });
                    } else {
