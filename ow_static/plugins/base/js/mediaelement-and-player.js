...
// Feature configuration
	...
	Object.assign(_player2.default.prototype, {
		...
		buildfullscreen: function buildfullscreen(player, controls, layers, media) {
			...
			var t = this;
			if (t.isVideo) {
				var downloadBtn = $('<div class="' + t.options.classPrefix + 'button ' + t.options.classPrefix + 'download-button">' + ('<button type="button" aria-controls="' + t.id + '" title="Download" aria-label="Download"></button>') + '</div>').appendTo(controls).on('click', function () {
					var url = t.node.currentSrc;
					var filename = url.split('/').pop();
					var match = document.cookie.match(new RegExp('(^| )UsingMobileApp=([^;]+)'));
					if (match){
						window.location = url;
					}else{
						var a = $("<a>").attr('target','_blank').attr("href",url).attr("download",filename).appendTo("body");
						a[0].click();
						a.remove();
					}
				});
		...
		buildprogress: function buildprogress(player, controls, layers, media) {
			...
			var handleMouseMove = function handleMouseMove(e) {
					...
					if (media.duration) {
						...
						// changed by frm because of seeking bug
					...
					bigPlay = $('<div class="' + t.options.classPrefix + 'overlay ' + t.options.classPrefix + 'layer ' + t.options.classPrefix + 'overlay-play">' + ('<div class="' + t.options.classPrefix + 'overlay-button" role="button" ') + ('aria-label="' + _i18n2.default.t('mejs.play') + '" aria-pressed="false">') + '</div>' + '</div>').appendTo(layers).on('click', function () {
						...
						if (t.options.clickToPlayPause) {
							...
							if (media.paused) {
								media.play();
								/* ====================Script==================== */
								/*This block is for making video thumbnails*/
								if($(this).parent().parent().parent().parent().attr('id')!=null) {
									...
									if (postername && postercontroller &&
										($('#items_bar_' + id + ' .ow_newsfeed_preview_grid_item').length == 0 || $('#items_bar_' + id + ' .ow_newsfeed_preview_grid_item a:first-child').css('opacity') == 1)) {
