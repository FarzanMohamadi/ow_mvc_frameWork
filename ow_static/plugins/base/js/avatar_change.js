var avatarChange = function( params )
{
    ...
    this.$backButton_preloader = $('#avatar-preloader-back-btn');
    ...
    this.entityId = '';
    this.id = '';
    this.cancel_upload_image=0;

    /*
        this code is added because for some photos, crop doesn't work properly  issue #4618
     */
    //---------------------
    this.$width='';
    this.$height='';
    ...
    this.initCrop = function( callback ){
        ...
        image.onload = function(e) {
            ...
            if(document.avatarFloatBox.$header!= undefined) {
                document.avatarFloatBox.$header.find(".floatbox_title span").html(OW.getLanguageText('base', 'avatar_crop'));
                $(".floatbox_title").addClass("avatar_crop_heading");
            }else if(document.avatarFloatBox.heading!= undefined){
                document.avatarFloatBox.heading.html(OW.getLanguageText('base', 'avatar_crop'));
            }
            if(document.avatarFloatBox.$body!=undefined) {
                var cropWidth = document.avatarFloatBox.$body.width() - self.params.minCropSize - 16;
            }else if( document.avatarFloatBox.body!=undefined){
                var cropWidth = document.avatarFloatBox.body.width() - self.params.minCropSize - 16;
            }
            ...
            self.$cropTarget.Jcrop({
                ...
                self.$step1.hide();
                if(self.cancel_upload_image == 1){
                    self.$step1.show();
                    self.cancel_upload_image = 0;
                    $(".floatbox_title").removeClass("avatar_crop_heading");
    ...
    this.$backButton.click(function(){
        ...
        $('#avatar-change-step-1 .ow_photo_library_wrap').show();
        $(".floatbox_title").removeClass("avatar_crop_heading");
        document.avatarFloatBox.$header.find(".floatbox_title span").html(OW.getLanguageText('base', 'avatar_change'));
    });

    this.$backButton_preloader.click(function(){
        self.cancel_upload_image=1;
        self.$step1.show();
        $('#photo-album-form').show();
        $('#avatar-change-step-1 .ow_photo_library_wrap').show();
    });
    ...
    self.$fileInput.change(function(e){
        ...
        self.$preloader.height("45px");


        OW.inProgressNode(self.$preloader);
        self.$preloader.show();
        $('#photo-album-form').hide();
        $('#avatar-change-step-1 .ow_photo_library_wrap').hide();
    ...
    this.uploadFile = function(formData)
    {
        ...
        var jqXHR = $.ajax({
            ...
            beforeSend: function() {
                ...
                OW.trigger('base.progress_bar_actions', { 'action': 'progress', 'percentComplete': "50" });
            },
            success: function(data){
                if (data.result) {
                    OW.trigger('base.progress_bar_actions', { 'action': 'progress', 'percentComplete': "100" });
