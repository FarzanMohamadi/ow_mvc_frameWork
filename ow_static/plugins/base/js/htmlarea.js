(function($) {
    ...
    jHtmlArea.fn = jHtmlArea.prototype = {
            ...
            if (elem.nodeName.toLowerCase() === "textarea") {
                ...
                var tempStyles = function(){
                    ...
                    if( !window.htmlAreaData.tempStyles ){
                        var baseCss = false, htmlareaCss = false;
                        
                        $.each(document.styleSheets, function(index, data){
                            if( data.href && data.href.match(/base\.css/) ){
                                baseCss = data;
                            }
                            if( data.href && data.href.match(/htmlarea\.css/) ){
                                htmlareaCss = data;
                            }
                        });

                        var base_rules, htmlarea_rules;
                        if( self.isMsie() ){
                            if( !baseCss || !baseCss.rules ){
                                return;
                            }

                            base_rules = baseCss.rules;
                            htmlarea_rules = htmlareaCss.rules;
                        }
                        else{
                            ...
                            base_rules = baseCss.cssRules;
                            htmlarea_rules = htmlareaCss.cssRules;
                        }
        ...
        video: function(){
            this.saveCaret();
            var self = this;
            var $contents = $('<div>'+window.htmlAreaData.labels.common.videoTextareaLabel+'<br /><textarea name="code" style="height:200px;"></textarea><br /><br /><div><p id="forum_video_textarea_guidance_label">'+window.htmlAreaData.labels.common.videoTextareaLabelGuidance+'</p></div></div>');
    ...
    var priv = {
        ...
        attachEditorEvents: function() {
            ...
            $(self.textarea).closest('form').submit(function() {
                self.toggleHTMLView();
                self.toggleHTMLView();
            });
        }
