...
final class FORUM_BOL_ForumService
{
    ...
    public function getCustomSectionGroupList()
    {
        ...
        $forumSectionsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_FORUM_SECTIONS_RETURN, array('sectionGroupList' => $sectionGroupList)));
     ...
     * @return array
     */
    public function getUserLatestGroupsTopicList( $topicLimit, $excludeGroupIdList = null,$userId ,$selectedGroupIds=array())
    {
        $topicList = $this->topicDao->findUserLastTopicGroupsList($topicLimit, $excludeGroupIdList, $userId,$selectedGroupIds);
        ...
        foreach ( $topicList as $topic )
        {
            ...
            $topic['lastPost'] = $postInfo;
            $topic['topicUrl'] = OW::getRouter()->urlForRoute('topic-default', array('topicId' => $topic['id']));
            $topic['groupEntityUrl'] = OW::getRouter()->urlForRoute('groups-view', array('groupId' => $topic['groupEntityId']));
            $topic['new'] = ($userId && !in_array($topic['id'], $readTopicIds));
            $topics[] = $topic;
        }
    ...
    public function addPost( FORUM_BOL_Topic $topicDto, array $data )
    {
        ...
        $event = new OW_Event('feed.action', $params,array('topicId'=> $postDto->topicId));
     ...
     * @param integer $page
     * @param $reverse_sort
     * @param bool $getFixedPostsOnly
     * @return array
     */
    public function getTopicPostList( $topicId, $page, $reverse_sort = null, $getFixedPostsOnly = false)
    {
        $count = $this->getPostPerPageConfig();
        $first = ($page - 1) * $count;

        if ($getFixedPostsOnly)
            $postDtoList = $this->postDao->findTopicOpeningAndClosingPosts($topicId);
        else
            $postDtoList = $this->postDao->findTopicPostList($topicId, $first, $count, null, $reverse_sort);
        ...
        //prepare topic posts
        $iteration = 0;
        $topicDto = $this->findTopicById($topicId);
        foreach ( $postDtoList as $postDto )
        {
            $post = array(
                'id' => $postDto->id,
                ...
                'edited' => array(),
                'isFirstPost'=> ($getFixedPostsOnly && $iteration == 0) ? true : false,
                'isClosingPost' => ($topicDto->conclusionPostId == $postDto->id) ? true : false,
                'commentsCount' => isset($postDto->entityId) ? $postDto->commentsCount : 0
            );
    ...
    {
        return (int) $this->postDao->findTopicPostCount($topicId) - 1;
    }
     ...
     * @return string
     */
    public function getPostUrl( $topicId, $postId, $anchor = true, $page = null, $params = array())
    {
        if ( empty($page) || !$page )
        {
            $count = $this->getPostPerPageConfig();
            $postNumber = 0;
            if (isset($params['cache']['topics_posts'][$topicId][$postId])) {
                $posts = $params['cache']['topics_posts'][$topicId];
                foreach ($posts as $key => $value) {
                    if ((int) $key <= (int) $postId) {
                        $postNumber += 1;
                    }
                }
                $postNumber -= 1;
    ...
    public function deleteTopic( $topicId )
    {
        ...
        //delete group topic notification
        $topicDto = $this->findTopicById($topicId);
        if ( $topicDto ){
            $forumGroup = $this->findGroupById($topicDto->groupId);
            $forumSection = $this->findSectionById($forumGroup->sectionId);
            if ($forumSection->isHidden && $forumSection->entity == 'groups'){
                OW::getEventManager()->trigger(new OW_Event('notifications.remove', array(
    ...
    {
        $result = [];
        $entities = $this->getTextSearchService()->advancedFindEntities($keyword, $first, $count, $userId, $parts, $period, $sort, $sortDirection, $searchPosts);
        if($searchPosts) {
            $entitiesTopics = $this->getTextSearchService()->advancedFindEntities($keyword, $first, $count, $userId, $parts, $period, $sort, $sortDirection, false);
            if(!empty($entitiesTopics) || !empty($entities)) {
                $result = $this->processFoundTopics($entitiesTopics, $keyword) + $this->processFoundPosts($entities, $keyword);
            }
        }else{
            if(!empty($entities)) {
                $result = $this->processFoundTopics($entities, $keyword);
            }
        }

        // check access to parent group
        foreach($result as $k => $topicDto) {
            $forumGroup = $this->findGroupById($topicDto['groupId']);
            if (isset($forumGroup->entityId)) {
                $group = GROUPS_BOL_Service::getInstance()->findGroupById($forumGroup->entityId);
                if (isset($group)) {
                    $forumSection = $this->findSectionById($forumGroup->sectionId);
                    if ($forumSection->isHidden && $forumSection->entity == 'groups') {
                        if (!FRMSecurityProvider::checkPluginActive('groups', true)) {
                            ...
                            $canView = GROUPS_BOL_Service::getInstance()->isCurrentUserCanView(
                                GROUPS_BOL_Service::getInstance()->findGroupById($forumGroup->entityId)
                            );
                        }
                        if (!$canView) {
                            unset($result[$k]);
    ...
    public function onCollectSearchItems(OW_Event $event){
        ...
        if( isset($selected_section) && $selected_section != OW_Language::getInstance()->text('frmadvancesearch','all_sections') && $selected_section!= OW::getLanguage()->text('frmadvancesearch', 'forum_label') )
            return;
        if ( !isset($params['q']) )
        {
            return;
        ...
        $count = 0;
        $userIdList = array_column($topics, 'userId');
...
//            $itemInformation['sectionName'] = $item['sectionName'];
            ...
            $itemInformation['image'] = FORUM_BOL_ForumService::getInstance()->generateDefaultImageUrl();
            $itemInformation['link'] = $item['topicUrl'];
            $itemInformation['label'] = OW::getLanguage()->text('frmadvancesearch', 'forum_label');
        ...
        $data = $event->getData();
        $data['forum_topics']= array('label' => OW::getLanguage()->text('frmadvancesearch', 'forum_label'), 'data' => $result);
        $event->setData($data);
    }
    ...
    public function concludeTopic($topicId, $postId) {
        $topicConcluded = true;
        $topicDto = $this->findTopicById($topicId);
        if ($topicDto->conclusionPostId == null || $topicDto->conclusionPostId != $postId)
            $topicDto->conclusionPostId = $postId;
        else{
    ...
    public function canCurrentUserConcludeTopic($topicId) {
        $topicDto = $this->findTopicById($topicId);
        $currentUser = OW::getUser();
        $isGroupManager = false;
        $isAuthorizedForGroup = false;

        $forumGroup = $this->findGroupById($topicDto->groupId);
        $forumSection = $this->findSectionById($forumGroup->sectionId);

        if ($forumSection->isHidden) {
            $isAuthorizedForGroup =  OW::getUser()->isAuthorized('group');
            $eventIisGroupsPlusManager = new OW_Event('frmgroupsplus.check.user.manager.status', array('groupId' => $forumGroup->entityId));
            OW::getEventManager()->trigger($eventIisGroupsPlusManager);
            if (isset($eventIisGroupsPlusManager->getData()['isUserManager'])) {
                $isGroupManager = $eventIisGroupsPlusManager->getData()['isUserManager'];
            }
        }

        $isTopicCreator = $topicDto->userId == $currentUser->getId();
        $isAdmin = $currentUser->isAdmin();
        $isAuthorizedForForum = OW::getUser()->isAuthorized('forum');

        return $isTopicCreator || $isAdmin || $isGroupManager || $isAuthorizedForForum || $isAuthorizedForGroup;
    }

    private function generateDefaultImageUrl()
    {
        return OW::getPluginManager()->getPlugin('forum')->getStaticUrl() . 'img/forum_default_image.svg';
    }
}