 ...
 * @since 1.0
 */
class FRMCFP_MCTRL_Base extends OW_MobileActionController
{
    /**
...
//        OW::getDocument()->setDescription(UTIL_String::truncate(strip_tags($event->getDescription()), 200, '...'));
        ...
        if(isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']!=null){
            ...
            $this->assign('backUrl', (OW::getRouter()->urlForRoute('frmcfp.main_menu_route')));
        }
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMCFP_BOL_Service::ON_BEFORE_EVENT_VIEW_RENDER));

        $this->assign("urlForBack",OW::getRouter()->urlForRoute("frmcfp.main_menu_route"));
    }
//-----------------------------------------------------------------------------------------------------
    ...
    public function add()
    {
        ...
        $form = $this->eventService->getAddForm('event_add', true);
    ...
    public function edit( $params )
    {
        ...
        $this->assign('backUrl', OW::getRouter()->urlForRoute('frmcfp.view', array('eventId' => $event->getId())));

        $language = OW::getLanguage();
        $form = $this->eventService->getAddForm('event_edit', true);
        ...
        OW::getNavigation()->activateMenuItem(OW_Navigation::MAIN, 'frmcfp', 'main_menu_item');
        $this->addForm($form);
        OW::getEventManager()->trigger(new OW_Event(FRMCFP_BOL_Service::ON_BEFORE_EVENT_VIEW_RENDER, array('eventId' => $event->getId(),
            'pageType' => "edit")));
    }
    ...
    {
        OW::getDocument()->setHeading(OW::getLanguage()->text('frmcfp', 'event_index'));
...
//                OW::getDocument()->setDescription($language->text('frmcfp', 'past_events_page_desc'));
        ...
        $this->addComponent('paging', new BASE_CMP_PagingMobile($page, ceil($eventsCount / $configs[FRMCFP_BOL_Service::CONF_EVENTS_COUNT_ON_PAGE]), 5));
