<?php
class FRMVIDEOPLUS_CMP_Validserviceproviders extends OW_Validator
{
    ...
    public function isValid( $value )
    {
        ...
        if ( empty($value) )
        {
            if(isset($_POST['id'])){
                ...
                if(!OW::getStorage()->fileExists($videoDir) && (!isset($_POST['code']) || $_POST['code']==null)){
                    $this->setErrorMessage(OW::getLanguage()->text('frmvideoplus', 'error_both_fields_empty'));
                    return false;
                }
            }
        ...
        switch ( false )
        {
            case is_uploaded_file($tmpName):
                $this->setErrorMessage(OW::getLanguage()->text('frmvideoplus', 'errors_video_upload'));
                return false;
        }
        ...
        {
            $this->setErrorMessage(OW::getLanguage()->text('frmvideoplus', 'upload_file_extension_is_not_allowed'));
            return false;
        }
        ...
        {
            $this->setErrorMessage(OW::getLanguage()->text('frmvideoplus', 'upload_file_max_upload_filesize_error',array('videofilesize' => $maxUploadSize)));
            return false;
        }
        return true;
    }
