/**
 * Upgraded to 4.7.1
 * By @author Farzan Mohammadi <farzan.mohamadii@gmail.com>
 */
//STATIC
...
//}
function frmadvanceeditor_get_ow_button_label(key,defaultLabel){
...
CKEDITOR.plugins.add('ow_image',{
	init:function(editor){
		...
		label:frmadvanceeditor_get_ow_button_label('image', 'Insert Image'),
		command:'ow_image'
		});
	}
});
CKEDITOR.plugins.add('ow_video',{
	init:function(editor){
		...
		label:frmadvanceeditor_get_ow_button_label('video', 'Insert Video'),
		command:'ow_video'
		});
	}
});
CKEDITOR.plugins.add('ow_more',{
	init:function(editor){
		...
		label:frmadvanceeditor_get_ow_button_label('more', 'More'),
		command:'ow_more'
		});
	}
});
...
jHtmlAreaCKWrapper.fn=jHtmlAreaCKWrapper.prototype={
	...
	initCK: function(elem, options) {
		...
		frmadvanceeditor_form_refresh_before_submit(element.parents('form').get(0),editor);
	},
        ...
        image: function(){
            this.tempFB = new OW_FloatBox({
                $title: frmadvanceeditor_get_ow_button_label('image', 'Insert Image'),
                width: '600px',
                height: '100%',
...
function frmadvanceeditor_textarea_attach(elem){
	var element=$(elem);
...
function frmadvanceeditor_form_refresh_before_submit(form,editor){
	if (form)
...
if (window.OW_Components_DragAndDropAjaxHandler) {
	...
	OW_Components_DragAndDropAjaxHandler.prototype.loadSettings=function(id, successFunction) {
		this.original_loadSettings(id, function(settingMarkup){successFunction(settingMarkup);frmadvanceeditor_load_after_loadSettings(id); });
	};
	function frmadvanceeditor_load_after_loadSettings(id){
