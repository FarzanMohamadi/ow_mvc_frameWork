<?php
class FRMPROFILEMANAGEMENT_MCTRL_DeleteUser extends OW_MobileActionController
{
    ...
    public function index( $params )
    {
        ...
        if(OW::getConfig()->configExists('frmsecurityessentials','passwordRequiredProfile')){
            ...
            if($passwordRequiredProfile){
                $password = BOL_UserService::getInstance()->getOldPasswordInput('oldPasswordCheck', $form->getName());
        ...
        $this->assign('returnUrl', OW::getRouter()->urlForRoute('frmprofilemanagement.edit'));
        $this->addForm($form);
        ...
        if ( OW::getRequest()->isPost() && !(OW::getRequest()->isAjax()) && $form->isValid($_POST))
        {
            ...
            {
                $this->redirect( OW::getRouter()->urlForRoute('frmprofilemanagement.edit') );
            }
        }
    }
}
