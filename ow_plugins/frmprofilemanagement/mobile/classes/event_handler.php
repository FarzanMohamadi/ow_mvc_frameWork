<?php
class FRMPROFILEMANAGEMENT_MCLASS_EventHandler
{
    ...
    {
        $service = FRMPROFILEMANAGEMENT_BOL_Service::getInstance();
    ...
    public function onMobileAddItem(BASE_CLASS_EventCollector $event){
        ...
        $url = OW::getRouter()->urlForRoute('frmprofilemanagement.edit');
        $event->add(array('label' => $label, 'url' => $url));

        $label = OW::getLanguage()->text('base','preference_index');
        $url = OW::getRouter()->urlForRoute('frmprofilemanagement.preference_index');
        $event->add(array('label' => $label, 'url' => $url));

        $label = OW::getLanguage()->text('base','change_password');
        $url = OW::getRouter()->urlForRoute('frmprofilemanagement.edit.changepassword');
        $event->add(array('label' => $label, 'url' => $url));
    }
    public function onPreferenceMenuItem( BASE_CLASS_EventCollector $event )
    {
        ...
        $menuItem->setUrl($router->urlForRoute('frmprofilemanagement.preference_index'));
        $menuItem->setOrder(1);
        $event->add($menuItem);
    }
