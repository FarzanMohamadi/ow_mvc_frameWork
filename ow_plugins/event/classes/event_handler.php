...
class EVENT_CLASS_EventHandler
{
    ...
    public function onCollectSearchItems(OW_Event $event){
        ...
        if( isset($selected_section) && $selected_section != OW_Language::getInstance()->text('frmadvancesearch','all_sections') && $selected_section!= OW::getLanguage()->text('frmadvancesearch', 'event_label') )
            return;
        $searchValue = '';
        if ( !empty($params['q']) )
        {
        ...
        foreach($events as $item){
            ...
            $itemInformation['label'] = OW::getLanguage()->text('frmadvancesearch', 'event_label');
            if(isset($item->image)){
                $itemInformation['image'] = EVENT_BOL_EventService::getInstance()->generateImageUrl($item->image, true);
            } else {
                $itemInformation['emptyImage'] = true;
                $itemInformation['image'] = EVENT_BOL_EventService::getInstance()->generateDefaultImageUrl();
            }
            $itemInformation['imageInfo'] = BOL_AvatarService::getInstance()->getAvatarInfo((int) $item->id, $item->image, 'event');
        ...
        $data = $event->getData();
        $data['event']= array('label' => OW::getLanguage()->text('frmadvancesearch', 'event_label'), 'data' => $result);
        $event->setData($data);
    }
    ...
    public function getEditedDataNotification(OW_Event $e)
    {
        $params = $e->getParams();
        $notificationData = $e->getData();
        if ($params['pluginKey'] != 'event')
            return;
        ...
        elseif ($entityType == 'event-add-file') {
            if(FRMSecurityProvider::checkPluginActive('frmeventplus', true)) {
                $eventplusFile=FRMEVENTPLUS_BOL_EventFilesDao::getInstance()->findById($entityId);
