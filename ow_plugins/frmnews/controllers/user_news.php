 ...
 * @since 1.0
 */
class FRMNEWS_CTRL_UserNews extends OW_ActionController
{

    public function index( $params )
    {

        $this->setDocumentKey('user_news_list_page');
        ...
        */
        ...
        if ( $displaySocialSharing && !BOL_AuthorizationService::getInstance()->isActionAuthorizedForUser(0, 'frmnews', 'view')  )
        {
            $displaySocialSharing  = false;
        }
        ...
        $this->setPageHeading(OW::getLanguage()->text('frmnews', 'news'));
        ...
        if ( !empty($_GET['month']) )
        {
            ...
            $yearTitle = $year;
            $lbChangeEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_AFTER_DEFAULT_DATE_VALUE_SET, array('changeNewsJalaliToGregorian' => true, 'faYear' =>  (int)$year, 'faMonth'=> (int)$month ,'faDay'=> 1)));
            if($lbChangeEvent->getData() && isset($lbChangeEvent->getData()['gregorianYearNews'])){
                $year = $lbChangeEvent->getData()['gregorianYearNews'];
                $monthTitle =  OW::getLanguage()->text('frmjalali', 'date_time_month_short_fa_'.$month);
            }
            ...
            $ubChangeEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_AFTER_DEFAULT_DATE_VALUE_SET, array('changeNewsJalaliToGregorian' => true, 'faYear' =>  (int)$year, 'faMonth'=> (int)$month+1 ,'faDay'=> 1)));
            ...
            OW::getDocument()->setTitle(OW::getLanguage()->text('frmnews', 'user_news_archive_title', array('month_name'=>$monthTitle, 'display_name'=>$displayName)));
            OW::getDocument()->setDescription(OW::getLanguage()->text('frmnews', 'user_news_archive_description', array('year'=>$yearTitle, 'month_name'=>$monthTitle, 'display_name'=>$displayName) ));
        }
        else
        {
            ...
            OW::getDocument()->setTitle(OW::getLanguage()->text('frmnews', 'user_news_title', array('display_name'=>$displayName)));
            OW::getDocument()->setDescription(OW::getLanguage()->text('frmnews', 'user_news_description', array('display_name'=>$displayName) ));
        }
            ...
            foreach ( $list as $dto ) /* @var dto Entry */
            {
                ...
                //{
                    $tb[$dto->getId()][] = array(
                        ...
                        'label' => OW::getLanguage()->text('frmnews', 'toolbar_comments') . '<span class="ow_outline">' . $commentInfo[$dto->getId()] . '</span> '
                    );
                //}
                ...
                if ( $tagsInfo[$dto->getId()] )
                {
                    ...
                    $t = OW::getLanguage()->text('frmnews', 'tags');
                    for ( $i = 0; $i < (count($tags) > 3 ? 3 : count($tags)); $i++ )
                    {
                        $t .= " <a href=\"" . OW::getRouter()->urlForRoute('frmnews.list', array('list'=>'browse-by-tag')) . "?tag={$tags[$i]}\">{$tags[$i]}</a>" . ( $i != 2 ? ',' : '' );
                    }
        ...
        $this->assign('my_drafts_url', OW::getRouter()->urlForRoute('frmnews-manage-drafts'));
