 ...
 * @since 1.0
 */
class FRMNEWS_CTRL_Save extends OW_ActionController
{
    public function create()
    {
        ...
        if ( OW::getRequest()->isPost() && (!empty($_POST['command']) && in_array($_POST['command'], array('draft', 'publish')) ) && $form->isValid($_POST) )
        {
            ...
            if($entryDto->isDraft())
            {
                ...
                OW::getApplication()->redirect(OW::getRouter()->urlForRoute('frmnews-manage-drafts'));
            }else{
    ...
    public function edit($params = array())
    {
        ...
        if ( OW::getRequest()->isPost() && (!empty($_POST['command']) && in_array($_POST['command'], array('draft', 'publish')) ) && $form->isValid($_POST) )
        {
            ...
            $publishTime = isset($values['publish_date'])?strtotime($values['publish_date']):time();
    ...
    public function delete( $params )
    {
        ...
        if(isset($author)){
            $this->redirect(OW::getRouter()->urlForRoute('user-frmnews', array('user' => $author->getUsername())));
        }else{
...
abstract class newsEntryForm extends Form
{
    ...
    public function __construct($formName)
    {
        ...
        $entryTextArea = new WysiwygTextarea('entry','frmnews', $buttons);
