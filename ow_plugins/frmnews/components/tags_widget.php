 ...
 * @since 1.0
 */
class FRMNEWS_CMP_TagsWidget extends BASE_CLASS_Widget
{
        ...
        if($displayWidgetAllowed)
        {
            ...
            if (!$allInOne) {
                foreach ($enabledTagList as $tag) {
                    ...
                    $eventForEnglishFieldSupport = new OW_Event('frmmultilingualsupport.show.data.in.multilingual', array('list' => $latestNews, 'entityType' => 'news', 'display' => 'list', 'forWidget' => true));
                    ...
                    $item = array(
                        'label' => $tag->label,
                        'link' => OW::getRouter()->urlForRoute('frmnews.list', array('list' => 'browse-by-tag')) . "?tag=" . $tag->label,
                        'list' => $entryItems
                    );
                    $list[] = $item;
                }
                ...
                foreach ($enabledTagList as $tag) {
                    ...
                    foreach ($latestNews as $entry) {
                        ...
                        $stringRenderer = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_RENDER_STRING, array('string' => $description)));
                    ...
                    $tagNewsItems[$tag->id]['link']=OW::getRouter()->urlForRoute('frmnews.list', array('list' => 'browse-by-tag')) . "?tag=" . $tag->label;
                }
    ...
    public static function getSettingList()
    {
        ...
        $settingList['all_in_one'] = array(
            'presentation' => self::PRESENTATION_CHECKBOX,
            'label' => OW::getLanguage()->text('frmnews', 'cmp_tags_widget_view_all_in_one'),
            'value' => false
        );

        $settingList['widget_language'] = array(
            'presentation' => self::PRESENTATION_SELECT,
            'label' => OW::getLanguage()->text('frmnews', 'select_language_tag_widget'),
    ...
    {
        $list = array(
            self::SETTING_TITLE => OW::getLanguage()->text('frmnews', 'tag_widget_heading'),
