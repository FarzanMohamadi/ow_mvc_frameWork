 ...
 * @since 1.0
 */
class FRMNEWS_CMP_UserNewsWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $params )
    {
        ...
        if ( $userId != OW::getUser()->getId() && !OW::getUser()->isAuthorized('frmnews', 'view') )
        {
        ...
        */
        ...
        foreach ( $userEntryList as $id => $item )
        {
            /* Check privacy permissions */
            /*
            if ( $item->authorId != OW::getUser()->getId() && !OW::getUser()->isAuthorized('frmnews') )
            {
                $eventParams = array(
                    'action' => EntryService::PRIVACY_ACTION_VIEW_BLOG_POSTS,
            ...
            foreach ( $list as $key => $item )
            {
                ...
                else
                {
                    $comments_tb_link = array(
                        'label' => OW::getLanguage()->text('frmnews', 'toolbar_comments') . '<span class="ow_txt_value">' . $commentInfo[$item->getId()] . '</span> ',
        ...
        $this->setSettingValue(
            self::SETTING_TOOLBAR, array(
                array('label' => OW::getLanguage()->text('frmnews', 'total_news', array('total' => $service->countUserEntry($userId) ))),
            array(
                'label' => OW::getLanguage()->text('frmnews', 'view_all'),
                'href' => OW::getRouter()->urlForRoute('user-frmnews', array('user' => $user->getUsername()))
            )
            )
        );
    }
    ...
    {
        return array(
            self::SETTING_TITLE => OW::getLanguage()->text('frmnews', 'news'),
