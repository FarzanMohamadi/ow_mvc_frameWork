...
class GROUPS_CLASS_ConsoleBridge
{
    ...
    public function onInviteDublicate( OW_Event $event )
    {
        ...
        $data['users'] = array_unique(array_merge($oldData['users'], $data['users']));
    ...
    public function onComment( OW_Event $e )
    {
        ...
        $groupUrl = OW::getRouter()->urlForRoute('groups-view', array('groupId' => $group->id));
        $groupTitle = UTIL_String::truncate($group->title, 100, '...');
        $groupImage = GROUPS_BOL_Service::getInstance()->getGroupImageUrl($group, GROUPS_BOL_Service::IMAGE_SIZE_SMALL, true);
    ...
    public function onGroupDelete( OW_Event $event )
    {
        ...
        OW::getEventManager()->call('notifications.remove', array(
            'entityType' => GROUPS_BOL_Service::GROUP_FEED_ENTITY_TYPE,
            'entityId' => $groupId
        ));

        $eventIisGroupsPlusFiles = new OW_Event('frmgroupsplus.delete.files', array('groupId'=>$groupId));
        OW::getEventManager()->trigger($eventIisGroupsPlusFiles);

        GROUPS_BOL_Service::getInstance()->deleteUser($groupId, [], true);

        GROUPS_BOL_InviteDao::getInstance()->deleteByGroupId($groupId);
        ...
        {
            $event = new OW_Event(GROUPS_BOL_Service::EVENT_DELETE_FORUM, array('entityId' => $groupId, 'entity' => 'groups'));
            OW::getEventManager()->trigger($event);
        }
    }
