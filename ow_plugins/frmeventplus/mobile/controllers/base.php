 ...
 * @since 1.0
 */
class FRMEVENTPLUS_MCTRL_Base extends OW_MobileActionController
{
    /**
    ...
    public function fileList( $params )
    {
        ...
        foreach ( $dtoList as $item ) {
            ...
            $itemInformation['imageSrc'] = FRMEVENTPLUS_BOL_Service::getInstance()->getProperIcon(strtolower($fileNameExt));
            ...
            $itemInformation['content'] = $language->text('frmeventplus', 'by') . ' :' . '<a href="'.OW::getRouter()->urlForRoute('base_user_profile', array('username' => BOL_UserService::getInstance()->getUserName($item->getUserId()))).'">' .BOL_UserService::getInstance()->getDisplayName($item->getUserId()) . '</a>';
            $itemInformation['id'] = $item->id;

            if ($item->userId == OW::getUser()->getId() || $canEdit) {
                $deleteUrl =  OW::getRequest()->buildUrlQueryString(OW::getRouter()->urlForRoute('frmeventplus.deleteFile',
                    array('attachmentId' => $item->id, 'eventId' => $eventId)),array('code' =>$code));
                $toolbar[] = array(
                    'label' => '<span>'.$language->text('frmeventplus', 'delete_item').'</span>',
                    'href' => $deleteUrl,
                    'class' => 'delete_file'
                );
            }
        ...
        $this->assign('addIconUrl', OW::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'images/File_Extentions/trash.svg');
        $this->assign("eventId", $eventId);
        $this->assign('backUrl',OW::getRouter()->urlForRoute('event.view' , array('eventId'=>$eventId)));
        OW::getDocument()->addStyleDeclaration('.owm_event_list.files_list span.ow_nowrap.delete_file a {background-image: url('.OW::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'images/File_Extentions/trash.svg'.');}');
        $params = array(
        ...
        OW::getEventManager()->trigger(new OW_Event('frm.on.before.event.view.render', array('eventId' => $eventId,
            'pageType' => "fileList")));
    }
