 ...
 * @since 1.0
 */
class FRMEVENTPLUS_BOL_Service
{
    ...
    const DELETE_FILES = 'frmeventplus.delete.files';
    const ADD_FILE_WIDGET = 'frmeventplus.add.file.widget';
    ...
    {
        $this->eventInformationDao = FRMEVENTPLUS_BOL_EventInformationDao::getInstance();
        $this->categoryDao = FRMEVENTPLUS_BOL_CategoryDao::getInstance();
        $this->eventFileDao = FRMEVENTPLUS_BOL_EventFilesDao::getInstance();
    }

    public function setTitleHeaderListItemEvent(OW_Event $event)
    {
        $params = $event->getParams();
        if (isset($params['listType']) && $params['listType'] == FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL) {
            OW::getDocument()->setTitle(OW::getLanguage()->text('frmeventplus', 'meta_title_event_add_general'));
            OW::getDocument()->setDescription(OW::getLanguage()->text('frmeventplus', 'meta_description_event_general'));
        } else if (isset($params['listType']) && $params['listType'] == FRMEVENTPLUS_BOL_Service::$EVENT_MY) {
            OW::getDocument()->setTitle(OW::getLanguage()->text('frmeventplus', 'meta_title_event_add_my'));
            OW::getDocument()->setDescription(OW::getLanguage()->text('frmeventplus', 'meta_description_event_my'));
        }
    }
    ...
    public function addListTypeToEvent(OW_Event $event)
    {
        ...
        if(isset($params['list'])){
            $list = $params['list'];
            $keys = array(FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL
            ,FRMEVENTPLUS_BOL_Service::$EVENT_MY, 'invited');
            ...
            if (isset($params['menuItems'])) {
                ...
                if (OW::getUser()->isAuthenticated()) {
                    ...
                    $item->setLabel(OW::getLanguage()->text('frmeventplus', FRMEVENTPLUS_BOL_Service::$EVENT_MY));
                    $item->setUrl(OW::getRouter()->urlForRoute('event.view_event_list', array('list' => FRMEVENTPLUS_BOL_Service::$EVENT_MY)));
                    $item->setKey(FRMEVENTPLUS_BOL_Service::$EVENT_MY);
                    $item->setOrder(1);
                    $item->setIconClass('ow_ic_my_events ow_dynamic_color_icon');
                    array_push($menuItems, $item);
                }
                $item = new BASE_MenuItem();
                $item->setLabel(OW::getLanguage()->text('frmeventplus', FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL));
                $item->setUrl(OW::getRouter()->urlForRoute('event.view_event_list', array('list' => FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL)));
                $item->setKey(FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL);
                $item->setOrder(2);
                $item->setIconClass('ow_ic_latest ow_dynamic_color_icon');
                array_push($menuItems, $item);
    ...
    public function getResultForListItemEvent(OW_Event $event)
    {
        if(class_exists("EVENT_BOL_EventService")) {
            ...
            $userId = '';
            $eventIds = array();
            $activeTab = FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL;
            $this->page = $params['page'];
            $this->setFilterParameters();
            if ($params['list'] != FRMEVENTPLUS_BOL_Service::$EVENT_MY) {
                ...
                $resultsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ADD_EVENT_FILTER_FORM, $eventAddFilterFormParams));
                if (isset($resultsEvent->getData()['eventFilterForm'])) {
                    $eventFilterForm = $resultsEvent->getData()['eventFilterForm'];
                }

            } else {
                $resultsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ADD_EVENT_FILTER_FORM, array('tab' => self::$EVENT_MY, 'participationStatus' => $this->participationStatus, 'dateStatus' => $this->dateStatus, 'categoryStatus' => $this->categoryStatus, 'searchTitle' => $this->searchTitle)));
                if (isset($resultsEvent->getData()['eventFilterForm'])) {
                    $eventFilterForm = $resultsEvent->getData()['eventFilterForm'];
                }
                $isPublic = false;
    ...
    public function setEventController($activeTab, $filterForm, $eventController)
    {
        ...
        $contentMenu->setItemActive($activeTab);
    ...
    public function getEventFilterForm($name, $tab, $selectedParticipationStatus = 1, $selectedDateStatus = 1, $selectedCategory=1,$searchedTitle=null)
    {
        ...
        $searchTitle->addAttribute('placeholder',OW::getLanguage()->text('frmeventplus', 'search_title'));
        ...
        $option = array();
        $option[FRMEVENTPLUS_BOL_Service::$DATE_ALL] = OW::getLanguage()->text('frmeventplus', 'date_all');
        $option[FRMEVENTPLUS_BOL_Service::$DATE_LATEST] = OW::getLanguage()->text('frmeventplus', 'date_latest');
        $option[FRMEVENTPLUS_BOL_Service::$DATE_PAST] = OW::getLanguage()->text('frmeventplus', 'date_past');
        ...
        if ($tab == self::$EVENT_MY) {
            ...
            $option = array();
            $option[FRMEVENTPLUS_BOL_Service::$PARTICIPATE_ALL] = OW::getLanguage()->text('frmeventplus', 'participate_all');
            $option[FRMEVENTPLUS_BOL_Service::$PARTICIPATE_SURE] = OW::getLanguage()->text('frmeventplus', 'participate_sure');
            $option[FRMEVENTPLUS_BOL_Service::$PARTICIPATE_MAYBE] = OW::getLanguage()->text('frmeventplus', 'participate_maybe');
            $option[FRMEVENTPLUS_BOL_Service::$PARTICIPATE_NO] = OW::getLanguage()->text('frmeventplus', 'participate_no');
        ...
        $resultsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ADD_CATEGORY_FILTER_ELEMENT, array('form' => $form, 'selectedCategory' => $selectedCategory)));
    ...
    */
    public function addCategoryFilterElement(OW_Event $event)
    {
        ...
        if(isset($params['form'])) {
            ...
            $option = array();
            $option[null] = OW::getLanguage()->text('frmeventplus','select_category');
            ...
            if(isset($params['selectedCategory'])) {
                ...
                $resultsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::GET_EVENT_SELECTED_CATEGORY_ID, array('eventId' => $params['eventId'])));
                ...
                foreach ($events as $ev) {
                    if ($ev->getId() == $eventId) {
                        $button = array(
                            'leave' =>
                                array(
                                    'url' => OW::getRouter()->urlForRoute('frmeventplus.leave', array('eventId' => $eventId)),
                                    'label' => OW::getLanguage()->text('frmeventplus', 'leave_button_label'),
                                    'confirmMessage' => OW::getLanguage()->text('frmeventplus', 'leave_confirm_message')
                                )
                        );
    ...
    */
    public function getEventSelectedCategoryLabel(OW_Event $event)
    {
        ...
        if(isset($params['eventId'])){
            $categoryId = $this->getEventCategoryByEventId($params['eventId']);
            if($categoryId!=null) {
                /* @var $category FRMEVENTPLUS_BOL_Category */
                $category = $this->categoryDao->findById($categoryId);
                $event->setData(array('categoryLabel' => $category->label, 'categoryId' => $categoryId));
            }
        }
    }
    ...
    {
        $category = new FRMEVENTPLUS_BOL_Category();
        $category->label = $label;
        FRMEVENTPLUS_BOL_CategoryDao::getInstance()->save($category);
    }
    ...
    public function getItemForm($id)
    {
        ...
        $formName = 'edit-item';
        $submitLabel = 'edit';
        $actionRoute = OW::getRouter()->urlFor('FRMEVENTPLUS_CTRL_Admin', 'editItem');
        ...
        $submit->setValue(OW::getLanguage()->text('frmeventplus', 'edit_item'));
        $form->addElement($submit);
    ...
    public function checkIfEventPlusIsActive(OW_Event $event)
    {
        ...
        if(isset($params['checkActive']))
        {
            if(FRMSecurityProvider::checkPluginActive('frmeventplus', true)) {
                if( OW::getUser()->isAuthenticated()) {
                    $event->setData(array('list' => FRMEVENTPLUS_BOL_Service::$EVENT_MY));
                }else{
                    $event->setData(array('list' => FRMEVENTPLUS_BOL_Service::$EVENT_GENERAL));
                }
            }
        }
    }
    ...
    public function getUploadFileForm($eventId)
    {
        ...
        OW::getDocument()->setHeading($language->text('frmeventplus', 'file_create_heading'));
        OW::getDocument()->setHeadingIconClass('ow_ic_new');
        OW::getDocument()->setTitle($language->text('frmeventplus', 'file_create_page_title'));
        OW::getDocument()->setDescription($language->text('frmeventplus', 'file_create_page_description'));

        $form = new FRMEVENTPLUS_FileUploadForm($eventId);
        $actionRoute = OW::getRouter()->urlFor('FRMEVENTPLUS_CTRL_Base', 'addFile', array('eventId' => $eventId));
        $form->setAction($actionRoute);
        return $form;
    }
    ...
    public function addFileWidget(OW_Event $event)
    {
        ...
        if(isset($params['controller']) && isset($params['eventId'])){
            ...
            $eventController->addComponent('eventFileList', new FRMEVENTPLUS_MCMP_FileListWidget($bcw));
            $fileBoxInformation = array(
                'show_title' => true,
                'title' => OW_Language::getInstance()->text('frmeventplus', 'widget_files_title'),
                'wrap_in_box' => true,
                'icon' => 'ow_ic_info',
                'type' => "",
            );
    ...
    public function onCollectNotificationActions( BASE_CLASS_EventCollector $e )
    {
        $e->add(array(
            ...
            'description' => OW::getLanguage()->text('frmeventplus', 'email_notifications_setting_file'),
            'selected' => true,
            'sectionLabel' => OW::getLanguage()->text('frmeventplus', 'email_notification_section_label'),
    ...
    public function onInviteUser(OW_Event $event){
        ...
        $data = array(
            ...
            'string' => array(
                'key' => 'frmeventplus+event_user_invitation_notification',
                'vars' => array(
    ...
    public function manageAddFile($eventId, $item){
        ...
        $data = array(
            'time' => time(),
            'string' => array(
                "key" => 'frmeventplus+feed_add_file_string',
                "vars" => array(
        ...
        */
        ...
        $notificationData = array(
            'string' => array(
                "key" => 'frmeventplus+notif_add_file_string',
                "vars" => array(
                    'eventTitle' => $eventDto->title,
                    'eventUrl' => $url,
    ...
    public function getFileUrlByFileId($fileId, $params = array()){
        $item = null;
        if (isset($params['cache']['event_files'][$fileId])) {
            $attachmentId = $params['cache']['event_files'][$fileId]->attachmentId;
    ...
    public function feedOnItemRender( OW_Event $e)
    {
        $params = $e->getParams();
        $data = $e->getData();
        if (!isset($data["string"]["key"]) || $data["string"]["key"] != "frmeventplus+feed_add_file_string")
            return;
        else {
            ...
            if(isset($event)){
                ...
                $data["string"]["vars"]["fileUrl"] = $this->getFileUrlByFileId($params["action"]["entityId"], $params);
                $e->setData($data);
            }
        }
    }
...
class FRMEVENTPLUS_FileUploadForm extends Form
{
    public function __construct($groupId)
    {
        ...
        $nameField->setLabel($language->text('frmeventplus', 'create_field_file_name_label'));
        $this->addElement($nameField);

        $fileField = new FileField('fileUpload');
        $fileField->setLabel($language->text('frmeventplus', 'create_field_file_upload_label'));
        ...
        $saveField->setValue(OW::getLanguage()->text('frmeventplus', 'create_submit_btn_label'));
