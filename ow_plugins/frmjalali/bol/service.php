...
class FRMJALALI_BOL_Service
{
    ...
    public function onRenderFormatDateField(OW_Event $event){
        ...
        if((isset($params['isPresentationDate']) && $params['isPresentationDate']==true) &&  OW::getConfig()->getValue('frmjalali', 'dateLocale')==1)
        {
            $timeStamp = $params['timeStamp'];
            $idt = new FRMJALALI_CLASS_IntlDateTime();
            ...
            if ($format === 'dmy')
                ...
                $event->setData(array('jalaliSimpleFormat' => $result =  $fa_date[0] .'/'.$fa_date[2] .'/'.$fa_date[1]));
        }
        else if((isset($params['isBirthday']) && $params['isBirthday']==true) &&  OW::getConfig()->getValue('frmjalali', 'dateLocale')==1)
        {
            $idt = new FRMJALALI_CLASS_IntlDateTime();
            ...
            if ($format === 'dmy')
                ...
                $event->setData(array('jalaliSimpleFormat' =>$result = $faDate_array['mm'] . ' '. $fa_date[2] .' '. $fa_date[0]));

        }
        else if(isset($params['timeStamp'])  &&  OW::getConfig()->getValue('frmjalali', 'dateLocale')==1) {
            $timeStamp = $params['timeStamp'];
            ...
            if ( $onlyDate ){
                if ($format === 'dmy')
                    ...
                    $event->setData(array('jalaliSimpleFormat' =>
                        $faDate_array['mm'] . ' '.$fa_date[2] .' '. $fa_date[0]));
            }else{
                if ($format === 'dmy')
                    $event->setData(array('jalaliSimpleFormat' =>
                    $militaryTime ?   $fa_date[2] .' '.$faDate_array['mm'] . ' '.$fa_date[0] .' '.$language->text('frmjalali', 'date_time_month_short_fa_hour').' '.strftime("%H:%M", $timeStamp):$fa_date[2] .' '.$faDate_array['mm'] . ' '.$fa_date[0] . ' '.$language->text('frmjalali', 'date_time_month_short_fa_hour'). ' '.strftime(" %I:%M%p", $timeStamp)));
                else
                    $event->setData(array('jalaliSimpleFormat' =>
                        $militaryTime ?   $faDate_array['mm'] . ' '.$fa_date[2] .' '.$fa_date[0] .' '.$language->text('frmjalali', 'date_time_month_short_fa_hour').' '.strftime("%H:%M", $timeStamp):$faDate_array['mm'] . ' '.$fa_date[2] .' '.$fa_date[0] . ' '.$language->text('frmjalali', 'date_time_month_short_fa_hour'). ' '.strftime(" %I:%M%p", $timeStamp)));
    ...
    public function onBeforeValidatingField(OW_Event $event){
        ...
        $idt = new FRMJALALI_CLASS_IntlDateTime();
        if(isset($params['field']) && $params['field'] instanceof DateField && OW::getConfig()->getValue('frmjalali', 'dateLocale')==1) {
        ...
        }else if(isset($params['data']) && isset($params['data']['year_range']) && isset($params['data']['year_range']['from']) && isset($params['field']) && $params['field'] instanceof YearRange && OW::getConfig()->getValue('frmjalali', 'dateLocale')==1) {
    ...
    public function onAfterDefaultDateValueSet(OW_Event $event){
        ...
        $birthdayEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::SET_BIRTHDAY_RANGE_TO_JALALI, $params));
        ...
        $dateRangeEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::CHANGE_DATE_RANGE_TO_JALALI, $params));
        ...
        $dateBlogAndNewsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::CHANGE_DATE_FORMAT_TO_JALALI_FOR_BLOG_AND_NEWS, $params));
        ...
        $dateBlogAndNewsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::CHANGE_DATE_FORMAT_TO_GREGORIAN, $params));
        if($dateBlogAndNewsEvent->getData()!=null && sizeof($dateBlogAndNewsEvent->getData())>0){
            $event->setData($dateBlogAndNewsEvent->getData());
        }
        //calculate jalali max day of a month
        $maxJalaliLastDayEvent= OW::getEventManager()->trigger(new OW_Event(FRMEventManager::CALCULATE_JALALI_MONTH_LAST_DAY, $params));
    ...
    public function calculateJalaliMonthLastDay(OW_Event $event){
        $params = $event->getParams();
        $idt = new FRMJALALI_CLASS_IntlDateTime();
        if(isset($params['jalaliMaxMonthDay']) && $params['jalaliMaxMonthDay']==true  && OW::getConfig()->getValue('frmjalali', 'dateLocale')==1){
    ...
    public function setBirthdayRangeToJalali(OW_Event $event){
        $params = $event->getParams();
        $idt = new FRMJALALI_CLASS_IntlDateTime();
        if(isset($params['DefaultBirthRange']) && $params['DefaultBirthRange']==true  && OW::getConfig()->getValue('frmjalali', 'dateLocale')==1){
            $defaultYearFrom ='';
    ...
    public function onBeforeDocumentRender()
    {
        if(OW::getConfig()->getValue('frmjalali', 'dateLocale')==1)
        {
            if (!isset($_COOKIE['frmjalali']) )
            {
                setcookie('frmjalali',1);
            }
        }
        $jsDir = OW::getPluginManager()->getPlugin('frmjalali')->getStaticJsUrl();
        OW::getDocument()->addScript($jsDir.'frmjalali.js');
    }
}