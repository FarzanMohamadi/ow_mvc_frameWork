...
class FRMJALALI_CLASS_IntlDateTime extends \DateTime {

/*	F	*/
function OW_Jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='en'){
...
public function jgetdate($timestamp='',$none='',$tz='Asia/Tehran',$tn='en'){
 ...
 $jdate=explode('_',$this->OW_Jdate('F_G_i_j_l_n_s_w_Y_z',$ts,'',$tz,$tn));
 return array(
...
public function jdate_words($array,$mod=''){
 ...
 foreach($array as $type=>$num){
  ...
  switch($type){
	...
	case'mm':
	$key=array
	(OW::getLanguage()->text('frmjalali', 'date_time_month_short_fa_1'),OW::getLanguage()->text('frmjalali', 'date_time_month_short_fa_2')
...
public function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
	$j_y=$this->tr_num($j_y); $j_m=$this->tr_num($j_m); $j_d=$this->tr_num($j_d);
 $d_4=($j_y+1)%4;
...
public function  maxDay($yearVal,$monthVal)
{
	if ($monthVal == 7 || $monthVal == 8 || $monthVal == 9 || $monthVal == 10 || $monthVal == 11) {
		return 30;
	}
	else if ($monthVal == 12) {
