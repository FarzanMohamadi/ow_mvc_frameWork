...
class FRMAPARATSUPPORT_BOL_Service
{
    ...
    public function addVideoUploader(){
        ...
        $videoUpload->setLabel(OW::getLanguage()->text('frmaparatsupport', 'add_aparat_url_element_label'));
        $videoUpload->setDescription(OW::getLanguage()->text('frmaparatsupport', 'add_aparat_url_element_desc'));
        $videoUpload->addValidator(new FRMAPARATSUPPORT_CMP_Validserviceproviders());
        return $videoUpload;
    }
    ...
    public function onBeforeVideoUploadFormRenderer(OW_Event $event)
    {
        ...
        if(isset($params['form'])){
            ...
            $ajaxUrl = OW::getRouter()->urlForRoute('frmaparatsupport.load', ['vid'=>'']);
            $js = "
var lastVid = '';
...
function check_aparat(first_try){
    ...
    var aparatUrlChecker = new RegExp('^(http://|https://|)(www.|)aparat.com/v/[a-zA-Z0-9\.\_\%\+\-\?]{4,}$');
    if (aparatUrlChecker.test(url)) {
        var arr = url.split('/');
        var editedVid = arr[arr.length - 1].split('?sid=');
        vid=editedVid[0];
        if (vid !== lastVid){
            clear_thumb();
            lastVid = vid;
