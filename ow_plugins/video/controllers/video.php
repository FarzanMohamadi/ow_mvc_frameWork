...
class VIDEO_CTRL_Video extends OW_ActionController
{
    ...
    {
        $validLists = array( 'latest', 'featured', 'toprated', 'tagged');
        $classes = array('ow_ic_latest ow_dynamic_color_icon', 'ow_ic_clock ow_dynamic_color_icon', 'ow_ic_most_popular ow_dynamic_color_icon', 'ow_ic_tag ow_dynamic_color_icon');

        if ( !VIDEO_BOL_ClipService::getInstance()->findClipsCount('featured') )
        {
            $validLists = array( 'latest', 'toprated', 'tagged');
            $classes = array('ow_ic_latest ow_dynamic_color_icon', 'ow_ic_most_popular ow_dynamic_color_icon', 'ow_ic_tag ow_dynamic_color_icon');
        }
        ...
        $validListsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ADD_LIST_TYPE_TO_VIDEO,array('menuItems' => $menuItems)));
    ...
    public function view( array $params )
    {
        ...
        OW::getDocument()->addStyleSheet(OW::getPluginManager()->getPlugin('video')->getStaticCssUrl() . 'video.css');
        
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_OBJECT_RENDERER, array('privacy' => $clip->privacy, 'ownerId' => $clip->userId)));
...
//        OW::getDocument()->setDescription($language->text('video', 'meta_description_video_view', array('title' => $clip->title, 'tags' => $tagStr)));
        ...
        //set JSON-LD
        $this->clipService->addJSONLD($clip);

        $this->setDocumentKey("video_view");
    }
    ...
    public function edit( array $params )
    {
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_VIDEO_UPLOAD_FORM_RENDERER,array('form' => $videoEditForm,'component'=>$this,'code'=>$clip->code, 'clipId' => $clip->id)));
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_VIDEO_UPLOAD_COMPONENT_RENDERER,array('form' => $videoEditForm, 'component' => $this)));
...
<iframe src="https://www.aparat.com/video/video/embed/videohash/'.$aparat_video_ID.'/vt/frame" allowFullScreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" ></iframe></div>';
        ...
        $this->setDocumentKey("video_edit");
    }
    ...
    public function viewList( array $params )
    {
        ...
        $validListsEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ADD_LIST_TYPE_TO_VIDEO,array('validLists' => $validLists)));
        ...
        $this->menu->setItemActive($listType);
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::SET_TILE_HEADER_LIST_ITEM_VIDEO, array('listType' => $listType)));
        $params = array(
            "sectionKey" => "video",
            "entityKey" => "viewList",
        ...
        $this->setDocumentKey("user_videos");
    }
...
//        OW::getDocument()->setDescription($lang->text('video', 'meta_description_user_video', array('displayName' => $displayName)));
    ...
    public function viewTaggedList( array $params = null )
    {
        ...
        $this->setDocumentKey("video_search_result_tag");
        ...
        $tag = !empty($params['tag']) ? trim((urldecode($params['tag']))) : '';

        $this->addComponent('videoMenu', $this->menu);

        $this->menu->setItemActive('tagged');
...
//            OW::getDocument()->setDescription(OW::getLanguage()->text('video', 'meta_description_video_tagged_as', array('tag' => $tag)));
    ...
    public function ajaxResponder()
    {
        ...
        {
            if(FRMSecurityProvider::checkPluginActive('frmsecurityessentials', true) && $_POST['ajaxFunc']=='ajaxDeleteClip') {
                $code =$_POST['code'];
                if(!isset($code)){
                    throw new Redirect404Exception();
                }
    ...
    public function validateVideoIframe()
    {
        if(OW::getRequest()->isAjax() && isset($_POST['url']))
        {
...
class videoEditForm extends Form
{
    ...
    public function process()
    {
        ...
        if ( $values['id'] )
        {
            ...
            if ( $clip )
            {
                $clip->title =  UTIL_HtmlTag::stripTagsAndJs($values['title']);
                ...
                if(isset($values['videoUpload']) || isset($values["videoUploadThumbnail"]) ) {
