...
class VIDEO_MCTRL_Video extends OW_MobileActionController
{
    ...
    private function getMenu()
    {
        ...
        $classes = array('ow_ic_latest ow_dynamic_color_icon', 'ow_ic_clock ow_dynamic_color_icon', 'ow_ic_tag ow_dynamic_color_icon');

        if ( !VIDEO_BOL_ClipService::getInstance()->findClipsCount('featured') )
        {
            $validLists = array( 'latest', 'tagged');
            $classes = array('ow_ic_latest ow_dynamic_color_icon', 'ow_ic_tag ow_dynamic_color_icon');
        }
...
//        OW::getDocument()->setDescription($language->text('video', 'meta_description_video_view', array('title' => $clip->title, 'tags' => $tagStr)));
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_VIDEO_RENDER, array('this' => $this, 'objectId' => $clip->id,'userId' => $clip->userId, 'privacy' => $clip->privacy)));
    }
...
<iframe src="https://www.aparat.com/video/video/embed/videohash/'.$aparat_video_ID.'/vt/frame" allowFullScreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" ></iframe></div>';
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_VIDEO_RENDER, array('videoId' => $clip->id, 'pageType' => "edit")));
    }
}
