<?php
class FRMUSERLOGIN_MCTRL_Iisuserlogin extends OW_MobileActionController
{
        ...
        if($details != null) {
            foreach ($details as $detail) {
                if(session_id() == $detail->sessionId){
                    ...
                    $actions = '<a class="owm_btn_class_2 ow_right" href="javascript://" onclick="terminateDevice('.$detail->id.')">'. OW::getLanguage()->text('frmuserlogin','terminate_device').'</a>';
                }
                $items[] = array(
                    'time' => UTIL_DateTime::formatSimpleDate($detail->time),
                    'browser' => $detail->browser,
