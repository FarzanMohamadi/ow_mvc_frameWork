 ...
 * @since 1.0
 */
class FRMUSERLOGIN_BOL_Service
{
    ...
    {
        $this->loginDetailsDao = FRMUSERLOGIN_BOL_LoginDetailsDao::getInstance();
        $this->activeDetailsDao = FRMUSERLOGIN_BOL_ActiveDetailsDao::getInstance();
    }
    ...
    public function deleteActiveLoginDetails()
    {
        ...
        $loginCookiesList = BOL_LoginCookieDao::getInstance()->findAllCookies();
        $activeCookieList = $this->activeDetailsDao->findAllNonDeletedCookies();
        $NonExistInLoginCookiesList = array_diff($activeCookieList, $loginCookiesList);
        $this->activeDetailsDao->deletedByCookies($NonExistInLoginCookiesList);
    }
    ...
    public function getCurrentBrowserInformation(){
        $event = OW::getEventManager()->trigger(new OW_Event('frmmobilesupport.browser.information'));
    ...
    public function getUserLoginDetails($userId, $checkAuth = true){
        ...
        $numberOfLoginDetails = (int)OW::getConfig()->getValue('frmuserlogin', FRMUSERLOGIN_BOL_Service::NUMBER_OF_LOGIN_DETAILS);
    ...
    public function sendEmailToUsers($userId)
    {
        ...
        $mail->setSubject(OW::getLanguage()->text('frmuserlogin', 'email_login_to_network'));
        $mail->setHtmlContent(OW::getLanguage()->text('frmuserlogin', 'email_login_details', array('username' => $user->username, 'email' => $user->email, 'ip' => $detail->ip, 'time' => UTIL_DateTime::formatSimpleDate($detail->time), 'browser' => $detail->browser)));
        $mail->setTextContent(OW::getLanguage()->text('frmuserlogin', 'email_login_details', array('username' => $user->username, 'email' => $user->email, 'ip' => $detail->ip, 'time' => UTIL_DateTime::formatSimpleDate($detail->time), 'browser' => $detail->browser)));
        $mails[] = $mail;
    ...
    public function onPreferenceAddFormElement( BASE_CLASS_EventCollector $event )
    {
        ...
        $fromElement = new CheckboxField('frmuserlogin_login_detail_subscribe');
        $fromElement->setLabel($language->text('frmuserlogin', 'preference_login_detail_subscribe_label'));
        $fromElement->setDescription($language->text('frmuserlogin', 'preference_login_detail_subscribe_description'));

        if ( isset($values['frmuserlogin_login_detail_subscribe']) )
        {
            $fromElement->setValue($values['frmuserlogin_login_detail_subscribe']);
        }
    ...
    public function onUserLogout(OW_Event $event){
        $deleteEvent = OW::getEventManager()->trigger(new OW_Event('frmuserlogin.before_delete_session'));
     ...
     * @return bool
     */
    public function terminateAllDevices($userId)
    {
        if ($userId == OW::getUser()->getId()) {
            return false;
        }

        $result = $this->activeDetailsDao->deleteAllDevices($userId);
        if($result){
            return true;
        }
        return false;
    ...
    public function getMenu($active=0){
        ...
        if(OW::getConfig()->configExists('frmuserlogin','update_active_details') && OW::getConfig()->getValue('frmuserlogin','update_active_details')) {
            ...
            $item->setLabel($language->text('frmuserlogin', 'active_details_header'));
            $item->setUrl(OW::getRouter()->urlForRoute('frmuserlogin.active'));
            $item->setKey('active');
            $item->setOrder(1);
            $item->setActive($active==1);
            $item->setIconClass('ow_ic_user ow_dynamic_color_icon');
            array_push($menuItems, $item);
        }

        $item = new BASE_MenuItem();
        $item->setLabel($language->text('frmuserlogin', 'login_details_header'));
        $item->setUrl(OW::getRouter()->urlForRoute('frmuserlogin.login'));
        $item->setKey('login');
    ...
    {
        $jsDir = OW::getPluginManager()->getPlugin("frmuserlogin")->getStaticJsUrl();
        OW::getDocument()->addScript($jsDir . "frmuserlogin.js");
        OW::getLanguage()->addKeyForJs('frmuserlogin','loggedOut');
    }
    ...
    public function getSections($currentSection = null)
    {
        ...
        for ($i = 1; $i <= 2; $i++) {
            $sections[] = array(
                ...
                'url' => OW::getRouter()->urlForRoute('frmuserlogin.admin.currentSection', array('currentSection' => $i)),
    ...
    {
        if ($sectionId == 1) {
            return OW::getLanguage()->text('frmuserlogin', 'general_setting');
        } else if ($sectionId == 2) {
            return OW::getLanguage()->text('frmuserlogin', 'cookie_setting');
        }
    }

    public function deleteAllUsersActiveCookies()
    {
        $this->activeDetailsDao->deleteAllUsersActiveCookies();
    }
