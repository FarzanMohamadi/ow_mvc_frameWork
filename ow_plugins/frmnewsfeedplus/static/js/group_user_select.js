    ...
    submit: function(){
        ...
        if( underSelf.resultList.length == 0 )
        {
            if(underSelf.forwardDesType=='groups') {
                OW.warning(OW.getLanguageText('frmnewsfeedplus', 'group_select_empty_list_message'));
            }else{
        ...
        $.ajax({
            ...
            complete: function (data) {
                ...
                {
                    if(underSelf.forwardDesType=='groups') {
                        OW.info(OW.getLanguageText('frmnewsfeedplus', 'groups_invite_success_message', {count: selectedCount}));
                    }else{
                        OW.info(OW.getLanguageText('frmnewsfeedplus', 'users_forward_success_message', {count: selectedCount}));
                    }
                }

                else
                {
                    OW.error(OW.getLanguageText('frmnewsfeedplus', 'error_in_forward_progress'));
                }
            }
        });
    },
...
OW.bind('base.avatar_user_list_select',
    function(list)
    {
        if(!$("#SearchUserForm").length || $("#SearchUserForm input[name=entityType]").val()!='frmnewsfeedplus')
        {
            return;
        }
        let extraParams = JSON.parse($("#SearchUserForm input[name=extraParams]").val());
        selectedCount=list.length;
        floatBox.close();
        $.ajax({
            type: 'POST',
            url: extraParams.forwardResponder,
            data: {"actionId":extraParams.actionId ,"sourceId": extraParams.currentId, "sendIdList": JSON.stringify(list),"privacy":extraParams.privacy,"visibility":extraParams.visibility,"feedType":extraParams.feedType,"forwardType":extraParams.forwardDesType},
            dataType: 'json',
            complete: function (data) {
                if( data.status==200 )
                {
