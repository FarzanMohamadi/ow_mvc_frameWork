...
function showEditNewsfeedComponent($eid, $etype){
    if (typeof(OWM) != "undefined"){
        editNewsfeedComponent = OWM.ajaxFloatBox('FRMNEWSFEEDPLUS_CMP_EditPostFloatBox', {iconClass: 'ow_ic_add',eId: $eid, eType: $etype})
    }else{
        editNewsfeedComponent = OW.ajaxFloatBox('FRMNEWSFEEDPLUS_CMP_EditPostFloatBox', {iconClass: 'ow_ic_add',eId: $eid, eType: $etype})
    }
}

function closeEditNewsfeedComponent($text, $actionId){
    //with reply
    if($('#action-feed1-'+$actionId + ' .ow_newsfeed_body .ow_newsfeed_content > .ow_newsfeed_content_status').length > 0){
        $('#action-feed1-'+$actionId + ' .ow_newsfeed_body .ow_newsfeed_content > .ow_newsfeed_content_status').html($text);
    ...
    for (var i = 0; i < statusBody[0].childNodes.length; i++) {
        ...
        else if (statusBody[0].childNodes[i].className!=undefined && statusBody[0].childNodes[i].className.indexOf('ow_frmnewsfeedplus_attachment')>-1) {
    ...
    var newsfeedBody = $('#action-feed1-'+$actionId + ' .owm_newsfeed_body');
    if(newsfeedBody.length != 0){
        $('.owm_newsfeed_body_pic', newsfeedBody).remove();
        $('.owm_newsfeed_body_info', newsfeedBody).remove();
        $('.owm_newsfeed_body_cont', newsfeedBody).remove();
    }
...
OW.bind('base.update_attachment', function(data) {
    if(!data || !data.pluginKey  || data.pluginKey !="frmnewsfeedplus")
    {
        return;
    }
    var attachmentIds = document.getElementById('attachment_feed_data').value;
    var dbId = false;
    if (data.pluginKey == 'frmnewsfeedplus') {
...
function showUserGroupsComponent($aid, $fid,$vis,$pri,$fty,$sid,$rqs,$title){
    if (typeof (OWM) != "undefined") {
        forwardNewsfeedComponent = OWM.ajaxFloatBox('FRMNEWSFEEDPLUS_CMP_ForwardPostFloatBox', {
            aId: $aid,
            fId: $fid,
            vis: $vis,
            pri: $pri,
        ...
        forwardNewsfeedComponent = OW.ajaxFloatBox('FRMNEWSFEEDPLUS_CMP_ForwardPostFloatBox', {
            aId: $aid,
            fId: $fid,
            vis: $vis,
            pri: $pri,
...
function newsfeedImageViewerSliderBarChanger( $object, $id, $downloadUrl, $previewUrl, $type){
    ...
    if($type == 'image'){
        ...
        $('#Extended_download_link_'+$id+'').attr('href',''+$downloadUrl+'');
...
OW.bind('frmnewsfeedplus.add.to.previewlist', function(data) {
    if(!data)
    {
        return;
    }
...
OW.bind('frmnewsfeedplus.remove.from.previewlist', function(data) {
    if(!data)
    {
        return;
    }
