...
class FRMNEWSFEEDPLUS_CMP_RenderAttachmentPreview extends OW_Component
{
    public function __construct($attachmentsList = array(), $params = array())
    {
        ...
        $thumbnailController= OW::getRouter()->urlFor('FRMNEWSFEEDPLUS_CTRL_VideoThumbnail', 'index');
        ...
        $pluginStaticUrl = OW::getPluginManager()->getPlugin('frmnewsfeedplus')->getStaticUrl();
        ...
        if ($idList != null)
        {
            $thumbnailList = FRMNEWSFEEDPLUS_BOL_ThumbnailDao::getInstance()->getThumbnailsByAttachmentIds(array_column($attachmentsList,'id'));
            $thumbnailUrl = OW::getPluginManager()->getPlugin('frmnewsfeedplus')->getUserFilesUrl();
        }
        foreach ($attachmentsList as $attachment) {
            ...
            $thumbnail = UTIL_File::getFileUrl($filePath, 'userfiles-base-attachments-' . $attachment->fileName, 100, 100, 'min', $params);
            $previewUrl = UTIL_File::getFileUrl($filePath, 'userfiles-base-attachments-' . $attachment->fileName, 600, 600, 'min', $params);
            ...
            if ($itemType == 'video') {
                ...
                {
                    $key = array_search($attachment->id, array_column($thumbnailList, 'attachmentId'));
                    $thumbnail = $thumbnailUrl . $thumbnailList[$key]->name;
                ...
                if( sizeof($attachmentsList) == 1 ){
                    $aloneAudioClass = 'aloneAudio';
                }
            }
        ...
        $this->assign('mobileMode', $mobileMode );
        $this->assign('aloneAudioClass', $aloneAudioClass );
    }
}