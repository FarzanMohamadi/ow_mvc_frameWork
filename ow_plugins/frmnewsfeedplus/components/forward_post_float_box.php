<?php
class FRMNEWSFEEDPLUS_CMP_ForwardPostFloatBox extends OW_Component
{
    /**
     * @var string
     */
    ...
    public function __construct($aid, $fid,$vis,$pri,$fty,$sid,$defaultRQS=true)
    {
        ...
        $this->defaultRQS = (boolean)$defaultRQS;
        $this->feedType=$fty;
        $this->sectionId=$sid;
        $this->feedId=$fid;
        if($fty=='groups')
        {
            ...
            $this->visibility = $vis;
            $this->privacy = $pri;
            $this->countLabel = OW::getLanguage()->text('frmnewsfeedplus', 'group_list_select_count_label');
            $this->buttonLabel = OW::getLanguage()->text('frmnewsfeedplus', 'group_list_select_button_label');
            $this->actionId=$aid;
        }
        else if($fty=='user')
        {
    ...
    public function onBeforeRender()
    {
        ...
        $this->assign('sections', FRMNEWSFEEDPLUS_BOL_Service::getInstance()->getForwardSections($this->sectionId,$this->actionId,$this->feedId,$this->visibility,$this->privacy,$this->feedType));
        /*
         * section 1: forward to group(s)
         */

        OW::getLanguage()->addKeyForJs('base', 'avatar_user_select_empty_list_message');
        OW::getLanguage()->addKeyForJs('frmnewsfeedplus', 'users_forward_success_message');
        OW::getLanguage()->addKeyForJs('frmnewsfeedplus', 'group_select_empty_list_message');
        OW::getLanguage()->addKeyForJs('frmnewsfeedplus', 'error_in_forward_progress');
        OW::getLanguage()->addKeyForJs('frmnewsfeedplus', 'groups_invite_success_message');
        if($this->sectionId==1 && FRMSecurityProvider::checkPluginActive('groups', true))
        {
            ...
            foreach ($userGroups as $userGroup) {
                ...
                $event = OW::getEventManager()->trigger(new OW_Event('frmgroupsplus.on.channel.add.widget',array('groupId' => $userGroup->id)));
                ...
                $groups[$userGroup->id] = array('groupId' => $userGroup->id, 'src' => OW::getRouter()->urlForRoute('groups-view', array('groupId' => $userGroup->id)), 'label' => $userGroup->title, 'imageUrl' => $imageUrl, 'imageInfo' => BOL_AvatarService::getInstance()->getAvatarInfo($userGroup->id, $imageUrl, 'group'));
            ...
            $forwardResponder = OW::getRouter()->urlFor('FRMNEWSFEEDPLUS_CTRL_Forward', 'forward');
    ...
    $('.asl_groups .ow_group_list_item').each(function(i,obj){
         ...
         */
            ...
            if ($this->defaultRQS) {
                ...
                array_push($this->idList, OW::getUser()->getId());
                ...
                foreach ($this->idList as $id) {
                    ...
                    if (FRMSecurityProvider::checkPluginActive('frmsecurityessentials', true)) {
                        $whoCanPostPrivacy = FRMSECURITYESSENTIALS_BOL_Service::getInstance()->getActionValueOfPrivacy('who_post_on_newsfeed', $id);
            ...
            cmp.init();  ");
                ...
                $forwardResponder = OW::getRouter()->urlFor('FRMNEWSFEEDPLUS_CTRL_Forward', 'forward');
                $extraParams= json_encode(['forwardResponder'=>$forwardResponder,'actionId'=>$this->actionId, 'currentId'=>$this->currentId,'privacy'=>$this->privacy, 'visibility'=> $this->visibility,'feedType' => $this->feedType,'forwardDesType'=>'user']);
                $this->addComponent('userSearchComponent', new BASE_CMP_SearchUserListSelect([], 'frmnewsfeedplus',OW::getUser()->getId(),null,null,$extraParams));
            }
        }
    }
}
