...
final class FRMTICKETING_BOL_TicketService
{
    ...
    public function getPageHeaderLabel($sectionId)
    {
        if ($sectionId == 1) {
            ...
            return OW::getLanguage()->text('frmticketing', 'users_setting');
        }
    }
    ...
    public function addTicket($data)
    {
        ...
        $ticketDto->ticketTrackingNumber = FRMSecurityProvider::generateUniqueId($more_entropy=true);
        ...
        $ticketDto->description = UTIL_HtmlTag::stripTagsAndJs(
            $data['description'],
        ...
        $ticketDto->description = str_replace(
            '&lt;!--more--&gt;', '<!--more-->', $ticketDto->description);

        $ticketDto->addition=json_encode(array());
    ...
    public function findTicketsByAuthorId($userId,$page)
    {
        $count = self::TICKET_PER_PAGE;
        $first = ($page - 1) * $count;
        return $this->ticketDao->findTicketsByAuthorId($userId,$this->searchTitle,$this->searchCategory,$this->searchLock,$this->searchOrder,$first, $count);
    }
    ...
    public function findAssignedTicketsByUserIdCount($userId)
    {
        $categories = FRMTICKETING_BOL_TicketCategoryUserDao::getInstance()->findCategoriesOfUser($userId);
        if($categories == array())
        {
            return 0;
        }
        return $this->ticketDao->findTicketByCategoryListCount($categories,$this->searchTitle,$this->searchCategory,$this->searchLock,$this->searchOrder);
    }

    public function findAssignedTicketsByUserId($userId,$page)
    {
        $count = self::TICKET_PER_PAGE;
        $first = ($page - 1) * $count;
        $categories = FRMTICKETING_BOL_TicketCategoryUserDao::getInstance()->findCategoriesOfUser($userId);
        return $this->ticketDao->findTicketByCategoryList($categories,$this->searchTitle,$this->searchCategory,$this->searchLock,$this->searchOrder,$first,$count);
    }
    ...
    public function getTicketFilterForm($name)
    {
        ...
        if(trim($this->searchLock)!=null) {
            ...
            {
                $lockField->setValue('unlocked');
            }elseif ($this->searchLock == 1)
            {
                $lockField->setValue('locked');
            }
        }
