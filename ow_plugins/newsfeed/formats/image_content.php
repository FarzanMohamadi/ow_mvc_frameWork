...
class NEWSFEED_FORMAT_ImageContent extends NEWSFEED_CLASS_Format
{
    public function onBeforeRender()
    {
        ...
        $stringRenderer = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_AFTER_NEWSFEED_STATUS_STRING_READ,array('string' => $tplVars['image'])));
        if(isset($stringRenderer->getData()['string'])){
            $tplVars['image'] = $stringRenderer->getData()['string'];
        }
        $stringRenderer = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_AFTER_NEWSFEED_STATUS_STRING_READ,array('string' => $tplVars['thumbnail'])));
        if(isset($stringRenderer->getData()['string'])){
            $tplVars['thumbnail'] = $stringRenderer->getData()['string'];
        }
        $stringDecode = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_AFTER_NEWSFEED_STATUS_STRING_READ,array('toDecode' => $tplVars['title'])));
        ...
        if(isset($tplVars["imageId"]) && isset($tplVars["url"]))
        {
            $tplVars["imageInfo"] = BOL_AvatarService::getInstance()->getAvatarInfo($tplVars["imageId"], $tplVars["url"], 'group');
        }else{
            $tplVars["imageInfo"] = null;
        }
