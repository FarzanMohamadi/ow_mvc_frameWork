...
function main_oghat(cityName, $timeUrl) {
    showdate();
    if (cityChanged){
...
function sun(m, d, h, lg) {
    if (m < 7) {
        // JAT = 1;
        JAT = 0; // Changed to 0 if Raeisi or AhmadiNejad are president and they will not change daylight saving hours, else change to 1
        d = 31 * (m - 1) + d + h / 24;
    } else {
        JAT = 0;
        d = 6 + 30 * (m - 1) + d + h / 24;
...
function addActiveAzanClass(elementClass) {
    document.getElementById("azan_p1").classList.remove("activeAzan");
    document.getElementById("azan_p2").classList.remove("activeAzan");
    document.getElementById("azan_p3").classList.remove("activeAzan");
    document.getElementById("azan_p4").classList.remove("activeAzan");
    document.getElementById("azan_p5").classList.remove("activeAzan");
    document.getElementById(elementClass).classList.add("activeAzan");
}
var offset;
var azanOffset;
function preShowNow($timeUrl) {
    if (offset === undefined ){
...
function showNow() {
    ...
    if (azan_ttt.getTime()+azanOffset > today.getTime()+offset) {
        addActiveAzanClass("azan_p1");
        diff = azan_ttt.getTime()+azanOffset - today.getTime()+offset;
        diff = Math.floor(diff / (1000 * 60));
        hh = Math.floor(diff / (60));
        ss = diff - (hh * 60);
        document.getElementById("azanazan").innerHTML = "<font color=#" + cl + ">" + hh + "</font><font color=#" + cl + " id=donokh>:</font><font color=#" + cl + ">" + ss + "</font> " + OW.getLanguageText('frmoghat', 'until') + " <font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'Azan_am')+"</font>";
        donokh_show();
    }
    else {
        if (azan_ttt.getTime()+azanOffset == today.getTime()+offset) {
            ...
            addActiveAzanClass("azan_p1");
            document.getElementById("azanazan").innerHTML = "<font color=#" + cl + " id=donokh></font><font color=#" + cl + ">" + OW.getLanguageText('frmoghat', 'Azan_am_time_horizon') + " " + document.getElementById("cities").innerHTML + "</font>";
        }
        else {
            azan_ttt = new Date();
            ...
            if (azan_ttt.getTime()+azanOffset > today.getTime()+offset) {
                addActiveAzanClass("azan_p2");
                diff = azan_ttt.getTime()+azanOffset - today.getTime()+offset;
                diff = Math.floor(diff / (1000 * 60));
                hh = Math.floor(diff / (60));
                ss = diff - (hh * 60);
                document.getElementById("azanazan").innerHTML = "<font color=#" + cl + ">" + hh + "</font><font color=#" + cl + " id=donokh>:</font><font color=#" + cl + ">" + ss + "</font> "+OW.getLanguageText('frmoghat', 'until')+" <font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'Azan_pm')+"</font>";
                donokh_show();
            }
            else {
                if (azan_ttt.getTime()+azanOffset == today.getTime()+offset) {
                    addActiveAzanClass("azan_p2");
                    document.getElementById("azanazan").innerHTML = "<font color=#" + cl + " id=donokh></font><font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'Sunrise')+"</font>";
                }
                else {
                    azan_ttt = new Date();
                    ...
                    if (azan_ttt.getTime()+azanOffset > today.getTime()+offset) {
                        ...
                        if (azan_ttt.getTime()+azanOffset == today.getTime()+offset) {
                            document.getElementById('displ').style.display = 'block';
                            addActiveAzanClass("azan_p3");
                            document.getElementById("azanazan").innerHTML = "<font color=#" + cl + " id=donokh></font><font color=#" + cl + ">" + OW.getLanguageText('frmoghat', 'Azan_pm_time_horizon') + " " + document.getElementById("cities").innerHTML + "</font>";
                        }
                        else {
                            azan_ttt = new Date();
                            ...
                            if (azan_ttt.getTime()+azanOffset > today.getTime()+offset) {
                                addActiveAzanClass("azan_p4");
                                diff = azan_ttt.getTime()+azanOffset - today.getTime()+offset;
                                diff = Math.floor(diff / (1000 * 60));
                                hh = Math.floor(diff / (60));
                                ss = diff - (hh * 60);
                                document.getElementById("azanazan").innerHTML = "<font color=#" + cl + ">" + hh + "</font><font color=#" + cl + " id=donokh>:</font><font color=#" + cl + ">" + ss + "</font> "+OW.getLanguageText('frmoghat', 'until')+" <font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'Sunset')+"</font>";
                                donokh_show();
                            }
                            else {
                                if (azan_ttt.getTime()+azanOffset == today.getTime()+offset) {
                                    addActiveAzanClass("azan_p4");
                                    document.getElementById("azanazan").innerHTML = "<font color=#" + cl + " id=donokh></font><font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'Sunset')+"</font>";
                                }
                                else {
                                    azan_ttt = new Date();
                                    ...
                                    if (azan_ttt.getTime()+azanOffset > today.getTime()+offset) {
                                        addActiveAzanClass("azan_p5");
                                        diff = azan_ttt.getTime()+azanOffset - today.getTime()+offset;
                                        diff = Math.floor(diff / (1000*60));
                                        hh = Math.floor(diff / (60));
                                        ss = diff - (hh * 60);
                                        document.getElementById("azanazan").innerHTML = "<font color=#" + cl + ">" + hh + "</font><font color=#" + cl + " id=donokh>:</font><font color=#" + cl + ">" + ss + "</font> "+OW.getLanguageText('frmoghat', 'until')+" <font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'azan_maghreb')+"</font>";
                                        donokh_show();
                                    }
                                    else {
                                        if (azan_ttt.getTime()+azanOffset == today.getTime()+offset) {
                                            document.getElementById('displ').style.display = 'block';
                                            addActiveAzanClass("azan_p5");
                                            document.getElementById("azanazan").innerHTML = "<font color=#" + cl + " id=donokh></font><font color=#" + cl + ">"+OW.getLanguageText('frmoghat', 'azan_maghreb_time_horizon') + " " + document.getElementById("cities").innerHTML + "</font>";
...
function changeCity($timeUrl){
    ...
    cityChanged = true;
    main_oghat(selectedCity.value, $timeUrl);
}

cl = "000000"; // range bold

var options = { weekday: 'long', year: 'numeric', month: 'numeric', day: 'numeric' };
var today  = new Date();
persian_date = today.toLocaleDateString("fa-IR", options);
$(document).ready(function(){
    $('.ic_oghat').after('<div class="todaysDate">' +persian_date + "</div>");
});
