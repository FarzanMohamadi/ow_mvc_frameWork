...
class PHOTO_MCTRL_Upload extends OW_MobileActionController
{
    ...
    public function photo( array $params = null )
    {
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_PHOTO_RENDER));
    }
    public function photos( array $params = null )
    {
        ...
        else
        {
            ...
            $('#upload-file-field').change(function(){
                ...
                $('#imagesDiv').append('<div class=\"owm_preloader\"></div>');
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        if(status == 'ok'){
                            ...
                            var image = '<span class=\"owm_photo_display_container\" style=\"background-image: url('+fileUrl+');\"></span>'
                            ...
                            $('#imagesDiv .owm_preloader').remove()
    ...
    public function ajaxUpload($params){
        ...
        if(!OW::getUser()->isAuthenticated())
        {
            $errorMessage = OW::getLanguage()->text('base', 'user_is_not_authenticated');
    ...
    protected function onSubmitComplete( $entityType, $entityId, PHOTO_BOL_PhotoAlbum $album, $photos )
    {
        ...
        {
            if ( $photo->status == PHOTO_BOL_PhotoDao::STATUS_APPROVAL )
            {
