...
{'use strict';
    ...
    {
        case 'albums':
            actionStr = 'setURL,setDescription,setUserInfo,setPhotosCount,setAlbumName,setSecurity';
            break;
        case 'albumPhotos':
...
{'use strict';
    ...
    var Slot = (function( BaseObject )
    {
        ...
        function Slot( slot, listType, listView )
        {
            ...
            this.node
                ...
                .data('slotId', slot.id);
            this.data = $.extend({listType: listType, listView: listView}, slot);
        }
        ...
        {
            this.node.find('.ow_photo_pint_album').on('click', this.self(function()
            {
        ...
        Slot.prototype.setAlbumName = utils.fluent(function()
        {
            ...
            if ( name )
            {
                this.node.find('.ow_photo_item_info_name').show().find('.ow_photo_name').html(name);
            }
        });

        Slot.prototype.setPhotosCount = utils.fluent(function()
        {
            var photosCount, data = this.data;

            photosCount = data.count || 0;

            if ( photosCount )
            {
                this.node.find('.ow_photo_item_info_count').show().find('.ow_photo_count').html(photosCount);
            }
        });
        ...
        Slot.prototype.setSecurity = utils.fluent(function()
        {
            if(this.node.find('.feed_image_privacy').length>0 && this.data.privacy_label) {
                this.node.find('.feed_image_privacy').show();
        ...
        Slot.prototype.setUserInfo = utils.fluent(function()
        {
            ...
            let avatar_info = get_avatar_info(this.data.userAvatarUrl, this.data.userId);
            if (avatar_info['is_empty']){
                $(user_avatar).css("background-color", avatar_info['color']);
            }
    ...
    var SlotManager = (function( BaseObject )
    {
        ...
        SlotManager.prototype.load = utils.fluent(function( data )
        {
            ...
            if(data)
            {
                params.action = data.ajaxFunc;
                params.listType = data.listType;
                params.listView = data.listView;
                params.showFilter = data.showFilter || null;
                params.searchVal = data.searchVal || null;
                params.hashId = data.id || null;
            }else{
                data = {};
            }

            if (params.listView === "albums" || data.listView === "albums"){
                var ajaxData = $.extend({
                    ajaxFunc: params.action || 'getAlbumList',
                    listType: params.listType || 'latest',
                    listView: params.listView || 'albums',
                    searchVal: params.searchVal || null,
                    showFilter: params.showFilter || null,
                    id: params.hashId || null,
                    offset: ++this.offset
                ...
                var ajaxData = $.extend({
                    ...
                    listType: params.listType || 'latest',
                    listView: params.listView || 'photos',
                    searchVal: params.searchVal || null,
                    showFilter: params.showFilter || null,
                    id: params.hashId || null,
                    offset: ++this.offset
            ...
            ajaxData.listType = data.listType || ajaxData.listType;
            ajaxData.listView = data.listView || ajaxData.listView;
            ...
            this.request = $.ajax({
                ...
                beforeSend: this.self(function( jqXHR, settings )
                {
                    ...
                    if ( data.hasOwnProperty('listView') )
                    {
        ...
        SlotManager.prototype.buildPhotoItem = utils.fluent(function( slot )
        {
            ...
            var slotItem = new Slot(slot, this.listType, this.listView);
    ...
    var List = (function( BaseObject )
    {
        ...
        List.prototype.hideLoader = utils.fluent(function()
        {
            ...
            {
                $(".ow_photo_search_actions_container").hide();
                this.content.append(
                    $('<div class="ow_nocontent">', {style: 'text-align:center; padding-top: 24px;'}).html(OW.getLanguageText('photo', 'no_items'))
                );
            }
        });
        ...
        SearchResultItem.prototype.setSearchResultItemInfo = utils.fluent(function()
        {
            ...
            switch ( this.getData('searchType') )
            {
                case 'user':
                    ...
                    var user_avatar = this.node.find(".user_avatar").show();
                    let avatar_info = get_avatar_info(this.getData('avatar'), this.getData('id'));
                    if (avatar_info['is_empty']){
                        $(user_avatar).css("background-color", avatar_info['color']);
                    }
    ...
    var SearchEngine = (function( BaseObject )
    {
        ...
        function SearchEngine()
        {
            ...
            this.listViewSelect = $('.ow_fw_menu > select[name="choose_photo_search_show_view"]');
            instance = this;
        }
        ...
        function check_close_icon_visibility(target) {
            if (target.value.length === 0){
                ...
                $(".ow_searchbar_btn").removeClass("ow_searchbar_btn_active");
            }
            else {
                $(target).parent().addClass("full_search_input").removeClass("empty_search_input");
                $(".ow_searchbar_btn").addClass("ow_searchbar_btn_active");
            }
        }
        ...
        SearchEngine.prototype.init = utils.fluent(function()
        {
            ...
            this.search_submit_button.on('click', this.self(function( event )
            {
                ...
                if (searchVal.length === 0) {
                    if(event.currentTarget.id == "selectType"){
                        params['listType'] = event.currentTarget.value;
                    }else if (event.currentTarget.id == "selectView"){
                        params['listView'] = event.currentTarget.value;
                    }
                    this.loadList(params['listType'], params['listView']);
                }else
            ...
            this.listViewSelect.on('change', this.self(function( event )
            {
                event.preventDefault();
                //after changing listView (in search page)
            }));
            ...
            $('.ow_btn_close_search', this.searchBox).on('click', this.self(function()
            {
                ...
                SlotManager().load({listType: params['showFilter'],listView: params['listView']});
            }));
        ...
        SearchEngine.prototype.getSearchResultPhotos = utils.fluent(function( mixed )
        {
            ...
            data.listView = params.listView;
            this.searchData = data;
            SlotManager().load(data);
        });

        SearchEngine.prototype.loadList = utils.fluent(function( listType, listView, searchVal )
        {
            this.resetPhotoListData();
            ...
            this.destroySearchResultList();
            SlotManager().load({listType: listType,listView: listView, searchVal: searchVal});
        });
    ...
    return root.Object.freeze({
        init: function()
        {
            ...
            if ( ['albums', 'userPhotos', 'albumPhotos', 'photo_friends'].indexOf(params.listType) === -1 )
            {
    ...
    var _methods = {
        ...
        init: function()
        {
            ...
            OW.bind('photo.onRenderPhotoItem', function( slot )
            {
                ...
                if ( ( _params.listType == 'albums' || _params.listView == 'albums' ) && slot.name.trim() == OW.getLanguageText('photo', 'newsfeed_album').trim() )
                {
...
window.onkeyup = function(e) {
    ...
    if (key == 37) {
        $('.floatbox_container .ow_photoview_arrow_left').click();
    }else if (key == 39) {
        $('.floatbox_container .ow_photoview_arrow_right').click();
    }
}
