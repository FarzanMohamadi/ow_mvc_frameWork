...
}.call(window, jQuery, window.ajaxPhotoUploadParams, function( $, params, undf )
{
        ...
        FileManager.prototype.upload = fluent(function( file )
        {
            ...
            else
            {
                ...
                $.ajax({
                    ...
                    {
                        if(jqXHR !=null && jqXHR.status !=null && jqXHR.status == 400){
                            OW.error(OW.getLanguageText('base', 'upload_bad_request_error'));
                        }else {
    ...
    var AlbumListManager = (function()
    {
        ...
        AlbumListManager.prototype.bindEvents = fluent(function()
        {
            ...
            })).eq(0).on('click', self(function()
            {
                ...
                if(this.albumForm != null && this.albumForm != 'undefined'){
                    $('textarea', this.albumForm).val('');
                }else if(this.form != null && this.form != 'undefined'){
    ...
    root.OW.bind('base.onFormReady.ajax-upload', function()
    {
        ...
        {
            var textareas = $('#'+this.id+ ' textarea.ow_hidden.invitation');
            ...
            root.Object.keys(fileManager.cache).forEach(function( index )
            {
                ...
                var value = invitation;//(file.description || file.editor.getValue()).trim();
                textareas.each(function( key ) {
                    var slotId = textareas[key].parentElement.parentElement.id; // slot-id
                    if(slotId == 'slot-'+index){
                        value = textareas[key].value;
                    }
                });
