<?php
/**
 * @author Farzan Mohammadi <farzan.mohamadii@gmail.com>
 * Date: 8/25/2017
 * Time: 10:29 AM
 */

$languageService = Updater::getLanguageService();

$languageService->addOrUpdateValueByLanguageTag('fa-IR', 'friends', 'friend_request_was_sent', 'درخواست افزودن دوست فرستاده شد');
$languageService->addOrUpdateValueByLanguageTag('en', 'friends', 'friend_request_was_sent', 'Friend request has been sent');
$languageService->addOrUpdateValueByLanguageTag('fa-IR', 'friends', 'feedback_request_was_sent', 'درخواست افزودن دوست ارسال شد');
$languageService->addOrUpdateValueByLanguageTag('en', 'friends', 'feedback_request_was_sent', 'Friend request has been sent');
