...
//        OW::getEventManager()->bind('friends.request-sent',                     array($this,'onSentRequest'));
    ...
    public function onCollectProfileActionTools( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( $dto === null )
        {
            ...
            $class = $class." add_to_friends_button";
        }
        else
        {
            switch ( $dto->getStatus() )
            {
                case FRIENDS_BOL_Service::STATUS_ACTIVE:
                    ...
                    $class = $class." remove_from_friends_button";
                    break;
                ...
                case FRIENDS_BOL_Service::STATUS_PENDING:
                    ...
                    if ( $dto->getUserId() == OW::getUser()->getId() )
                    {
                        ...
                        $class = $class." friend_request_was_sent_button";
                        $href = $router->urlFor('FRIENDS_CTRL_Action', 'cancel', array('id' => $userId, 'redirect'=>true));
                        $frmSecuritymanagerEvent= OW::getEventManager()->trigger(new OW_Event('frmsecurityessentials.on.generate.request.manager',
                            array('senderId'=>OW::getUser()->getId(),'receiverId'=>$userId,'isPermanent'=>true,'activityType'=>'cancel_friends')));
                        if(isset($frmSecuritymanagerEvent->getData()['code'])){
...
//                        $extra_label = $language->text('friends', 'cancel_request');
        ...
        $resultArray = array(
            ...
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => $class,
...
//    }
    ...
    {
        $language = OW::getLanguage();
        $privacyValueEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_PRIVACY_ITEM_ADD, array('key' => 'friends_view')));
