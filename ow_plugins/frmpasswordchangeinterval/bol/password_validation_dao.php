 ...
 * @since 1.0
 */
class FRMPASSWORDCHANGEINTERVAL_BOL_PasswordValidationDao extends OW_BaseDao
{
    ...
    {
        return OW_DB_PREFIX . 'frmpasswordchangeinterval_password_validation';
    }
    ...
    public function setAllUsersPasswordInvalid($sendEmail)
    {
        ...
        foreach ($users as $key => $user) {
            ...
            FRMPASSWORDCHANGEINTERVAL_BOL_Service::getInstance()->sendNotificationToCurrentUserForChangingPassword($user->getId());
        }
     ...
     * @return int
     */
    public function setPasswordExpireForUserIds($userIds) {
        $savedPasswordValidation = array();
        ...
        foreach ($userIds as $userId) {
            ...
            if(!isset($old_row)){
                $token = md5(UTIL_String::getRandomString(8, 5));
                $savedPasswordValidation[] = $this->createPasswordValidationObject($userId, true, $token);
            }
        }
    ...
    public function createPasswordValidationObject($userId, $valid, $token, $time = null)
    {
        ...
        $passwordValidation = new FRMPASSWORDCHANGEINTERVAL_BOL_PasswordValidation();
    ...
    public function createEmailForChangingPassword($email, $token, $username)
    {
        ...
        $mail->setSubject(OW::getLanguage()->text('frmpasswordchangeinterval', 'email_for_changing_password_subject'));
    ...
    {
        $html = '<p>' . OW::getLanguage()->text('frmpasswordchangeinterval', 'email_for_changing_password_description', array('username' => $username)) . '</p>';
        $html .= '<br/>';
        $html .= OW::getRouter()->urlForRoute('frmpasswordchangeinterval.check-validate-password', array('token' => $token));
        return $html;
    }
