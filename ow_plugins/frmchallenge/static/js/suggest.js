var chaluserListCache = {};
var chaluserLastItems;
function chaluser_wait_for_at() {
    var text_input_selector = '.solitary_information input[name="opponent_username"]';
    setInterval(function() {
        if(chaluserLastItems != $(text_input_selector).length){
            chaluserLastItems = $(text_input_selector).length;
            $(text_input_selector).suggest_chaluser('@', {
                data: function (q) {
                    if(q.length <2 ) return [];
                    if (q in chaluserListCache)
                        return chaluserListCache[q];
                    var ret = $.getJSON(chaluserLoadUsernamesUrl + q);
                    chaluserListCache[q] = ret;
                    return ret;
                },
                map: function (user) {
                    return {
...
(function ($) {
    ...
    var Suggest_chaluser = function(el, key, options) {
        var that = this;

        this.$element = $(el);
        this.$items = undefined;
        this.options = $.extend(true, {}, $.fn.suggest_chaluser.defaults, options, this.$element.data(), this.$element.data('options'));
        this.key = key;
        this.isShown = false;
        this.query = '';
        this._queryPos = [];
    ...
    Suggest_chaluser.prototype = {
        __setListener: function() {
            this.$element
                .on('suggest_chaluser.show', $.proxy(this.options.onshow, this))
                .on('suggest_chaluser.select', $.proxy(this.options.onselect, this))
                .on('suggest_chaluser.lookup', $.proxy(this.options.onlookup, this))
        ...
        __keyup: function(e) {
            ...
            var arrowChars = [37,38,39,40]; //don't query when already shown
            if (this.isShown && $.inArray(e.keyCode, arrowChars) !== -1) {
                return;
            }
            var specialChars = [13, 27, 32,    18, 9, 20, 91, 93, 36, 35, 45, 33, 34, 144, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123, 145, 19];
            ...
            if(notFound){
                this.query = '';
                this._queryPos = [currentPos, currentPos];
                this._keyPos = currentPos;
                this.hide();
                return true;
            }
        },
        ...
        __build: function() {
            ...
            $dropdown
                .on('click', 'li:has(a)', function(e) {
                    e.preventDefault();
                    $('.solitary_information input[name="opponent_username"]').val('');
        ...
        __select: function(index) {
            ...
            $el.trigger($.extend({type: 'suggest_chaluser.select'}, this), item);
        ...
        __lookup: function(q, $resultItems) {
            this.$element.trigger($.extend({type: 'suggest_chaluser.lookup'}, this), [q, $resultItems]);
        ...
        lookup: function(q) {
            ...
            if(data && $.isFunction(data.promise)) {
                data.done(function(data) {
                    if(q != that.query)
                        return;
                    if(!that.$element[0].value.includes(q))
                        return;
        ...
        show: function() {
            ...
            if (!this.isShown) {
                ...
                if (options.position !== false) {
                    ...
                    if (typeof options.position == 'string') {
                        switch (options.position) {
                            ...
                            case 'caret_right':
                                position.top = caretPos.top - el.scrollTop;
                                position.left = caretPos.left - el.scrollLeft - $dropdownMenu.width();
                                break;
                            case 'caret_center':
                                position.top = caretPos.top - el.scrollTop;
                                position.left = caretPos.left - el.scrollLeft - ($dropdownMenu.width()/2);
                                break;
                        }
                ...
                this.isShown = true;
                $el.trigger($.extend({type: 'suggest_chaluser.show'}, this));
            }
        }
    };

    var old = $.fn.suggest_chaluser;

    // .suggest_chaluser( key [, options] )
    // .suggest_chaluser( method [, options] )
    // .suggest_chaluser( suggestions )
    $.fn.suggest_chaluser = function(arg1) {
        var arg2 = arguments[1];

        var createSuggest_chaluser = function(el, suggestions) {
            var newData = {};
            $.each(suggestions, function(keyChar, options) {
                var key =  keyChar.toString().charAt(0);
                newData[key] = new Suggest_chaluser(el, key, typeof options == 'object' && options);
            });
        ...
        return this.each(function() {
            var that = this,
                $this = $(this),
                data = $this.data('suggest_chaluser'),
                suggestion = {};
            ...
            if (typeof arg1 == 'string') {
                if (arg1.length > 1 && data) {
                    ...
                    if (arg2) {
                        ...
                        if (!data) {
                            $this.data('suggest_chaluser', createSuggest_chaluser(this, suggestion));
                        } else if (data && !(arg1 in data)) {
                            $this.data('suggest_chaluser', $.extend(data, createSuggest_chaluser(this, suggestion)));
                        }
                    }
                }
            } else {
                // arg1 contains set of suggestions
                if (!data) $this.data('suggest_chaluser', createSuggest_chaluser(this, arg1));
                else if (data) {
                    ...
                    $this.data('suggest_chaluser', $.extend(data, createSuggest_chaluser(that, suggestion)))
                }
            }
        });
    };

    $.fn.suggest_chaluser.defaults = {
        data: [],
        map: undefined,
        filter: {
            casesensitive: false,
            limit: chaluserMaxCount
        },
        dropdownClass: '',
        position: 'caret',
        // events hook
    ...
    $.fn.suggest_chaluser.Constructor = Suggest_chaluser;

    $.fn.suggest_chaluser.noConflict = function () {
        $.fn.suggest_chaluser = old;
        return this;
    }
