<?php
class FRMCOMPETITION_MCTRL_Competition extends OW_MobileActionController
{

    public function index($params)
    {
        OW::getDocument()->setTitle(OW::getLanguage()->text('frmcompetition', 'main_menu_item'));
        OW::getDocument()->setHeading(OW::getLanguage()->text('frmcompetition', 'main_menu_item'));
        ...
        foreach ($competitions as $competition) {
            $competitionsInf = array(
                ...
                'url' => OW::getRouter()->urlForRoute('frmcompetition.competition', array('id' => $competition->id))
            );
...
//            }
            ...
            $competitionsInf['toolbar'][] = array('label' => OW::getLanguage()->text('frmcompetition', 'endDate') . ': ' . UTIL_DateTime::formatSimpleDate($competition->endDate, true), 'class' => 'competition_start_date ow_ipc_date');
...
//            $competitionsInf['sizeOfParticipant'] = $sizeOfParticipant;
    ...
    public function viewCompetition($params){
        ...
        OW::getDocument()->setHeading($competition->title);
        ...
        if($competition->type == $service->TYPE_GROUP) {
            ...
            foreach ($competitionGroups as $competitionGroup) {
                ...
                $competitionGroupInf = array(
                    ...
                    'url' => OW::getRouter()->urlForRoute('groups-view', array('groupId' => $group->id)),
                    'title' => $group->title,
        ...
        $this->assign('backUrl', OW::getRouter()->urlForRoute('frmcompetition.index'));
        ...
        $cssDir = OW::getPluginManager()->getPlugin("frmcompetition")->getStaticCssUrl();
        OW::getDocument()->addStyleSheet($cssDir . "frmcompetition.css");
        OW::getEventManager()->trigger(new OW_Event(FRMCOMPETITION_BOL_Service::ON_BEFORE_COMPETITION_VIEW_RENDER));
    }
}