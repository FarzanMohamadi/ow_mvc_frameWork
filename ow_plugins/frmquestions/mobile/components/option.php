 ...
 * Time: 1:11 PM
 */
class FRMQUESTIONS_MCMP_Option extends OW_MobileComponent
{
    ...
    public function __construct(FRMQUESTIONS_BOL_Option $opt, $userId, $editMode = false, $multiple = true, $disabled = false, $additionalInfo = array())
    {
        ...
        $this->setTemplate(OW::getPluginManager()->getPlugin('frmquestions')->getMobileCmpViewDir() . 'option.html');
    }
    ...
    public function onBeforeRender()
    {
        ...
        $avatarList = new FRMQUESTIONS_MCMP_Answers($this->userIds, $this->answerCount, $this->additionalInfo);
        ...
        foreach ($idList as $item)
            $userIds[] = (int) $item;
        $showUsers = 'javascript: OWM.showUsers('.json_encode($userIds).')';
