 ...
 * Time: 10:03 AM
 */
class FRMQUESTIONS_BOL_QuestionDao extends OW_BaseDao
{
    const OWNER = 'owner';
    ...
    {
        return OW_DB_PREFIX . 'frmquestions_question';
    }
    ...
    public function findAllByUserSortByTime($count = 5, $last = 0)
    {
        ...
        $params = array(
            ...
            'every_body' => FRMQUESTIONS_BOL_Service::PRIVACY_EVERYBODY,
            'friends_only' => FRMQUESTIONS_BOL_Service::PRIVACY_FRIENDS_ONLY,
            'only_me' => FRMQUESTIONS_BOL_Service::PRIVACY_ONLY_FOR_ME,
            'row' => $last,
            'num' => $count,
        );
        return $this->dbo->queryForObjectList($query, FRMQUESTIONS_BOL_Question::class, $params);
    }
    ...
    public function findAllHottestQuestion($count = 5, $last = 0)
    {
        ...
        $query = "
            SELECT *  FROM `".OW_DB_PREFIX."frmquestions_question` AS q1
            INNER JOIN  (
                SELECT `questions`.`id` as `id`,COUNT(`ans`.id) as cnt FROM (
                    SELECT `q`.* FROM `" . $this->getTableName() . "` AS `q`
                        WHERE `q`.`privacy` = :every_body AND `q`.`context` <> 'groups' AND `q`.`entityType` IS NOT NULL AND `q`.`entityId` IS NOT NULL
                ...
                LEFT JOIN `".FRMQUESTIONS_BOL_AnswerDao::getInstance()->getTableName()."` AS `ans` ON `questions`.`id` = `ans`.`questionId`
    ...
    public function findAllHottestQuestionCount()
    {
        ...
        $query = "
            SELECT COUNT(*) as cnt  FROM `".OW_DB_PREFIX."frmquestions_question` AS q1
                INNER JOIN  (
                SELECT `questions`.`id` as `id`,COUNT(`ans`.id) as cnt FROM (
                    SELECT `q`.* FROM `" . $this->getTableName() . "` AS `q`
                        WHERE `q`.`privacy` = :every_body AND `q`.`context` <> 'groups' AND `q`.`entityType` IS NOT NULL AND `q`.`entityId` IS NOT NULL
