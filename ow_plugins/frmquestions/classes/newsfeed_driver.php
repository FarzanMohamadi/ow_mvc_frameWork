 ...
 * @since 1.0
 */
class FRMQUESTIONS_CLASS_NewsfeedDriver extends NEWSFEED_CLASS_Driver
{
    private $listCount = 0;
    ...
    {
        if (!empty($limit))
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findAllQuestions(intval($limit[0]), intval($limit[1]));
        else
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findAllQuestions();
        $idList = array();
    ...
    {
        if (!empty($limit))
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findHottestQuestions(intval($limit[0]), intval($limit[1]));
        else
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findHottestQuestions();
        $idList = array();
        foreach ($list as $question) {
            $action = NEWSFEED_BOL_Service::getInstance()->findAction($question->entityType, $question->entityId);
            if (isset($action))
    ...
    {
        if (!empty($limit))
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findMyQuestions(intval($limit[0]), intval($limit[1]));
        else
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findMyQuestions();
        $idList = array();
        foreach ($list as $question) {
            $action = NEWSFEED_BOL_Service::getInstance()->findAction($question->entityType, $question->entityId);
            if (isset($action))
    ...
    {
        if (!empty($limit))
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findFriendQuestions(intval($limit[0]), intval($limit[1]));
        else
            list($list, $count) = FRMQUESTIONS_BOL_Service::getInstance()->findFriendQuestions();
        $idList = array();
        foreach ($list as $question) {
            $action = NEWSFEED_BOL_Service::getInstance()->findAction($question->entityType, $question->entityId);
            if (isset($action))
