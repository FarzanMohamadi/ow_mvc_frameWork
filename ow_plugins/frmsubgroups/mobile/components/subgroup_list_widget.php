<?php
class FRMSUBGROUPS_MCMP_SubgroupListWidget extends BASE_CLASS_Widget
{

    /**
     * FRMSUBGROUPS_MCMP_SubgroupListWidget constructor.
    ...
    private function assignList( $groupId, $count )
    {
        ...
        foreach ( $subGroupsDto as $group )
        {
            ...
            $subGroupList[$group->id]['imageUrl'] =  GROUPS_BOL_Service::getInstance()->getGroupImageUrl($group);
        }
        ...
        $this->setTemplate(OW::getPluginManager()->getPlugin('frmsubgroups')->getMobileCmpViewDir() . 'subgroup_list_widget.html');
