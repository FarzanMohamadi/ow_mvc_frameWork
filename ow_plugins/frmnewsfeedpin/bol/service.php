<?php
class FRMNEWSFEEDPIN_BOL_Service
{
    ...
    public function afterStatusComponentAddition(OW_Event $event)
    {
        ...
        $jsDir = OW::getPluginManager()->getPlugin("frmnewsfeedpin")->getStaticJsUrl();
        OW::getDocument()->addScript($jsDir . 'newsfeed_pin.js');
        $cssDir = OW::getPluginManager()->getPlugin("frmnewsfeedpin")->getStaticCssUrl();
    ...
    public function feedRemove(OW_Event $event)
    {
        ...
        if (isset($params['action_id'])) {
            ...
            if (isset($action)) {
                ...
                FRMNEWSFEEDPIN_BOL_PinDao::getInstance()->deleteByEntityIdAndEntityType($entityId, $entityType);
            }
        }
    }
    ...
    {
        if(!FRMSecurityProvider::checkPluginActive('groups', true) || !isset($group))
            return false;
...
//            return;
        ...
        if($params['feedType'] == 'groups') {
            ...
            if (FRMSecurityProvider::checkPluginActive('groups', true)) {
                if ($group == null || !($group instanceof GROUPS_BOL_Group)) {
                    $group = $this->findGroupById($groupId);
                }
            }

            $isCurrentUserManager = false;
            $checkUserManager = true;
            ...
            if (isset($params['additionalInfo']['currentUserIsManager']) && isset($params['additionalInfo']['entityId']) && $params['additionalInfo']['entityId'] == $groupId) {
                ...
                $isCurrentUserManager = $params['additionalInfo']['currentUserIsManager'];
                $checkUserManager = false;
            } else if (isset($params['cache']['groups_managers'])) {
                if (isset($params['cache']['groups_managers'][$groupId])) {
                    $isCurrentUserManager = in_array(OW::getUser()->getId(), $params['cache']['groups_managers'][$groupId]);
                    $checkUserManager = false;
                }
            ...
            $canEditPin = $isCurrentUserManager || $this->canEditGroup($group, $checkUserManager);
        }else{
        ...
        if (isset($canEditPin)) {
            ...
            $pin = null;
            if (isset($params['cache']['pinned_actions'])) {
                if (isset($params['cache']['pinned_actions'][$entityType . '-' . $entityId])) {
                    $pin = true;
                }
            } else {
                $pin = FRMNEWSFEEDPIN_BOL_PinDao::getInstance()->findByEntityIdAndEntityType($entityId, $entityType);
            }
            if (isset($pin)) {
                $data['view']['class'] = 'frmnewsfeedpin_pined_class';
            }
            if ($canEditPin) {
                if (!isset($pin)) {
                    array_unshift($data['contextMenu'], array(
                        'label' => OW::getLanguage()->text('frmnewsfeedpin', 'pin_button_label'),
                        "class" => "frmnewsfeedpin_add_pin frmnewsfeedpin_pin_id_[" . $entityType . ']_[' . $entityId . ']'
                    ));
                } else {
                    array_unshift($data['contextMenu'], array(
                        'label' => OW::getLanguage()->text('frmnewsfeedpin', 'un_pin_button_label'),
                        'attributes' => array(
                            'data-confirm-msg' => OW::getLanguage()->text('base', 'are_you_sure')
                        ),
                        "class" => "frmnewsfeedpin_remove_pin frmnewsfeedpin_pin_id_[" . $entityType . ']_[' . $entityId . ']'
                    ));
                }
            }
            $isPinnedPage = false;
            ...
            $dataParams = array(
                'removeURL' => OW::getRouter()->urlForRoute('frmnewsfeedpin.pin_delete'),
                'addURL' => OW::getRouter()->urlForRoute('frmnewsfeedpin.add_pin_by_entity'),
                'entityId' => $entityId,
                'entityType' => $entityType,
                'isPinnedPage' => $isPinnedPage
            );
            $js = UTIL_JsGenerator::composeJsString('
                    window.ow_frmnewsfeedpin_feed_list[{$feedAutoId}] = new FRMNEWSFEEDPIN_PinItem({$isPinned},{$feedAutoId});
                    window.ow_frmnewsfeedpin_feed_list[{$feedAutoId}].construct({$data});
                ', array(
    ...
    public function findPinedFeedByFeed($feedType, $feedId, $limit = null, $startTime = null, $formats = null, $driver = null, $endTime = null)
    {
        ...
        if (OW::getUser()->isAuthenticated() && OW::getUser()->isAdmin()) {
            ...
            $eventPrivacyCondition = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_QUERY_FEED_CREATE, array('feedId' => $feedId)));
        ...
        $query = 'SELECT action.id FROM ' . NEWSFEED_BOL_ActionDao::getInstance()->getTableName() . ' action
            ...
            INNER JOIN ' . FRMNEWSFEEDPIN_BOL_PinDao::getInstance()->getTableName() . ' pin ON pin.entityId=action.entityId AND pin.entityType=action.entityType
