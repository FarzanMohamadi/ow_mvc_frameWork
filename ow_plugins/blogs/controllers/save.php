...
class BLOGS_CTRL_Save extends OW_ActionController
{
    ...
    public function delete( $params )
    {
        ...
        if(isset($author)){
            $this->redirect(OW::getRouter()->urlForRoute('blog-manage-drafts'));
        }else{
...
abstract class BlogPostForm extends Form
{
    public function __construct($formName)
    {
        ...
        $postTextArea = new WysiwygTextarea('post','blogs', $buttons);
...
class EditBlogPostForm extends BlogPostForm
{
    ...
    public function __construct($postDto)
    {
        ...
        if(!$postDto->isDraft())
        {
            ...
            $draftText = OW::getLanguage()->text('blogs', 'change_status_to_publish');
            $draftSubmit->addAttribute('onclick', "$('#save_post_command').attr('value', 'publish');$(this).addClass('ow_inprogress');");
        }
        $this->addElement($draftSubmit->setValue($draftText));
