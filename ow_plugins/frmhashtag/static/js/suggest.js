/* ===================================================
 * BASED ON bootstrap-suggest.js v1.2.2 [http://github.com/lodev09/bootstrap-suggest]
 * EDITED BY Issa Annamoradnejad (i.moradnejad@gmail.com)
 * ===================================================
...
(function ($) {
    ...
    var Suggest_hashtag = function(el, key, options) {
        var that = this;

        this.$element = $(el);
        this.$items = undefined;
        this.options = $.extend(true, {}, $.fn.suggest_hashtag.defaults, options, this.$element.data(), this.$element.data('options'));
        this.key = key;
        this.isShown = false;
        this.query = '';
        this._queryPos = [];
    ...
    Suggest_hashtag.prototype = {
        __setListener: function() {
            this.$element
                .on('suggest_hashtag.show', $.proxy(this.options.onshow, this))
                .on('suggest_hashtag.select', $.proxy(this.options.onselect, this))
                .on('suggest_hashtag.lookup', $.proxy(this.options.onlookup, this))
        ...
        __select: function(index) {
            ...
            $el.trigger($.extend({type: 'suggest_hashtag.select'}, this), item);
        ...
        __lookup: function(q, $resultItems) {
            this.$element.trigger($.extend({type: 'suggest_hashtag.lookup'}, this), [q, $resultItems]);
        ...
        show: function() {
            ...
            if (!this.isShown) {
                ...
                if (options.position !== false) {
                    ...
                    if (typeof options.position == 'string') {
                        switch (options.position) {
                            ...
                            case 'mobile_top':
                                $dropdownMenu.show();
                                var hh = $dropdownMenu.outerHeight(false) + 4;
                                $dropdownMenu.css('position', 'relative');
                                $dropdownMenu.css('margin-top',-hh);
                                position.top = 0;
                                position.left = 0;
                                position.right = 0;
                                break;
                            case 'mobile_bottom':
                                var hh = $dropdownMenu.outerHeight(false);
                                $dropdownMenu.css('margin-top',-hh);
                                position.top = hh+$el.outerHeight()+$el.position().top;
                                position.left = 0;
                                position.right = 0;
                                break;
                        }
                ...
                this.isShown = true;
                $el.trigger($.extend({type: 'suggest_hashtag.show'}, this));
            }
        }
    };

    var old = $.fn.suggest_hashtag;

    // .suggest_hashtag( key [, options] )
    // .suggest_hashtag( method [, options] )
    // .suggest_hashtag( suggestions )
    $.fn.suggest_hashtag = function(arg1) {
        var arg2 = arguments[1];

        var createSuggest_hashtag = function(el, suggestions) {
            var newData = {};
            $.each(suggestions, function(keyChar, options) {
                var key =  keyChar.toString().charAt(0);
                newData[key] = new Suggest_hashtag(el, key, typeof options == 'object' && options);
            });
        ...
        return this.each(function() {
            var that = this,
                $this = $(this),
                data = $this.data('suggest_hashtag'),
                suggestion = {};
            ...
            if (typeof arg1 == 'string') {
                if (arg1.length > 1 && data) {
                    ...
                    if (arg2) {
                        ...
                        if (!data) {
                            $this.data('suggest_hashtag', createSuggest_hashtag(this, suggestion));
                        } else if (data && !(arg1 in data)) {
                            $this.data('suggest_hashtag', $.extend(data, createSuggest_hashtag(this, suggestion)));
                        }
                    }
                }
            } else {
                // arg1 contains set of suggestions
                if (!data) $this.data('suggest_hashtag', createSuggest_hashtag(this, arg1));
                else if (data) {
                    ...
                    $this.data('suggest_hashtag', $.extend(data, createSuggest_hashtag(that, suggestion)))
                }
            }
        });
    };

    $.fn.suggest_hashtag.defaults = {
        data: [],
        map: undefined,
        filter: {
            casesensitive: false,
            limit: frmhashtagMaxCount
        },
        dropdownClass: '',
        position: 'caret',
        // events hook
    ...
    $.fn.suggest_hashtag.Constructor = Suggest_hashtag;

    $.fn.suggest_hashtag.noConflict = function () {
        $.fn.suggest_hashtag = old;
        return this;
    }
