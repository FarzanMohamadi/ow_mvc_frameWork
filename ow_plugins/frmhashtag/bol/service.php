...
class FRMHASHTAG_BOL_Service
{
    ...
    {
        $this->hashtagTagDao = FRMHASHTAG_BOL_TagDao::getInstance();
        $this->hashtagEntityDao = FRMHASHTAG_BOL_EntityDao::getInstance();
    }
    ...
    public function add_hashtag($hashtag, $entityId, $entityType,$context = null){
        ...
        if(!$this->hashtagEntityDao->itemExists($tag->id, $entityId, $entityType)) {
            $entity = new FRMHASHTAG_BOL_Entity();
            $entity->tagId = $tag->id;
    ...
    {
        $params = ['idList'=>json_encode($idList)];
        $valid = FRMSecurityProvider::sendUsingRabbitMQ($params, 'deleteEntitiesByListIds');
        if (!$valid) {
            $this->directDeleteEntitiesByListIds($idList);
        }
    }
     ...
     * @param $idList
     */
    private function directDeleteEntitiesByListIds($idList)
    {
        //delete tag entities
    ...
    public function findEntityCountByTag($hashtag){
        ...
        if(count($tags)>0) {
            $tagId = $tags[0]->id;
            $q = "SELECT `entityType`,`context`,count(*) FROM `" . OW_DB_PREFIX . "frmhashtag_entity` WHERE `tagId`="
    ...
    public function getContentMenu($tag, $selectedTab)
    {
        ...
        foreach ( $validLists as $key => $type )
        {
            ...
            $item->setLabel($language->text('frmhashtag', 'at').' '.$language->text('frmhashtag', 'menu_' . $type).' ('.$countArray[$type].')');
            $item->setUrl(OW::getRouter()->urlForRoute('frmhashtag.tag.tab', array('tag'=>$tag, 'tab' => $type)));
            $item->setKey($type);
     ...
     * @return Form
     */
    public function getSearchForm($selectedTab=""){
        $form = new Form("form");

        $textField = new TextField('txt');
        $textField->setLabel(OW::getLanguage()->text('frmhashtag', 'search'))
            ->setRequired(true);
        $textField->addAttribute('placeholder', OW::getLanguage()->text('frmhashtag', 'search_placeholder'));
        $form->addElement($textField);

        $submit = new Submit('submit');
        $submit->setValue(OW::getLanguage()->text('frmhashtag', 'search'));
        ...
        if ( OW::getRequest()->isPost() && $form->isValid($_POST) )
        {
            ...
            $tag = $data['txt'];
            if($selectedTab != ""){
                OW::getApplication()->redirect(OW::getRouter()->urlForRoute('frmhashtag.tag.tab', array('tag' => $tag , 'tab' => $selectedTab)));
            }else{
                OW::getApplication()->redirect(OW::getRouter()->urlForRoute('frmhashtag.tag', array('tag' => $tag)));
            }
        }
        return $form;
    }
    ...
    public function onAddComment( OW_Event $e )
    {
        ...
        $valid = FRMSecurityProvider::sendUsingRabbitMQ($params, 'onAddComment');
        if (!$valid) {
            $this->_onAddComment($params);
        }
    }
     ...
     * @param $params
     */
    private function _onAddComment($params){
    ...
    public function onEntityUpdate(OW_Event $e )
    {
        $params = $e->getParams();
        $params['event_name'] = $e->getName();
        $valid = FRMSecurityProvider::sendUsingRabbitMQ($params, 'refreshTotalHashTagCount');

        if (!$valid) {
            $this->refreshTotalHashTagCount($params);
        }
    }
    ...
    public function onRabbitMQNotificationRelease(OW_EVENT $event) {
        ...
        if (isset($params['itemType'])){
            ...
            if($params['itemType'] == 'refreshTotalHashTagCount') {
                $this->refreshTotalHashTagCount($params);
            }elseif($params['itemType'] == 'onAddComment') {
                $this->_onAddComment($params);
            }elseif($params['itemType'] == 'deleteEntitiesByListIds'){
                $this->directDeleteEntitiesByListIds(json_decode($params['idList'],true));
            }
        }
    }
     ...
     * @param $params
     */
    public function refreshTotalHashTagCount($params)
    {
        $entityId = $params['entityId'];
        $entityType = $params['entityType'];
    ...
    private function findAndReplaceTagsFromView($content){
        ...
        $replace1 = preg_replace_callback('/'.$this->regex_view.'(?=[^>]*(<|$))/u', function($matches) {
            ...
            $url = OW::getRouter()->urlForRoute('frmhashtag.tag', array('tag'=>$matches1));
            //Proper rendering for English/Persian hashtags
            if(preg_match("/^\w*$/",$matches1)) {
                return $matches[3] . '<a class="frmhashtag_tag english_tag" href="' . $url . '">#' . $matches1 . '</a>';
            }else{
                return $matches[3] . '<a class="frmhashtag_tag persian_tag" href="' . $url . '">#&#8235;' . $matches1 . '</a>';
            }
        },  $content);
        return $replace1;
    }
    ...
    public function checkNewsfeedItemsForDisplay($idList){
        ...
        foreach ($ids as $key=>$item) {
            $actions = array_merge($actions,FRMHASHTAG_BOL_Service::getInstance()->findActionsByEntityIds($key, $item));
        }
        $actionIdList = array();
        $existingEntityIds = array();
    ...
    {
        return OW::getPluginManager()->getPlugin('frmhashtag')->getStaticUrl() . 'img/hashtag_default_image.svg';
    }
}
