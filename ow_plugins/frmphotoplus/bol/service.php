 ...
 * @since 1.0
 */
class FRMPHOTOPLUS_BOL_Service
{
    ...
    {
        $this->statusPhotoDao = FRMPHOTOPLUS_BOL_StatusPhotoDao::getInstance();
    }

    public function setTtileHeaderListItemPHOTO( OW_Event $event )
    {
        $params = $event->getParams();
        if (isset($params['listType']) && $params['listType'] == FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS) {
            OW::getDocument()->setTitle(OW::getLanguage()->text('frmphotoplus', 'meta_title_photo_add_friends'));
            OW::getDocument()->setDescription(OW::getLanguage()->text('frmphotoplus', 'meta_description_photo_friends'));
        }
    }
    public function getValidListForPhoto( OW_Event $event )
    {
        ...
        if(isset($params['validLists'])){
            ...
            if(OW::getUser()->isAuthenticated()) {
                $validLists[] = FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS;
            }
    ...
    public function addListTypeToPhoto( OW_Event $event )
    {
        ...
        if(isset($params['menu']) && OW::getUser()->isAuthenticated()){
            $menu = $params['menu'];
            if(isset($menu->sortItems[FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS]) && isset($menu->sortItems[FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS]['isActive']) && $menu->sortItems[FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS]['isActive']==true) {
        ...
        && isset($params['uniqId'])){
            ...
            $menuItems['photo_friends'] = array(
                    'label' => OW::getLanguage()->text('frmphotoplus', FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS),
            ...
            $item->setLabel(OW::getLanguage()->text('frmphotoplus', FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS));
            $item->setUrl(OW::getRouter()->urlForRoute('view_photo_list', array('listType' => FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS)));
            $item->setKey(FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS);
            $item->setIconClass('ow_ic_friends_photos ow_dynamic_color_icon');
    ...
    public function getResultForListItemPhoto( OW_Event $event )
    {
        ...
        if(isset($params['listtype']) &&
            $params['listtype'] == FRMPHOTOPLUS_BOL_Service::$PHOTO_FRIENDS){
            $exclude = array();
