 ...
 * @since 1.0
 */
class FRMMOBILESUPPORT_BOL_WebServicePhoto
{
    ...
    public function getUserAlbumsByUserId($userId){
        ...
        if(!OW::getUser()->isAuthenticated() || OW::getUser()->getId() != $userId){
            $canView = PHOTO_BOL_PhotoService::getInstance()->checkUserPhotosPrivacy($userId);
            if(!$canView){
                return array();
            }
        }
    ...
    public function getPhoto(){
        ...
        if(!OW::getUser()->isAuthenticated() || OW::getUser()->getId() != $album->userId){
            $canView = PHOTO_BOL_PhotoService::getInstance()->checkUserPhotosPrivacy($album->userId);
            if(!$canView){
                return array('valid' => false, 'message' => 'authorization_error');
            }
        }
        ...
        $comments = FRMMOBILESUPPORT_BOL_WebServiceComment::getInstance()->getCommentsInformation('photo_comments', $photo->id, $page);
    ...
    public function getAlbumPhotos(){
        ...
        $albumData = array(
            'name' => $album->name,
            'userId' => $album->userId,
            "user" => FRMMOBILESUPPORT_BOL_WebServiceUser::getInstance()->getUserInformationById($album->userId),
            'createDatetime' => $album->createDatetime,
            'description' => $album->description,
            'id' => $album->id,
        );
    ...
    public function canUserCreatePhoto(){
        $pluginActive = FRMSecurityProvider::checkPluginActive('photo', true);
    ...
    public function createAlbum(){
        ...
        if(isset($_POST['title'])){
            $albumName = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->stripString($_POST['title'], true);
        }
    ...
    public function createPhoto(){
        ...
        if (isset($_POST['privacy'])) {
            ...
            if(!in_array($privacy, array('everybody', 'friends_only', 'only_for_me'))) {
                $privacy = '';
            }
        }
        $photo = $tmpPhotoService->moveTemporaryPhoto($tmpId, $album->id, $description, null, 0, null, null, $privacy);
        if ( $photo )
        {
            if ($album->userId)
            {
    ...
    public function getPhotoInformation($photo){
        $description = '';
        if(!empty($photo['description'])){
            $description = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->stripString($photo['description'], true);
        ...
        $rateInfo = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->getRateInfo($photo['id'], 'photo_rates');
        $canRate = BOL_RateService::getInstance()->canUserRate($photo['id'], 'photo_rates');
    ...
    public function getAlbumInformation($album){
        ...
        if(!empty($album['description'])){
            $description = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->stripString($album['description'], true);
        }
        ...
        $title = '';
        if(!empty($album['name'])){
            $title = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->stripString($album['name'], true);
        }

        $privacy = FRMSECURITYESSENTIALS_BOL_Service::getInstance()->getPrivacyOfAlbum($album['id']);
        if($privacy == null){
            $eventParams = array(
    ...
    public function canUserSeePhotoOfUserId($viewerId, $albumId, $album = null){
        ...
        if(FRMSecurityProvider::checkPluginActive('frmsecurityessentials', true)) {
            $photoPrivacy = FRMSECURITYESSENTIALS_BOL_Service::getInstance()->getPrivacyOfAlbum($albumId);
        }
