 ...
 * @since 1.0
 */
class FRMMOBILESUPPORT_BOL_WebServiceEvent
{
    ...
    public function leave(){
        ...
        $pluginActive = FRMSecurityProvider::checkPluginActive('frmeventplus', true);
        if(!$pluginActive){
            return array('valid' => false, 'message' => 'plugin_not_found');
        }
        ...
        if($userInvitedBefore != null) {
            if($event->userId == $userId){
                ...
                FRMEVENTPLUS_BOL_Service::getInstance()->leaveEvent($eventId, $userId);
            }

            return array('valid' => true, 'deleted' => $deleted, 'event' => $this->populateEventData($event, false, array('comments')));
        }
        return array('valid' => false, 'message' => 'authorization_error');
    }
    ...
    public function getEventsByUserId($userId = null){
        ...
        if($userId != null){
            $checkPrivacy = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->checkPrivacyAction($userId, 'event_view_attend_events', 'event');
            if(!$checkPrivacy){
                return array();
            }
        }
    ...
    private function populateEventData($event, $loadEvents = true, $params = array()){
        ...
        if(FRMSecurityProvider::checkPluginActive('frmeventplus', true)){
            $categoryId = FRMEVENTPLUS_BOL_Service::getInstance()->getEventCategoryByEventId($event->id);
            if($categoryId!= null){
                $category = FRMEVENTPLUS_BOL_Service::getInstance()->getCategoryById($categoryId);
                if($category != null){
            ...
            $filesList = FRMEVENTPLUS_BOL_Service::getInstance()->findFileList($event->id, $first, $count);
            $filesInformation = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->preparedFileList($event, $filesList);
        }
        ...
        if (in_array('comments', $params)){
            $comments = FRMMOBILESUPPORT_BOL_WebServiceComment::getInstance()->getCommentsInformation('event', $event->id);
        }

        $description = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->stripString($event->description, false);
        ...
        foreach ( $usersObject as $userObject )
        {
            ...
            $data = FRMMOBILESUPPORT_BOL_WebServiceUser::getInstance()->populateUserData($userObject, $avatarUrl, $username);
            $data['status'] = $usersStatus[$userObject->id];
            $users[] = $data;
        }

        $startTime = array('hour' => date('G', $event->getStartTimeStamp()), 'minute' => date('i', $event->getStartTimeStamp()), 'disabled' => (bool) $event->startTimeDisabled);
        ...
        {
            $endTime = array('hour' => date('G', $event->getEndTimeStamp()), 'minute' => date('i', $event->getEndTimeStamp()), 'disabled' => (bool) $event->endTimeDisabled);
        ...
        if (OW::getConfig()->getValue('event', 'showEventCreator')) {
            $userInfo = FRMMOBILESUPPORT_BOL_WebServiceUser::getInstance()->getUserInformationById($event->userId);
        }
        ...
        $result =  array(
            ...
            "end_date_flag" => (bool) $event->endDateFlag,
            "registered" => $registered,
            "registrable" => $registrable,
            "end_time" => $endTime,
            "end_date" => $endDate,
            "imageUrl" => $imageUrl,
            'imageInfo' => BOL_AvatarService::getInstance()->getAvatarInfo((int) $event->id, $imageUrl, 'event'),
    ...
    public function getEventFields(){
        ...
        if(FRMSecurityProvider::checkPluginActive('frmeventplus', true)) {
            $categories = FRMEVENTPLUS_BOL_Service::getInstance()->getEventCategoryList();
            if(sizeof($categories) > 0) {
                $values = array();
                $values[null] = OW::getLanguage()->text('frmeventplus', 'select_category');
                ...
                $fields[] = array(
                    'name' => 'categoryStatus',
                    'type' => 'select',
                    'label' => $language->text('frmeventplus', 'select_category'),
                    'presentation' => 'radio',
                    'values' => $values,
                    'required' => false
                );
    ...
    public function addFile(){
        ...
        $resultArr = FRMEVENTPLUS_BOL_Service::getInstance()->manageAddFile($eventId, $_FILES['file']);
        if(!isset($resultArr) || !$resultArr['result']){
            return array('valid' => false, 'message' => 'authorization_error');
        }

        $filesList = FRMEVENTPLUS_BOL_Service::getInstance()->findFileList($eventId, 0, 1);
        $filesInformation = FRMMOBILESUPPORT_BOL_WebServiceGeneral::getInstance()->preparedFileList($event, $filesList);
    ...
    public function deleteFile(){
        ...
        try {
            FRMEVENTPLUS_BOL_Service::getInstance()->deleteFileForEvent($eventId, $attachmentId);
