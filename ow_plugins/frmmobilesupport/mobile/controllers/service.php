<?php
class FRMMOBILESUPPORT_MCTRL_Service extends OW_MobileActionController
{
    ...
    public function index($params)
    {
        if (isset($params['key'])) {
            if ($params['key'] == 'information') {
                ...
                $information['userInformation'] = FRMMOBILESUPPORT_BOL_WebServiceUser::getInstance()->getUserInformation(false, true);
                $versionCode = null;
                if(isset($_GET['versionCode'])){
                    $versionCode  = $_GET['versionCode'];
                }
                $service = FRMMOBILESUPPORT_BOL_Service::getInstance();
                $information['androidInformation'] = $service->getAppInformation(FRMMOBILESUPPORT_BOL_Service::getInstance()->AndroidKey, $versionCode);
                $information['iosInformation'] = $service->getAppInformation(FRMMOBILESUPPORT_BOL_Service::getInstance()->iOSKey, $versionCode);
                OW::getEventManager()->trigger(new OW_Event('frmmobilesupport.save.login.cookie'));
    ...
    private function findActiveLanguages()
    {
    ...
    private function parseCountryFromTag($tag)
    {
    ...
    {
        if(!FRMMOBILESUPPORT_BOL_Service::getInstance()->isUserShoouldUseOnlyMobile()){
