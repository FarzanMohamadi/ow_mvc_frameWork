    ...
    initialize: function(){
        ...
        this.model.on('change:conversationRead', function(){
            if (this.model.get('conversationRead')){
                ...
                $('#contactItem'+this.model.attributes.opponentId).find('.ow_count_wrap').fadeOut(300)
            }
            else{
...
//                this.model.set('show', false);
        ...
        OW.bind('mailbox.message', function(message){
            ...
            if (self.$el && self.$el.attr('id') && $('#' + self.$el.attr('id')).length > 0) {
                $('#'+self.$el.attr('id')).remove();
            }
            self.conversationItemListContainer.find('#conversationItemListSub').prepend(self.$el);
            if(parseInt(message.new_count) > 0 && !self.$el.hasClass('ow_mailbox_convers_info_selected') && $('#main_tab_contact_'+message.senderId).length==0){
                self.$el.find('.ow_mailbox_convers_count_new').html(message.new_count);
                self.$el.find('.ow_mailbox_convers_count_new').show();
            }
    ...
    setClickListeners: function(){
        ...
        this.$el.bind('click', function(e){
            ...
            if (!OW.Mailbox.conversationController.someConversationLoading) {
                ...
                if (document.getElementById("foundConversation").style.display != "none" || document.getElementById("nocontentConversation").style.display != "none"){
                    $('#nocontentConversation').hide();
                    $('#foundConversation').hide();
                    $('#conversationContainer').show();
                    document.getElementById('conversation_search').value = "";
                }
    ...
    render: function(){
        ...
        if (this.model.get('newMessageCount')>0){
            this.$el.find('.ow_mailbox_convers_count_new').html( this.model.get('newMessageCount') );
        }
        else{
...
function ChangeMessageSearch() {

    var search_state= document.getElementById("Message_search").options[document.getElementById("Message_search").selectedIndex].value;
    if(search_state == "content_search_select"){
        $("#contacts_search").hide();
        $("#contacts_search_close_btn_search").hide();
        $("#conversation_search").show();
    }
    else if (search_state == "contacts_search_select"){
        $("#conversation_search").hide();
        $("#conversation_search_close_btn_search").hide();
        $("#contacts_search").show();
    }
}
function CheckOptionsChecked(labelcheckbox){
    var checkbox=labelcheckbox.previousElementSibling;
    if (document.getElementById(checkbox.id).checked) {
        $('#'+ labelcheckbox.id +'.ow_options_checked').removeClass("ow_label_options_checked");
    } else {
        $('#'+ labelcheckbox.id +'.ow_options_checked').addClass("ow_label_options_checked");
    }
}
function CheckOptionsSelectAll(labelcheckbox){
    var checkbox=labelcheckbox.previousElementSibling;
    if (document.getElementById(checkbox.id).checked) {
        $('#conversationItemListSub .ow_options_checked').removeClass("ow_label_options_checked");
    } else {
        $('#conversationItemListSub .ow_options_checked').addClass("ow_label_options_checked");
    }
}
...
//            if (self.model.get('loadedConvCount') < OW.Mailbox.conversationsCollection.length && !self.listLoadInProgress && isAtBottom){
        ...
        $('#closeBulkOptionsBtn').click(function(e){
            ...
            $('#conversationItemListSub .ow_mailbox_convers_info.clearfix.chats label').removeClass("ow_options_checked");
            $('#conversationItemListSub .ow_mailbox_avatar .ow_avatar').removeClass("ow_avatar_options_checked");
            $('#conversationItemListSub .ow_mailbox_convers_info_body').removeClass("ow_info_body_checked");
            $('#MailboxConvTxt').hide();
            return false;
        });

        $('#MailboxConvOptionsButton').click(function(){
            self.preloaderControl.addClass('ow_mailbox_bulk_options');
            $('#conversationItemListSub .ow_mailbox_convers_info.clearfix.chats label').addClass("ow_options_checked");
            $('#conversationItemListSub .ow_mailbox_avatar .ow_avatar').addClass("ow_avatar_options_checked");
            $('#conversationItemListSub .ow_mailbox_convers_info_body').addClass("ow_info_body_checked");
            $('#MailboxConvTxt').show();
        });

        $('#mailboxConvOptionSelectAll').click(function(){
            if ($(this).prop('checked')) {
                $('.ow_mailbox_conv_option').prop('checked', true);
                $('.ow_mailbox_convers_info.clearfix.chats label').addClass("ow_options_checked");
                $('.ow_mailbox_conv_options_checklab').addClass("ow_options_checked");
            }
            else {
                $('.ow_mailbox_conv_option').prop('checked', false);
                $('.ow_mailbox_convers_info.clearfix.chats label').removeClass("ow_options_checked");
                $('.ow_mailbox_conv_options_checklab').removeClass("ow_options_checked")
            }
        });
        ...
        $('#mailboxConvActionDelete').click(function(){
            ...
            _.each(list, function (checkbox) {
                convIdList.push( $(checkbox).attr('id').replace('conversation_', '') );
                opponentIdList.push( $(checkbox).parent('.ow_mailbox_convers_info').attr('id').replace('chatItem', ''));
            });

            self.bulkAction(opponentIdList, convIdList, 'delete');
        });
    ...
    bulkAction: function(opponentIdList, convIdList, actionName){
            ...
            success: function(data){
                if ( typeof data != 'undefined' )
                {
                    ...
                    if (actionName == 'delete'){
                        let index = 0
                        _.each(convIdList, function (id) {
                            let opponentId = opponentIdList[index];
                            OW.trigger('mailbox.conversation_deleted', {convId: parseInt(id), opponentId:parseInt(opponentId)});
                            index++;
                        });
                    }
    ...
    createItem: function(conversation){
        ...
        if (this.conversationItemListContainer.find('#conversationItemListSub').length > 0){
            var addConversation = true;
            if (conversationItemView.render().$el && conversationItemView.render().$el && conversationItemView.render().$el[0].id && $('#' + conversationItemView.render().$el[0].id).length > 0) {
...
//            }
            ...
            if (!self.syncing){
                ...
                setTimeout(function(){
                    ...
                    $('#contacts_search_close_btn_search').css('display','block');
...
MAILBOX_ConversationView = function () {
    ...
    OW.bind('mailbox.conversation_deleted', function(data) {
        if($('#chatItem'+data.opponentId).hasClass('ow_mailbox_convers_info_selected') || $('.ow_mailbox_convers_info_selected').length==0){
            self.hideConversationData();
        }
    });
    ...
    this.setConversationId = function(params){
        ...
        ajaxData['actionCallbacks'] = {
            success: function(data){
                ...
                if ( typeof data != 'undefined' )
                {
                    if (typeof self.conversation === 'undefined') {
                        self.conversation = new MAILBOX_Conversation(data);
                        $('#conversationCapBlock').show()
                    }
                    ...
                    let user_id = data.opponentId;
                    let avatar_info = get_avatar_info(avatar_url, user_id);
                    if (avatar_info['is_empty']){
                        $('#conversationOpponentAvatar').css('background-color', avatar_info['color']);
                    }
    ...
    this.deleteBtn.bind('click', function(){
        let jc = $.confirm(OW.getLanguageText('mailbox', 'confirm_conversation_delete'));
    ...
    OW.bind('mailbox.message', function(message){
        ...
        if(typeof socket == "undefined" || socket.readyState !== socket.OPEN || (message && message.senderId != OWMailbox.userDetails.userId) || message.attachments.length != 0) {
            addMessage = true;
        }
        if (addMessage) {
...
MAILBOX_ConversationView.prototype = {
    ...
    sendMessage: function(text, timeStamp){
        ...
        $("#messageItem"+tmpMessage.id+" .ow_dialog_item").addClass("pending_message");
        var storage = OWMailbox.getStorage();
    ...
    updateChatMessage: function(message){
        ...
        messageContainer.find('.ow_dialog_item').removeClass("pending_message");
        var html = '';
        if (message.isSystem)
        {
            html = message.text;
        ...
        else
        {
            if (message.attachments.length != 0)
            {
                ...
                if (message.attachments[i]['type'] == 'image')
                {
                    ...
                    if (message.attachments.length < 0){
                        var ajax_notification =  $('.ow_dialog_picture_item').clone();
                    }
                }
                else
                {
        ...
        if (message.isSystem){
            ...
            if (message.attachments.length != 0){
                ...
                if (message.attachments[i]['type'] == 'image'){
                    ...
                    messageContainer.addClass('ow_dialog_picture_item');
                    $('#dialogMessageText', messageContainer).html( '<div class="message_img"><a href="javascript://"><img src="'+message.attachments[i]['downloadUrl']+'" /></a></div>'
                        + "<div style='margin: 5px 0'>" + caption + create_time_span(message.timeLabel) + "</div>");
                }
                else{
                    $('.ow_dialog_in_item', messageContainer).addClass('fileattach');
...
loadFeatures = function (message, messageContainer,self) {
    ...
    let avatar_info = get_avatar_info(message.senderAvatar, message.senderId);
    if (avatar_info['is_empty']){
...
function RedirectToConversation(conversationId,opponentId){
    if (document.getElementById("foundConversation").style.display != "none" || document.getElementById("nocontentConversation").style.display != "none"){
        $('#nocontentConversation').hide();
        $('#foundConversation').hide();
        $('#conversationContainer').show();
    ...
    $('#conversation_search').addClass('chat_empty_search_box');
}