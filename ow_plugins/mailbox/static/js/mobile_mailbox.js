...
MAILBOX_MessageView = Backbone.View.extend({
    ...
    render: function(){
        ...
        let avatar_url = self.model.get('senderAvatar');
        let user_id = self.model.get('senderId');
        let avatar_info = get_avatar_info(avatar_url, user_id);
        if (avatar_info['is_empty']){
            $('img',avatar).css('background-color', avatar_info['color']);
        }
...
MAILBOX_ConversationsView = Backbone.View.extend({
    ...
    renderItem: function(item){
        $(".no_user_in_conversation_found").hide();
    ...
    initialize: function(){
        ...
        if (this.model.get('mode') == 'chat'){
                ...
                if ( window.FileReader ) {
                    ...
                    for(i;i<self2.files.length;i++) {
                        ...
                        reader.onload = function (e) {
                            ...
                            ahref.title = OW.getLanguageText('mailbox', 'attache_file_delete_button');
            ...
            $('body').on('click', 'a.remove-chat-file-attachment', function() {
                ...
                var currentFileList = Array.from($("#newmessage-att-file")[0].files);
                ...
                $("#newmessage-att-file")[0].files = updatedFileList;
                if($("#newmessage-att-file")[0].files.length<=1){
        ...
        this.model.on('message_sent', function(data){
            if ($(formElement.input).val().charCodeAt(0) == 10){
                let newValue = $(formElement.input).val().substring(1);
                $(formElement.input).val(newValue);
            }
            $('html, body').animate({scrollTop:$(document).height()}, 'slow');
        });
