...
OWMailbox.Application = function(params){
    ...
    this.pingInterval = params.pingInterval || 30000;
    ...
    if (typeof socket !== 'undefined') {
        ...
        OW.getPing().getCommand('mailbox_ping').start(this.pingInterval);
    }
        ...
        if(data.data != undefined) {
            ...
            switch (data.data.type) {
                case "new_message":
                    ...
                    if (data.data.conversation_info && data.data.opponentId) {
                        const convInfo = data.data.conversation_info;
                        const recipientId = data.data.opponentId;
                        messageData.new_count = convInfo.new_count;
                        messageData.tmpMessageUid = data.data.tmpMessageUid;
                        if ($('.messages_index').length == 0 && ($('#main_tab_contact_' + recipientId).length == 0 || $('.ow_chat_dialog.ow_chat_dialog_active.ow_active').length == 0)) {
                            OW.Mailbox.contactManagerView.maximize();
                            OW.Mailbox.contactManagerView.model.set('chatBlockActive', '1');
                            OW.Mailbox.contactManagerView.updateList('');
                        }
                    }
                    result['stack'][0]['result'] = {'messageList': [messageData]};
                    checked = true;
                    break;
                case "edited_message":
                    result['stack'][0]['result'] =
                        {
                            'changes': [{
                                "id": data.data.editedMessageId,
                                "text": data.data.editedMessageText
                            }]
                        };
                    checked = true;
                    break;
                ...
                case "mark_message":
                    ...
                    result['stack'][0]['result'] =
                        {
                            'opponentUnreadMessageData': {},
                            'opponentLastDataId': {}
                        };
                    result['stack'][0]['result']['opponentUnreadMessageData'][recipientId] = [];
                    result['stack'][0]['result']['opponentLastDataId'][recipientId] = data.data.lastMessageId;
                    checked = true;
                    break;
            }
            if(checked){
    ...
    $.fn.extend({
        ...
        dialogAutosize: function(options, action){
            ...
            if (!action){
                ...
                $('#MessagesPageSendBtn').click(function(ev) {
                    ...
                    $('#dialogTextarea').attr('rows', 1);
                    $('#dialogTextarea').css('height', options.textareaHeight);
        ...
        if(opponentId == -1 || opponentId == recipientId) {
            ...
            for (var i = 0; i < allMessages.length; i++) {
                ...
                if (selfMessages && selfMessages.length > 0) {
                    ...
                    if (jQuery.inArray(id, item) == -1 && (opponentLastDataId[recipientId] != -1 &&  parseInt(id) <= parseInt(opponentLastDataId[recipientId]))) {
...
function add_mailbox_search_content_events(ajax_url) {
    ...
    $('#conversation_search').on('change keydown paste input', function () {
        ...
        $('#conversation_search_close_btn_search').css('display','block');
                        ...
                        if (data.result === 'ok') {
                            ...
                            if(data.results.length === 0){
                                ...
                                $.each(data.results, function (index, value) {
                                    let avatar_url = value.avatarUrl;
                                    let user_id = value.opponentId;
                                    let avatar_info = get_avatar_info(avatar_url, user_id);

                                    html += '<div class="clearfix message message_seen" id="\'messageItem'+value.id+'\'"  style="border-top: 1px solid rgb(238, 238, 238);"  onclick="RedirectToConversation(\''+value.conversationId+'\',\'' + value.opponentId + '\');">' +
                                        '<div class="clearfix" >' +
                                        ...
                                        '<img width="32px" height="32px" id="contactItemAvatarUrl"  src="' + value.avatarUrl + '" style=" background-color: ' + avatar_info['color'] + '">' +
                                        '</span>' +
