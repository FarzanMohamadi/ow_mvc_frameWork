...
class MAILBOX_BOL_MessageDao extends OW_BaseDao
{
    ...
    public function deleteMessagesByIdList(array $idList)
    {
        return parent::deleteByIdList($idList); // TODO: Change the autogenerated stub
    }
    ...
    {
        $example = new OW_Example();
        $example->andFieldEqual('id', (int) $messageId);
     ...
     * @return array|string
     */
    public function findMessagesByIdList( $conversationId,  $idList )
    {
        $example = new OW_Example();
        $example->andFieldEqual('conversationId', (int) $conversationId);
        $example->andFieldInArray('id', $idList);
        return $this->findListByExample($example);
    }
    ...
    public function findMessagesAfterMessageId( $conversationId, $afterMessageId, $deletedTimestamp = 0 )
    {
        $sql = "SELECT * FROM `{$this->getTableName()}` WHERE `conversationId` = :conversationId AND `id` >= :afterMessageId AND `timeStamp` > :deletedTimestamp ORDER BY `id` DESC";

        $result = $this->dbo->queryForObjectList($sql, $this->getDtoClassName(), array('conversationId' => $conversationId, 'afterMessageId'=>$afterMessageId, 'deletedTimestamp'=>$deletedTimestamp));
        foreach($result as $message)
        {
            $message->text = MAILBOX_BOL_ConversationService::getInstance()->json_decode_text($message->text);
        }
    ...
    public function findLastOpponentReadMessageByConversationIdListAndUserIdList($userId, $userIdList )
    {
        $sql = "SELECT max(`id`) as `id`,`conversationId`, max(`timeStamp`), `recipientId`, `senderId`  FROM ". $this->getTableName()."  
          WHERE `recipientId` IN ( ".$this->dbo->mergeInClause($userIdList)." ) AND `senderId` = :userId and `recipientRead` = 1 GROUP BY `conversationId`, `recipientId`, `senderId`";
    ...
    public function findUnreadMessages( $userId, $ignoreList, $timeStamp = null, $activeModeList = array())
    {
        ...
        if (is_array($activeModeList) && $activeModeList != null) {
    ...
    public function findUnreadMessagesForConversation( $convId, $userId, $deletedTimestamp = 0)
    {
        $example = new OW_Example();
        $example->andFieldEqual('conversationId', $convId);
        $example->andFieldEqual('recipientId', $userId);
        $example->andFieldEqual('recipientRead', 0);
        $example->andFieldGreaterThan('timeStamp', $deletedTimestamp);
    ...
    public function findUnreadMessagesForConversations( $convIds, $userId )
    {
        if (!is_array($convIds) || empty($convIds)) {
            return array();
        }
        $example = new OW_Example();
        $example->andFieldInArray('conversationId', $convIds);
        $example->andFieldEqual('recipientId', $userId);
        $example->andFieldEqual('recipientRead', 0);
        ...
        {
            $message->text = MAILBOX_BOL_ConversationService::getInstance()->json_decode_text($message->text);
            $data[$message->conversationId][] = $message;
        }
        foreach ($convIds as $convId) {
            if (!isset($data[$convId])) {
                $data[$convId] = array();
            }
        }
        return $data;
    ...
    public function countUnreadMessagesForConversationByIds( $convIds, $userId )
    {
        ...
        $sql = "SELECT COUNT(*) as cnt, conversationId FROM `{$this->getTableName()}` as `m`
        WHERE `m`.`conversationId` in (" . $this->dbo->mergeInClause($convIds) . ") AND `m`.`recipientId` = :userId AND `m`.`recipientRead` = 0 group by conversationId";

        $res = $this->dbo->queryForList($sql, array('userId' => $userId));

        $data = array();
        foreach ($res as $item) {
            $data[$item['conversationId']] = $item['cnt'];
        }
