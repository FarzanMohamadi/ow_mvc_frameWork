...
//        }
        ...
        switch($mode)
        {
            case 'chat':
                ...
                $params['text'] = str_ireplace($breaks, "\n", $params['text']);
                $params['text'] = UTIL_HtmlTag::stripTagsAndJs($params['text']);
                $params['text'] = nl2br($params['text']);
                $params['text'] = str_replace( "\r\n", "", $params['text'] );
                $params['text'] = $stripped = preg_replace('/^(<br\s*\/?>)*|(<br\s*\/?>)*$/i', '', $params['text']);
                break;
        ...
        $replyId = null;
        if (isset($params['replyToId'])) {
            $replyId = $params['replyToId'];
            if(!$conversationService->checkIfReplyIdValid($conversationId,$userId))
            {
        ...
        try
        {
            $message = $conversationService->createMessage($conversation, $userId, $text,  $replyId, false, $tmpMessageUid);
    ...
    public function getLog($params, $additionalParams = array())
    {
        $conversationService = MAILBOX_BOL_ConversationService::getInstance();
        ...
        {
            $conversationService->markRead(array($conversationId), $userId, null, $additionalParams);
        ...
        return $conversationService->getConversationDataAndLog($conversationId, 0, 16, $additionalParams);
    }
    ...
    public function onSearch( BASE_CLASS_EventCollector $event )
    {
        ...
        $data = $this->buildData(array_unique($userIds), array($userId));
    ...
    public function onSearchForApi( BASE_CLASS_EventCollector $event )
    {
        ...
        $data = $this->buildDataForApi(array_unique($userIds), array($userId));
    ...
    public function onConversationSearch( BASE_CLASS_EventCollector $event )
    {
        ...
        $data = $this->buildConversationData($conversations, array($userId));
    ...
    protected function buildData( $userIds, $ignoreUserIds = array() )
    {
        if ( empty($userIds) )
        {
            return array();
    ...
    protected function buildDataForApi( $userIds, $ignoreUserIds = array() )
    {
        if ( empty($userIds) )
        {
            return array();
    ...
    protected function buildConversationData( $conversations, $ignoreConvIds = array() )
    {
