...
class UTIL_DateTime
{
    ...
    const DATETIME_SEPRATED_BY_DASH_DATE_FORMAT = 'Y-m-d h:m:s'; // "1998-03-23 12:03:00"
    ...
    {
        $simpleDateFormat =  OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_RENDER_FORMAT_DATE_FIELD, array('timeStamp' => $timeStamp, 'onlyDate' => $onlyDate)));
    ...
    public  static function gregorianFormatSimpleDate( $timeStamp, $onlyDate = false )
    {
        ...
        $militaryHour = strftime("%H:%M", $timeStamp);
        $hour = strftime(" %I:%M%p", $timeStamp);
        ...
        {
            return (date('Y', $timeStamp) === date('Y', time())) ?
                ($militaryTime ? $day.' '.$month .' ' .$militaryHour: $day.' '.$month .' ' .$hour) :
                ($militaryTime ? $day.' '.$month .' ' .$year . ' ' .$militaryHour : $day.' '.$month .' ' .$year . ' ' .$hour);
        }
        else{
            return (date('Y', $timeStamp) === date('Y', time())) ?
                ($militaryTime ? $month .' ' .$day.' '.$militaryHour: $month .' ' .$day.' '.$hour) :
                ($militaryTime ? $month .' ' .$day.' '.$year . ' ' .$militaryHour : $month .' ' .$day.' ' .$year . ' ' .$hour);
        }
    }
    ...
    public static function formatDate( $timeStamp, $onlyDate = null )
    {
        ...
        )
        {
            ...
            $comma = ',';
            if(OW::getLanguage()->valueExist('base', 'comma')){
                $comma = $language->text('base', 'comma');
            }

            return $language->text('base', 'date_time_yesterday') . $comma . ' ' . ( $militaryTime ? strftime("%H:%M", $timeStamp) : strftime("%I:%M%p", $timeStamp) );
        }
...
//               return $dateTime;
    ...
    { $simpleDateFormat =  OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_RENDER_FORMAT_DATE_FIELD, array('year' => $year, 'month' => $month , 'day' => $day , 'isBirthday' => true)));
        if($simpleDateFormat->getData() && isset($simpleDateFormat->getData()['jalaliSimpleFormat'])){
            return $simpleDateFormat->getData()['jalaliSimpleFormat'];
        }
    ...
    public static function getTimezones()
    {
        $zones = array(
            ...
            "Asia/Ujung_Pandang", "Asia/Tokyo", "Asia/Thimphu", "Asia/Thimbu",
            ...
            "Asia/Kabul", "Asia/Jayapura", "Asia/Jakarta", "Asia/Istanbul",
