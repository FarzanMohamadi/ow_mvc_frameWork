...
class OEmbedDefaultProvider extends OEmbedProvider
{
    ...
    {
        $content = @UTIL_HttpResource::getContents($url, 5);
        ...
        foreach ( $matches[1] as $img )
        {
            ...
            $checkImg = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_URL_IMAGE_ADD_ON_CHECK_LINK, array('img' => $img)));
            if(isset($checkImg->getData()['wrong'])){
                $wrongExtension = $checkImg->getData()['wrong'];
            }
            if(!$wrongExtension) {
                $images[] = $img;
            }
