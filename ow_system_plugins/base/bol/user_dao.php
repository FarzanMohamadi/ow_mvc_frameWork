...
class BOL_UserDao extends OW_BaseDao
{
    ...
    {
        $ex = new OW_Example();
        $ex->andFieldEqual('joinIp', $joinIp);

        return $this->findObjectByExample($ex);
    }

    public function findByUsernamesStartsWith($value)
    {
        $example = new OW_Example();
        $example->andFieldLike('username',$value.'%');
        $example->setLimitClause(0,5);
        $result = $this->findListByExample($example);
        return $result;
    }

    public function findIdsByUserNames( $usernameList )
    {
        if (empty($usernameList)) {
            return array();
        }
        $ex = new OW_Example();
        $ex->andFieldInArray('username', $usernameList);
    ...
    public function findOnlineList( $first, $count )
    {
        ...
        $query = "SELECT `u`.* FROM `{$this->getTableName()}` AS `u`
            ...
            INNER JOIN `" . BOL_UserOnlineDao::getInstance()->getTableName() . "` AS `o`
                ON(`u`.`id` = `o`.`userId`)
            WHERE `o`.`activityStamp` >= ? AND {$queryParts["where"]}
        ...
        return $this->dbo->queryForObjectList($query, $this->getDtoClassName(), array($expireTimestamp, $first, $count));
    }
    ...
    public function countOnline()
    {
        ...
        $query = "SELECT  COUNT(*) FROM `{$this->getTableName()}` AS `u`
            ...
            INNER JOIN `" . BOL_UserOnlineDao::getInstance()->getTableName() . "` AS `o`
                ON(`u`.`id` = `o`.`userId`)
            WHERE o.activityStamp >= :expireTimestamp AND {$queryParts["where"]}";
    ...
    public function findUserIdsByAccountType($type){
        return $this->dbo->queryForColumnList("SELECT `id` from `{$this->getTableName()}` WHERE  `accountType`=?",
            array($type));
    }
    ...
    public function findUserIdsByRoleId( $roleId, $first, $count )
    {
        ...
        return $this->dbo->queryForColumnList($query, array($roleId, $first, $count));
    }
     ...
     * @return mixed
     */
    public function countUsersByQuestionValues( $questionValues, $isAdmin = false, $aditionalParams = array(), $queryParams = array() )
    {
        ...
        foreach ( $questions as $question )
        {
            if ( !empty($questionValues[$question->name]) && $question->name != 'password' )
            {
                if ( $question->base == 1 )
                {
                    $where .= ' AND `user`.`' . $this->dbo->escapeValue($question->name) . '` LIKE \'' . $this->dbo->escapeValue($questionValues[$question->name]) . '%\'';
                }
                else
                {
                    $params = array(
                    ...
                    else
                    {
                        ...
                        if ( !empty($questionString) )
                        {
                            $innerJoin .= " INNER JOIN `" . BOL_QuestionDataDao::getInstance()->getTableName() . "` `" . $prefix . $counter . "`
                                ON ( `user`.`id` = `" . $prefix . $counter . "`.`userId` AND `" . $prefix . $counter . "`.`questionName` = '" . $this->dbo->escapeValue($question->name) . "' AND " . $questionString . " ) ";
                        }
                    }
                }
        ...
        {
            $where .= " AND `user`.`accountType` = '" . $this->dbo->escapeValue($questionValues['accountType']) . "' ";
        }
        ...
        return $this->dbo->queryForColumn($query,$queryParams);
    }
    ...
    public function findUserIdListByQuestionValuesQuery( $questionValues, $isAdmin = false, $aditionalParams = array() )
    {
        ...
        if ( !empty($aditionalParams['where']) && $isAdmin )
        {
            $where .= $aditionalParams['where'];
        }
        ...
        if ( $isAdmin === true )
        {
            $query = "SELECT `user`.id FROM {$usersTableName} `user`
            {$innerJoin}
                WHERE 1 {$where} {$group_by}
                ORDER BY {$order}";
        } else{
            ...
            $questionIdsList = "";
            foreach ($questions as $question){
                $questionIdsList .= $question->id . ", ";
            }

            $query = "SELECT * FROM(SELECT $distinct `user`.id, `user`.`activityStamp`, `user`.`username`, `user`.`accountType`  FROM {$usersTableName} `user` " .
                " WHERE `user`.id NOT IN (" .
                "SELECT userId FROM `" . OW_DB_PREFIX ."frmsecurityessentials_question_privacy` WHERE `questionId` IN ( " . substr($questionIdsList, 0, -2) . ") AND" .
                "(`privacy` = 'only_for_me' AND ( userId <> " . $currentUserId  .
                ") OR (privacy = 'friends_only' AND " .
                '`userId` NOT in (SELECT ff.`userId` FROM ' . FRIENDS_BOL_FriendshipDao::getInstance()->getTableName() . ' AS ff WHERE ff.friendId = ' . $currentUserId . ' AND ff.`status` = \'active\' union SELECT ff.`friendId` as userId FROM ' . FRIENDS_BOL_FriendshipDao::getInstance()->getTableName() . ' AS ff WHERE ff.userId = ' . $currentUserId . ' AND ff.`status` = \'active\')' .
                "))" .
                ")) AS user {$innerJoin} ".
                "{$queryParts["join"]} ".
    ...
    public function findUserIdListByQuestionValues( $questionValues, $first, $count, $isAdmin = false, $aditionalParams = array(),$queryParams = array() )
    {
        ...
        $params = array_merge(array('first' => $first, 'count' => $count),$queryParams);
        return $this->dbo->queryForColumnList($query . " LIMIT :first, :count ",$params);
    }
    ...
    {
        $result = '';
        $prefix = $this->dbo->escapeValue($prefix);
        ...
        switch ( $question->presentation )
        {
            ...
            case BOL_QuestionService::QUESTION_PRESENTATION_TEXTAREA :
                ...
                $result = " LCASE(`" . $prefix . "`.`textValue`) LIKE '%" . $this->dbo->escapeValue(strtolower($value)) . "%'";
                break;
            ...
            case BOL_QuestionService::QUESTION_PRESENTATION_FSELECT :
                ...
                if ( !empty($value) )
                {
                    if ( is_array($value) )
                    {
                        $result = ' `' . $this->dbo->escapeValue($prefix) . '`.`intValue` IN ( ' . $this->dbo->mergeInClause($value) . ') ';
                    }                    
                    else if ( (int) $value > 0 )
                    {
                        $result = ' `' . $this->dbo->escapeValue($prefix) . '`.`intValue` = \'' . ((int)$value) . '\' ';
                    }
                }
            ...
            case BOL_QuestionService::QUESTION_PRESENTATION_MULTICHECKBOX :
                ...
                if ( !empty ( $value ) )
                {
                    if ( is_array($value) )
                    {
                        ...
                        {
                            $whereClause.= " (`" . $prefix . "`.`textValue` LIKE '[".$val.",%' OR `" . $prefix . "`.`textValue` LIKE '%,".$val.",%' OR `" . $prefix . "`.`textValue` LIKE '%,".$val."]'". " OR `" . $prefix . "`.`textValue` LIKE '[".$val."]') AND ";
                        }
                        $whereClause.=" 1=1";
            ...
            case BOL_QuestionService::QUESTION_PRESENTATION_AGE :
                ...
                if ( isset($value['from']) && isset($value['to']) )
                {
                    ...
                    $result = " `" . $prefix . "`.`dateValue` BETWEEN  '" . $this->dbo->escapeValue($minDate) . "' AND '" . $this->dbo->escapeValue($maxDate) . "'";
                }
    ...
    public function findUserIdListByPreferenceValues( $preferenceValues, $start = 0, $count = 1000 )
    {
        ...
        foreach ( $preferenceValues as $key => $value )
        {
            $sqlList[$key] = " SELECT d.userId FROM " . (BOL_PreferenceDao::getInstance()->getTableName()) . " p
                ...
                WHERE p.`key` = '" . $this->dbo->escapeValue($key) . "' AND ( d.value = '" . $this->dbo->escapeValue($value) . "' OR d.value IS NULL AND p.defaultValue = '" . $this->dbo->escapeValue($value) . "' ) "
                    . " LIMIT :start, :count ";
            ...
            if ( !empty($value) && is_array($value) )
            {
                $sqlList[$key] = " SELECT d.userId FROM " . (BOL_PreferenceDao::getInstance()->getTableName()) . " p
                    LEFT JOIN " . (BOL_PreferenceDataDao::getInstance()->getTableName()) . " d ON ( d.`key` = p.`key` )
                    WHERE p.`key` = '" . $this->dbo->escapeValue($key) . "' AND ( d.value IN " . $this->dbo->mergeInClause($value) . " OR d.value IS NULL AND p.defaultValue IN " . $this->dbo->mergeInClause($value) . " ) "
                        . " LIMIT :start, :count ";
            }
        }
    ...
    public function updateSaltByUserId($userId,$salt)
    {
        $userId = (int) $userId;
        $sql = " UPDATE `{$this->getTableName()}` SET salt = ? WHERE id = ? LIMIT 1 ";
        $this->dbo->update($sql, array($salt, $userId));
    }
}