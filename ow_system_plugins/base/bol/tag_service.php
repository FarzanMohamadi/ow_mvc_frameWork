...
final class BOL_TagService
{
    ...
    private function __construct()
    {
        ...
        $this->configs[self::CONFIG_DEFAULT_TAGS_COUNT] = 40;
    ...
    {
        foreach($tags as &$tag){
            $tag = UTIL_HtmlTag::stripTagsAndJs($tag);
        }
        $tags = array_map('mb_strtolower', $tags);

        $result = OW_EventManager::getInstance()->trigger(new OW_Event('frmwordscorrection.correct_words',array('words'=>$tags)));
        if($result->getData() !== null && is_array($result->getData()))
        {
            $tags = $result->getData();
        }
    ...
    /**
     * delete Unused Base tags
     *
     */
    public function deleteUnusedBaseTags()
    {
        $this->tagDao->deleteUnusedBaseTags();
    }
     ...
     * 
     * @param BOL_EntityTag $a
     * @param BOL_EntityTag $b
     * @return integer
     */
