...
class BOL_AuthorizationService
{
    ...
    private function __construct()
    {
        ...
        $this->clearAndResetGroupAndActionCache();

        $this->superModeratorUserId = 0;

        $this->clearAndResetModeratorCache();
        ...
        $this->clearAndResetPermissionsCache();
    ...
    public function clearAndResetModeratorCache()
    {
    ...
    public function isActionAuthorized( $groupName, $actionName = null, $extra = null )
    {
        ...
        {
            OW::getLogger()->writeLog(OW_Log::ERROR, $ex->getMessage() . "\n" . print_r($ex->getTrace(), true) );
        }
...
//                        || isset($this->moderatorPermissionCache[$moderatorId][$adminGroupId])
    ...
    public function deleteModerator( $moderatorId )
    {
        ...
        $superModeratorId = (int) $this->getModeratorIdByUserId($this->getSuperModeratorUserId());
        if ( $moderatorId == $superModeratorId )
        {
            return false;
        }
    ...
    public function getLastDisplayLabelRoleOfIdList(array $userIds )
    {
        $userIds = array_unique($userIds);
        $roles = $this->userRoleDao->getLastDisplayLabelRoleOfIdList($userIds);
        $keyRoles = array();
    ...
    public function clearAndResetGroupAndActionCache()
    {
    ...
    public function clearAndResetPermissionsCache()
    {
     ...
     * @return array
     */
    public function findModeratorsByGroupNames( $groupNames = array() )
    {
        $query = "SELECT DISTINCT amp.moderatorId FROM `".OW_DB_PREFIX."base_authorization_group` ag 
        INNER JOIN `".OW_DB_PREFIX."base_authorization_moderator_permission` amp ON 
        ag.id = amp.groupId WHERE ag.name IN ( '" . implode("','", $groupNames) . "' ) 
        ORDER BY amp.moderatorId ASC";
     ...
     * @return array
     */
    public function findModeratorsUserIdByGroupNames( $groupNames = array() )
    {
        $query = "SELECT DISTINCT am.userId FROM `".OW_DB_PREFIX."base_authorization_moderator` am
        INNER JOIN `".OW_DB_PREFIX."base_authorization_moderator_permission` amp ON 
        am.id = amp.moderatorId
        INNER JOIN `".OW_DB_PREFIX."base_authorization_group` ag ON
        ag.id = amp.groupId 
        WHERE ag.name IN ( '" . implode("','", $groupNames) . "' )";
