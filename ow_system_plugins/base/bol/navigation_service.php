...
class BOL_NavigationService
{
    ...
    public function findMenuItemsForMenuList( $menuTypes )
    {
        ...
        $eventMenuItems= OW::getEventManager()->trigger(new OW_Event('on.side.menu.view.check',array('menuItems'=>$items,'menuType'=>$menuTypes)));
        if(isset($eventMenuItems->getData()['menuItems']))
        {
            $items= $eventMenuItems->getData()['menuItems'];
        }
        $resultArray = array();
    ...
    public function getMenuItems( array $menuItems )
    {
        ...
        foreach ( $menuItems as $value )
        {
            ...
            OW::getEventManager()->trigger(new OW_Event('frmmultilingualsupport.show.static.page.name.in.multilingual', array('menuItem'=>$menuItem,'entityId' => $value['id'],'entityType'=>'page')));
