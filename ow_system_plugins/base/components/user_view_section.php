...
class BASE_CMP_UserViewSection extends OW_Component
{
    public function __construct( $section, $sectionQuestions, $data, $labels, $template = 'table', $hideSection = false, $additionalParams = array() )
    {
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_QUESTIONS_DATA_PROFILE_RENDER, array('questions' => $sectionQuestions, 'userId' => $additionalParams['userId'], 'component' => $this)));
        switch ( $template )
        {
            case 'tabs':
