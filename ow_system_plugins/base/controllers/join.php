...
class BASE_CTRL_Join extends OW_ActionController
{
    ...
    public function index( $params )
    {
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_JOIN_CONTROLLER_START));
        ...
        $eventData = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_JOIN_PAGE_RENDER, array('joinCtrl' => $this)));
    ...
    protected function joinUser( $joinData, $accountType, $params )
    {
        ...
        if ( !empty($user->id) )
        {
            if ( BOL_QuestionService::getInstance()->saveQuestionsData($joinData, $user->id) )
            {
                ...
                {
                    $event = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_SEND_VERIFICATION_EMAIL, array('user' => $user, 'SSOEmail' => $SSOEmail)));
                ...
                if ( OW::getConfig()->getValue('base', 'mandatory_user_approve') && OW::getUser()->isAuthenticated() &&  !OW::getConfig()->getValue('base', 'confirm_email')) {
                    OW::getEventManager()->trigger(new OW_Event('base.mandatory_user_approve.edit', array('userId' => $user->id, 'newUser'=>true)));
...
class JoinForm extends BASE_CLASS_UserQuestionForm
{
    ...
    public function __construct( $controller )
    {
        ...
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_JOIN_FORM_RENDER, array('form' => $this, 'controller' => $controller)));
    ...
    public function setQuestionsLabel()
    {
        foreach ( $this->sortedQuestionsList as $question )
        {
            if ( !empty($question['realName']) ) {
                ...
                $event = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::DISTINGUISH_REQUIRED_FIELD, array('element' => $this->getElement($question['name']))));
    ...
    public function addClassToBaseQuestions()
    {
        foreach ( $this->sortedQuestionsList as $question )
        {
            if ( !empty($question['realName']) )
            {
                ...
                if($question['realName'] =="sex"){
                    $this->getElement($question['name'])->addAttribute("id", "SexInput");
                }
            }
        }
    }
    ...
    protected function addFakeQuestions()
    {
        ...
        $questionsChanged = $this->checkQuestionsChanged($oldQuestionList);

        if ( !empty($oldQuestionList) && !$questionsChanged )
        {
        ...
        else
        {
            ...
            foreach ( $this->questions as $sort => $question )
            {
                if ( (string) $question['base'] === '0' && $step === 2 || $step === 1 )
                {
                    ...
                    {
                        $randName = FRMSecurityProvider::generateUniqueId(UTIL_String::getRandomString(rand(5, 13), 2));
                        if(in_array($randName, $markedRandNames)){
                            for($h =0; $h<100; $h++){
                                $randName = FRMSecurityProvider::generateUniqueId(UTIL_String::getRandomString(rand(5, 13), 2), true);
                                if(!in_array($randName, $markedRandNames)){
                        ...
                        $question['trClass'] = FRMSecurityProvider::generateUniqueId('ow_'. UTIL_String::getRandomString(rand(5, 10), 2));
    ...
    protected function addLastStepQuestions( $controller )
    {
        ...
        switch ( $displayPhotoUpload )
        {
            ...
            case BOL_UserService::CONFIG_JOIN_DISPLAY_PHOTO_UPLOAD :
                if (OW::getApplication()->isMobile()) {
                    $userPhoto = OW::getClassInstance("FileField", 'userPhoto');
                }else{
                    $userPhoto = OW::getClassInstance("BASE_CLASS_AvatarField", 'userPhoto');
                }
         ...
         */
        OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_RENDER_JOIN_FORM, array('joinForm' => $this, 'controller' => $controller)));
    ...
    /**
     * check if questions inside session has been changed
     * @param $questionsInsideSession
     * @return bool
     */
    protected function checkQuestionsChanged($questionsInsideSession)
    {
        // email and password does not exist inside session
        $questionsCount = $this->questions ? (count($this->questions)) - 2 : 0;
        $questionsInsideSessionCount = $questionsInsideSession ? count($questionsInsideSession) : 0;

        if($questionsCount != $questionsInsideSessionCount){
            return true;
        }

        if ($questionsCount != 0) {
            foreach ($this->questions as $question) {
                if (in_array($question['name'], ["email", "password"])) {
                    continue;
                }
                if (array_search($question['name'], $questionsInsideSession) == false) {
                    return true;
                }
            }
        }
