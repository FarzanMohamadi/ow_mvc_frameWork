...
class BASE_CTRL_MediaPanel extends OW_ActionController
{
    ...
    public function __construct()
    {
        ...
        if ( !OW::getRequest()->isAjax() ) 
        {
            ...
            OW::getEventManager()->trigger(new OW_Event('frmmenu.hide.unwanted.element'));
        }
    ...
    public function ajaxUpload( $params )
    {
        ...
        $result = array();
        if(!OW::getUser()->isAuthenticated() || (!OW::getUser()->isAdmin() && !OW::getUser()->isAuthorized($pluginKey) && !OW::getUser()->isAuthorized($pluginKey,'add')))
        {
            $result = array(
                'error_message' => OW::getLanguage()->text('base', 'user_is_not_authenticated')
            );
        ...
        if (OW::getRequest()->isPost())
        {
            if ( !empty($_POST['command']) && $_POST['command'] == 'image-upload' )
            {
                ...
                $checkAnotherExtensionEvent = OW::getEventManager()->trigger(new OW_Event('frmclamav.is_file_clean', array('path' => $_FILES['file']['tmp_name'])));
                if (isset($checkAnotherExtensionEvent->getData()['clean'])) {
                    $isClean = $checkAnotherExtensionEvent->getData()['clean'];
                    if (!$isClean) {
                        $result = array(
                            'error_message' => OW::getLanguage()->text('frmclamav', 'virus_file_found', array('file' => $_FILES['file']['name'])),
                        );
                        die(json_encode($result));
                    }
                }
    ...
    public function index( $params )
    {
        ...
        $this->menu->setItemActive('upload');
    ...
    {
        $this->setDocumentKey("frm_media_panel_gallery");
...
class UploadImageForm extends Form
{
    ...
    public static function addFile( $plugin )
    {
        ...
        $file_name = FRMSecurityProvider::generateUniqueId() . '.' . UTIL_File::getExtension($name);
