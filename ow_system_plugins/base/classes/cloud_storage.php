...
//            }
    ...
    public function fileSetContent( $destPath, $content, $direct = false )
    {
        if ( empty($content) )
        {
            return false;
        ...
        do
        {
            $tmpFilename = $this->cloudfilesTmpDir . FRMSecurityProvider::generateUniqueId();
        }
        while ( OW::getStorage()->fileExists($tmpFilename) );

        file_put_contents($tmpFilename, $content);

        if ( !OW::getStorage()->fileExists($tmpFilename) )
        {
            return false;
        }
        ...
        OW::getStorage()->removeFile($tmpFilename);

        return $result;
    }

    public function removeFile( $path, $direct = false )
    {
        if ( $this->isFile($path) )
        {
//            try
