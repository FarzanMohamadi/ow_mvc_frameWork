...
class ADMIN_CTRL_Pages extends ADMIN_CTRL_Abstract
{
    ...
    public function manage()
    {
        ...
        $eventForEnglishSupport = new OW_Event('frmmultilingualsupport.show.static.page.name.in.multilingual', array('menuItems'=>$mainMenuItems,'entityType'=>'page'));
        OW::getEventManager()->trigger($eventForEnglishSupport);
        if(isset($eventForEnglishSupport->getData()['menuItems'])){
            $mainMenuItems = $eventForEnglishSupport->getData()['menuItems'];
        }
        usort($mainMenuItems, 'compare');
        $bottomMenuItems = $service->findMenuItems(BOL_NavigationService::MENU_TYPE_BOTTOM);
        $eventForEnglishSupport = new OW_Event('frmmultilingualsupport.show.static.page.name.in.multilingual', array('menuItems'=>$bottomMenuItems,'entityType'=>'page'));
        OW::getEventManager()->trigger($eventForEnglishSupport);
        if(isset($eventForEnglishSupport->getData()['menuItems'])){
            $bottomMenuItems = $eventForEnglishSupport->getData()['menuItems'];
        }
        usort($bottomMenuItems, 'compare');
        $hiddenMenuItems = $service->findMenuItems(BOL_NavigationService::MENU_TYPE_HIDDEN);
        $eventForEnglishSupport = new OW_Event('frmmultilingualsupport.show.static.page.name.in.multilingual', array('menuItems'=>$hiddenMenuItems,'entityType'=>'page'));
        OW::getEventManager()->trigger($eventForEnglishSupport);
        if(isset($eventForEnglishSupport->getData()['menuItems'])){
            $hiddenMenuItems = $eventForEnglishSupport->getData()['menuItems'];
        }
...
//                ->setDescription(OW::getLanguage()->text('admin', 'pages_page_field_meta_desc'));
        ...
        $metaTagsTextarea = new Textarea('meta-tags');
        $metaTagsTextarea->setLabel(OW::getLanguage()->text('admin', 'pages_page_field_meta_desc'))
                ->setId('meta-tags')
                ->setDescription(OW::getLanguage()->text('admin', 'pages_page_field_meta_desc'));
        ...
        $localUrlTextField->setInvitation('http://yoursite.com/')->setHasInvitation(true);
...
//~
        ...
        $eventForEnglishFieldSupport = new OW_Event('frmmultilingualsupport.store.multilingual.data', array('entityId' => $menuItem->getId(),'entityType'=>'page'));
