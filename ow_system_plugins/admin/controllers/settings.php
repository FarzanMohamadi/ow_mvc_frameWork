...
class ADMIN_CTRL_Settings extends ADMIN_CTRL_Abstract
{
    ...
    public function index()
    {
        ...
        $configSaveForm = new ADMIN_ConfigSaveForm();
        $this->addForm($configSaveForm);
        $currencyEvent = OW::getEventManager()->trigger(new OW_Event(FRMEventManager::ON_BEFORE_CURRENCY_FIELD_APPEAR));
    ...
    public function userInput()
    {
        ...
        if(!$config->configExists('base', 'image_compression_percentage')){
            $config->saveConfig('base', 'image_compression_percentage', 90);
        }
        ...
        $resourceList->setValue(implode(PHP_EOL, json_decode($config->getValue('base', 'tf_resource_list'), true)));
        if($attchMaxUploadSize != null) {
            $attchMaxUploadSize->setValue(round((float)$config->getValue('base', 'attch_file_max_size_mb'), 2));
        }
        $attchExtList->setValue(implode(PHP_EOL, json_decode($config->getValue('base', 'attch_ext_list'), true)));
    ...
    public function user()
    {
        ...
        $userSettingsForm->getElement('disable_signup_button')->setValue($conf->getValue('base', 'disable_signup_button'));
        ...
        if ( OW::getRequest()->isPost() && $userSettingsForm->isValid($_POST) )
        {
            ...
            $guestPassword = OW::getConfig()->getValue('base', 'guests_can_view_password');
    ...
    public function page()
    {
        ...
        if($customizePage) {
            ...
            $headCode2 = new Textarea('mobile_head_code');
            $headCode2->setLabel($language->text('admin', 'page_settings_form_headcode_label') . ' ['.$language->text('base','mobile_version_menu_item').']');
            $headCode2->setDescription($language->text('admin', 'page_settings_form_headcode_desc'));
            $form->addElement($headCode2);

            $bottomCode2 = new Textarea('mobile_bottom_code');
            $bottomCode2->setLabel($language->text('admin', 'page_settings_form_bottomcode_label') . ' ['.$language->text('base','mobile_version_menu_item').']');
            $bottomCode2->setDescription($language->text('admin', 'page_settings_form_bottomcode_desc'));
            $form->addElement($bottomCode2);
        }
        ...
        if ( OW::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                if($customizePage) {
                    ...
                    OW::getConfig()->saveConfig('base', 'html_head_code_mobile', $data['mobile_head_code']);
                    OW::getConfig()->saveConfig('base', 'html_prebody_code_mobile', $data['mobile_bottom_code']);
                }
        ...
        if($customizePage) {
            $headCode->setValue(htmlentities(OW::getConfig()->getValue('base', 'html_head_code')));
            $bottomCode->setValue(htmlentities(OW::getConfig()->getValue('base', 'html_prebody_code')));
            $headCode2->setValue(htmlentities(OW::getConfig()->getValue('base', 'html_head_code_mobile')));
            $bottomCode2->setValue(htmlentities(OW::getConfig()->getValue('base', 'html_prebody_code_mobile')));
        }
     ...
     * @return Selectbox
     */
    private function getLogLevelInput($input_name, $selected){
        $language = OW::getLanguage();
        $tmpField1 = new Selectbox($input_name);
    ...
    public function log()
    {
        ...
        $tmpField1 = $this->getLogLevelInput('file_log_level', $val_tmp);
        ...
        $tmpField1 = $this->getLogLevelInput('stdout_log_level', $val_tmp);
        $form->addElement($tmpField1);
        ...
        // syslog log
        $enabled_3 = new CheckboxField('syslog_log_enabled');
        $val_tmp = isset($configs['syslog_log_enabled'])? $configs['syslog_log_enabled']: false;
        $enabled_3->setValue($val_tmp);
        $form->addElement($enabled_3);

        $val_tmp = isset($configs['syslog_log_level'])? $configs['syslog_log_level']: $def_val_1;
        $tmpField1 = $this->getLogLevelInput('syslog_log_level', $val_tmp);
        $form->addElement($tmpField1);

        $tmpField3 = new Selectbox('syslog_output_format');
        $val_tmp = isset($configs['syslog_output_format'])? $configs['syslog_output_format']: $def_val_3;
        $tmpField3->setLabel($language->text('admin', 'settings_log_output_format_label'))
            ->setValue($val_tmp)
            ->setRequired();
        $tmpField3->addOption('default', $language->text('admin','default'));
        ...
        $tmpField1 = $this->getLogLevelInput('elastic_log_level', $val_tmp);
        $form->addElement($tmpField1);
        ...
        $element_un = new TextField('elastic_username');
        $val_tmp = isset($configs['elastic_username'])? $configs['elastic_username']: 'default';
        $element_un->setValue($val_tmp)->setLabel('Username');
        $form->addElement($element_un);

        $element_pw = new TextField('elastic_password');
        $val_tmp = isset($configs['elastic_password'])? $configs['elastic_password']: 'default';
        $element_pw->setValue($val_tmp)->setLabel('Password');
        $form->addElement($element_pw);
        ...
        if ( OW::getRequest()->isPost() && $form->isValid($_POST) )
        {
            ...
            $config->saveConfig('base', 'file_log_enabled', $data['file_log_enabled']==1);
            $config->saveConfig('base', 'stdout_log_enabled', $data['stdout_log_enabled']==1);
            $config->saveConfig('base', 'elastic_log_enabled', $data['elastic_log_enabled']==1);
            $config->saveConfig('base', 'syslog_log_enabled', $data['syslog_log_enabled']==1);

            $values = ['file_log_level', 'file_output_format',
                'stdout_log_level', 'stdout_output_format',
                'syslog_log_level', 'syslog_output_format',
                'elastic_log_level', 'elastic_host', 'elastic_port', 'elastic_username', 'elastic_password'];
        ...
        $tmpField9 = new TextField('repeat');
        $tmpField9->setLabel($language->text('base', 'user_list_widget_settings_count'))
            ->setValue('1')
            ->setRequired()
            ->addAttribute('type','number');
        $form2->addElement($tmpField9);
        ...
        if ( OW::getRequest()->isPost() && $form2->isValid($_POST) )
        {
            ...
            $repeat = (isset($_POST['repeat']))?intval($_POST['repeat']):1;
            for ($i = 0; $i < $repeat; $i++) {
                OW::getLogger()->writeLog($_POST['log_level'], $_POST['title'] . ' ' . ($i + 1) . '/' . $repeat . ' @'.$username);
            }
        ...
        fclose($file);
        $latestLogs = implode(PHP_EOL, $lines);
        $this->assign("LatestLogs", $latestLogs);

        //download log file
        $downloadForm = new Form('downloadForm');
        $dl_path = new Selectbox('dl_path');
        foreach(UTIL_File::findFiles(OW_DIR_LOG, ['log', 'zip']) as $file){
            $dl_path->addOption($file,$file);
        }
        $downloadForm->addElement($dl_path);
        $submit = new Submit('download');
        $submit->setValue('>'); //$language->text('admin', 'download_log_file')
        $downloadForm->addElement($submit);
        $this->addForm($downloadForm);
        if ( OW::getRequest()->isPost() && $downloadForm->isValid($_POST) )
        {
            $path = (!empty($_POST['dl_path']))?$_POST['dl_path']:$path;
 ...
 */
class ADMIN_ConfigSaveForm extends Form
{
...
//        $this->addElement($imageMaxSizeField);
        ...
        // -- socket settings --
        $socket = new CheckboxField('enableSocket');
        $socket->setValue(OW::getConfig()->getValue('base', 'socket_enabled'));
        $this->addElement($socket);

        $socketHost = new TextField('socketHost');
        $socketHost->setValue(OW::getConfig()->getValue('base', 'socket_host'));
        $socketHost->setRequired(false);
        $this->addElement($socketHost);

        // -- error reporting settings --
        $error_debug = new CheckboxField('debug_enabled');
        $error_debug->setValue(self::getDebugModeEnabled());
        $this->addElement($error_debug);

        $error_reporting = new CheckboxField('error_reporting_enabled');
        $error_reporting->setValue(self::getErrorEnabled());
        $this->addElement($error_reporting);
    ...
    private static function getErrorEnabled(){
        $error = ini_get('display_errors');
        $error = (empty($error))?false:($error=='1');
        if(!$error){
            return false;
        }
        ...
        foreach($data as $k=>$line){
            if (strpos($line, "ini_set('display_errors'")!==false) {
                return true;
            }
        }
        return false;
    ...
    private static function getDebugModeEnabled() {
        return defined("OW_DEBUG_MODE")?OW_DEBUG_MODE:false;
    }
...
//        $config->saveConfig('base', 'tf_max_pic_size', $values['tf-max-image-size']);
        ...
        $config->saveConfig('base', 'socket_host', $values['socketHost']);
        $config->saveConfig('base', 'socket_enabled', $values['enableSocket']);

        try {
            if (self::getDebugModeEnabled() != $values['debug_enabled']) {
                $filename = OW_DIR_ROOT . 'ow_includes' . DS . 'config.php';
                $data = file($filename); // reads an array of lines
                $stringVal = $values['debug_enabled'] ? 'true' : 'false';
                foreach ($data as $k => $line) {
                    if (strpos($line, 'OW_DEBUG_MODE') !== false) {
                        $data[$k] = "    define('OW_DEBUG_MODE', " . $stringVal . ");\r\n";
                    }
                }
                file_put_contents($filename, implode('', $data));
                sleep(1);
            }
            if (self::getErrorEnabled() != $values['error_reporting_enabled']) {
                $filename = OW_DIR_ROOT . 'ow_includes' . DS . 'config.php';
                $data = file($filename); // reads an array of lines
                foreach ($data as $k => $line) {
                    if (strpos($line, "ini_set('display_errors'") !== false) {
                    ...
                    if (strpos($line, "ini_set('display_startup_errors'") !== false) {
                        unset($data[$k]);
                    }
                    if (strpos($line, "error_reporting(") !== false) {
                        unset($data[$k]);
                    }
                }
                if ($values['error_reporting_enabled'])
                    $data[] = "\r\n\r\nini_set('display_errors', 1);\nini_set('display_startup_errors', 1);\nerror_reporting(E_ALL);\n";
                file_put_contents($filename, implode('', $data));
                sleep(1);
            }
        }catch (Exception $exception){
            OW::getLogger()->writeLog(OW_Log::ERROR, 'rewrite_config_file', ['cannot rewrite config.php to save error handling']);
        }
...
class UserSettingsForm extends Form
{
    ...
    public function __construct($maxUploadMaxFilesize)
    {
        ...
        $validator->setErrorMessage($language->text('admin', 'user_settings_avatar_size_error', array('max' => 150, 'min' => 40)));
        ...
        $validator = new IntValidator(150, 500);
        $validator->setErrorMessage($language->text('admin', 'user_settings_big_avatar_size_error', array('max' => 500, 'min' => 150)));
        ...
        // --- Join Page -----///
        $config = OW::getConfig();
        if (!OW::getPluginManager()->isPluginActive('frmsso')){
        ...
        $enableSignUpButton = new CheckboxField('disable_signup_button');
        $enableSignUpButton->setLabel($language->text('admin', 'disable_signup_button'));
        $enableSignUpButton->setDescription($language->text('admin', 'disable_signup_button_description'));
        $this->addElement($enableSignUpButton);
    ...
    public function process()
    {
        ...
        $config->saveConfig('base', 'disable_signup_button', ((bool) $values['disable_signup_button'] ? 1 : 0));
        ...
        if((int) $values['guests_can_view'] == 3)
        {
            ...
            $hashedPassword = FRMSecurityProvider::getInstance()->hashSha256Data($salt.$values['password']);
            $values['password'] = json_encode(array('guestPassword'=>$hashedPassword,'guestSalt'=>$salt));
