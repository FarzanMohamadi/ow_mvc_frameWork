...
class ADMIN_CTRL_Questions extends ADMIN_CTRL_Abstract
{
    ...
    public function accountTypes( $params = array() )
    {
        ...
        foreach ( $questions as $section => $list )
        {
            ...
            foreach ( $list as $question )
            {
                ...
                if ( !empty($question['condition']) ) {
                    ...
                    if(!empty($c['question'])) {
                        $label = BOL_QuestionService::getInstance()->getQuestionLang($c['question']);
                        $question['condition'] = OW::getLanguage()->text(
                            'admin',
                            'field_is_related_to',
                            ['label' => $label],
                            $label
        ...
        foreach ( $questionValues as $name => $value )
        {
            ...
            foreach ( $value['values'] as $item )
            {
                if(!empty($item->questionText)){
                    $valueLabels[($item->questionName)][$item->value] = $item->questionText;
                }else {
                    $valueLabels[($item->questionName)][$item->value] = BOL_QuestionService::getInstance()->getQuestionValueLang($item->questionName, $item->value);
                }
            }
        ...
        switch ( $command )
        {
            ...
            case 'confirmDeleteSection':
                ...
                if ( !empty($sectionName) )
                {
                    ...
                    echo json_encode( array(
                        'result' => "success",
                        'message' => OW::getLanguage()->text('admin', 'questions_delete_section_confirmation_without_move_questions' ))
                     );
                    exit;
                }
            ...
            case 'deleteSection':
                ...
                if ( !empty($_POST['sectionName']) && mb_strlen($_POST['sectionName']) > 0 )
                {
                  ...
                  $nonRemovableQuestions = $this->questionService->findNonRemovableQuestionsInSection($_POST['sectionName']);
                  if(empty($nonRemovableQuestions))
                  {
                      if($this->questionService->deleteSectionWithoutMoving($_POST['sectionName'])){
                          $result = array('result' => "success", 'message' => OW::getLanguage()->text('admin', 'questions_section_was_deleted'));
                          echo json_encode($result);
                      }
                      exit;
                  }else{
                      $questionNames = array_column($nonRemovableQuestions,'name');
                      $questions = array();
                      foreach ($questionNames as $questionName)
                      {
                          $questions[] = $this->questionService->getQuestionLang($questionName);
                      }
                      exit(json_encode(array(
                          'result' => "false",
                          'message' => OW::getLanguage()->text('admin', 'cannot_delete_section_with_nonremovable_item', array('questions'=>implode(', ', $questions)))
                          )
                      ));
                  }
                }
           ...
           case 'AddAccountType':
            ...
            case 'addSection' :
                ...
                $questionSection->name = md5(FRMSecurityProvider::generateUniqueId());
