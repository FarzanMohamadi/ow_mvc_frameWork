...
class ADMIN_CLASS_AddQuestionForm extends Form
{
    ...
    public function getPresentations2FormElements()
    {
        ...
        $displayFieldList = array(
            ...
            'qst_section' => true,
            'qst_condition_question' => true,
            'qst_condition_value' => true,
    ...
    protected function init( $params = array() )
    {
        ...
        $q = new Selectbox('qst_condition_question');
        $options = BOL_QuestionService::getInstance()->getQuestions(['radio','fselect','checkbox','multicheckbox','text']);
        $q->addOptions($options);
        $q->setLabel(OW::getLanguage()->text('admin', 'questions_question_condition_question_label'));
        $this->addElement($q);

        $q = new TextField('qst_condition_value');
        $q->setLabel(OW::getLanguage()->text('admin', 'questions_question_condition_value_label'));
        $q->setDescription(OW::getLanguage()->text('admin', 'questions_question_condition_value_desc'));
        $q->setValue('*');
        $this->addElement($q);
    ...
    public function process()
    {
        if ( OW_Request::getInstance()->isPost() )
        {
            ...
            if ( $this->isValid($data) )
            {
                ...
                $question->name = 'field_'.md5(FRMSecurityProvider::generateUniqueId());
                ...
                if(!empty($data['qst_condition_question']) && isset($data['qst_condition_value'])){
                    $question->setCondition(htmlspecialchars($data['qst_condition_question']), htmlspecialchars($data['qst_condition_value']));
                }
