...
class ADMIN_CLASS_EditQuestionForm extends ADMIN_CLASS_AddQuestionForm
{
    ...
    public function loadQuestionData( BOL_Question $question )
    {
        ...
        foreach ( $valuesDto as $value )
        {
            if(!empty($value->questionText)){
                $values[($value->value)] = $value->questionText;
            }else {
                $values[($value->value)] = BOL_QuestionService::getInstance()->getQuestionValueLang($value->questionName, $value->value);
            }
        }
        ...
        $this->getElement('qst_condition_question')->setValue($question->getConditionQuestionName());
        $this->getElement('qst_condition_value')->setValue($question->getConditionValue());
                ...
                foreach ( $this->getElements() as $element )
                {
                    if ( !$element->getAttribute('disabled') )
                    {
                        switch ( $element->getName() )
                        {
                            ...
                            case 'qst_condition_question':
                                if(isset($data['qst_condition_value'])){
                                    $this->question->setCondition(htmlspecialchars($data['qst_condition_question']),
                                        htmlspecialchars($data['qst_condition_value']));
                                }
                                break;
