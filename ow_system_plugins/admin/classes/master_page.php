...
class ADMIN_CLASS_MasterPage extends OW_MasterPage
{
    ...
    protected function init()
    {
        ...
        if ( empty($verString) )
        {
            $verString = OW::getLanguage()->text('admin', 'soft_version_text',
                array(
                    'build' => OW::getConfig()->getValue('base', 'soft_build'),
                    'date' => UTIL_DateTime::formatDate( OW::getConfig()->getValue('base', 'core_update_ts'))
                ));
        }
        ...
        OW::getEventManager()->trigger(new OW_Event('base.on_before_check_update_assign', array('url' => $checkUrl)));
