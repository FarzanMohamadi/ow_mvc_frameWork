    ...
    public function __construct( $questionId )
    {
        ...
        OW.bind( "admin.language_key_edit_success", function( params ) {
            ...
            if ( params && params.result == "success" && params.key )
            {
                ...
                {
                    document.getElementById("ql").innerHTML = params.value;
                    closeFloatbox = true;
                }
                else if ( /^'.$descriptionLang.'/g.test(params.key) )
                {
                    document.getElementById("qd").innerHTML = params.value;
                    closeFloatbox = true;
                }
        ...
        $(".tr_qst_condition_question  select").on("change", function() {
            $("input[name=qst_required]")[0].disabled="";
          if(this.value !== ""){
            $("input[name=qst_required]")[0].checked="";
            $("input[name=qst_required]")[0].disabled="disabled";
          }
        });
        $(".tr_qst_condition_question  select").change();
        ';
